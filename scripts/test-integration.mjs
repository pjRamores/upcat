 ...
 *   Stats  - overview, summary, subject-breakdown, difficulty-breakdown, progress-over-time, weak-areas, leaderboard
...
const MONGODB_DB_NAME = process.env.MONGODB_DB_NAME ?? process.env.MONGODB_DB ?? "upcat";
...
async function call(path, init = {}, token) {
  ...
  try {
    ...
    const msg = controller.signal.aborted ? `request timed out after ${REQUEST_TIMEOUT_MS}ms` : (error?.cause?.message || fallback || "network failure");
    throw new Error(`HTTP ${init?.method ?? "GET"} ${url} failed: ${msg}.` + `Ensure the API server is running (npm run dev:api) and API_BASE_URL is correct.`);
  } finally {
...
log("POST /auth/login (wrong password)", r.status === 401, `status=${r.status}`);
...
log(
    ...
    `status=${r.status} sessionId=${sessionId}`,
);
