...
async function setupIndexes() {
  ...
  try {
    const db = client.db(
      process.env.MONGODB_DB ?? process.env.MONGODB_DB_NAME ?? undefined
    );
    ...
    await sessions.createIndex(
      { "offlineData.syncedAt": 1 },
      { sparse: true, name: "offline_synced_at" }
    );
    console.log("exam_sessions indexes created");

    // question_sets / exam_set_assignments / exam_set_assignment_events
    console.log(
      "Creating indexes on 'question_sets', 'exam_set_assignments', and 'exam_set_assignment_events'..."
    );

    const questionSets = db.collection("question_sets");
    await questionSets.createIndex(
      { setId: 1 },
      { unique: true, name: "set_id_unique" }
    );
    await questionSets.createIndex(
      { isActive: 1, subjectArea: 1, difficulty: 1 },
    ...
    await syncQueue.createIndex(
      { userId: 1, status: 1, receivedAt: -1 },
      { name: "user_status_received" }
    );
    await syncQueue.createIndex(
      { sessionId: 1, sequenceNumber: 1 },
      { sparse: true, name: "session_sequence" }
    );
    await syncQueue.createIndex(
      { expiresAt: 1 },
      { expireAfterSeconds: 0, name: "sync_queue_ttl" }
    );
    ...
    await recovery.createIndex(
      { userId: -1, sessionId: 1 },
      { unique: true, name: "user_session_unique" }
    );
    await recovery.createIndex(
      { status: 1, updatedAt: -1 },
      { name: "status_updated" }
    ...
    await recovery.createIndex(
      { expiresAt: 1 },
      { expireAfterSeconds: 0, name: "session_recovery_ttl" }
    );
    ...
    await windows.createIndex(
      ...
      { name: "status_scheduled_start" }
    );
    await windows.createIndex(
      { status: 1, scheduledEnd: 1 },
      { name: "status_scheduled_end" }
    );
    ...
    await connectionEvents.createIndex(
    ...
    await connectionEvents.createIndex(
      { expiresAt: 1 },
      { expireAfterSeconds: 0, name: "connection_events_ttl" }
    );
