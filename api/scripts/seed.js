...
const questions = [
  ...
  q({
    ...
    subtopic: "Algebra",
    difficulty: "medium",
    questionText: "If f(x) = x^2 - 3x + 2, what is the value of f(4)?",
    choices: [
      { label: "A", text: "2" },
      { label: "B", text: "6" },
      { label: "C", text: "8" },
    ...
    correctAnswer: "B",
    rationale: "Substitute x = 4: f(4) = 16 - 12 + 2 = 6.",
  ...
  q({
    ...
    choices: [
      { label: "A", text: "16 sq.cm" },
      { label: "B", text: "30 sq.cm" },
      { label: "C", text: "60 sq.cm" },
      { label: "D", text: "120 sq.cm" },
    ],
    correctAnswer: "B",
    rationale: "Area of a triangle = (1/2) x base x height = (1/2) x 10 x 6 = 30 sq.cm.",
    tags: ["area", "triangles"],
  }),
  q({
    subjectArea: "Mathematics",
    subtopic: "Geometry",
    difficulty: "medium",
    questionText: "A right triangle has legs of length 5 and 12. What is the length of the hypotenuse?",
    choices: [
      { label: "A", text: "13" },
      { label: "B", text: "15" },
      { label: "C", text: "17" },
    ...
    correctAnswer: "A",
    rationale: "By the Pythagorean theorem: c^2 = 5^2 + 12^2 = 25 + 144 = 169, so c = 13.",
  ...
  q({
    ...
    correctAnswer: "B",
    rationale: "sin(30°) = 1/2 is one of the standard trigonometric values from the 30-60-90 triangle.",
  ...
  q({
    ...
    correctAnswer: "A",
    rationale:
      "Apply the power rule term by term: d/dx(3x^4) = 12x^3, d/dx(-2x^2) = -4x, d/dx(7) = 0.",
  ...
  q({
    ...
    choices: [
      ...
      {
        label: "B",
        text: "Each new DNA molecule contains one old strand and one new strand.",
      },
      { label: "C", text: "Only half of the chromosome is copied." },
      {
        label: "D",
        text: "The process conserves only half of the genetic information.",
      },
    ],
    correctAnswer: "B",
    rationale:
      "In semi-conservative replication, each daughter DNA molecule contains one original strand and one newly synthesized strand.",
    tags: ["genetics", "DNA"],
  }),
  q({
    subjectArea: "Science",
  ...
  q({
    ...
    correctAnswer: "B",
    rationale:
      "Pure water at 25°C has equal concentrations of H+ and OH- ions (1 x 10^-7 M each), giving a pH of 7.",
    tags: ["acids-bases", "pH"],
  }),
  q({
    subjectArea: "Science",
  ...
  q({
    ...
    correctAnswer: "A",
    rationale:
      "Newton's third law: when one body exerts a force on a second body, the second body exerts an equal and opposite force on the first.",
  ...
  q({
    ...
    correctAnswer: "C",
    rationale:
      "Earth's structure from outside in: crust → mantle → outer core → inner core. The mantle is the thick layer just below the crust.",
  ...
  q({
    ...
    correctAnswer: "B",
    rationale:
      "In American English, collective nouns like 'team' typically take a singular verb when treated as a single unit: 'The team is...'.",
  ...
  q({
    ...
    correctAnswer: "C",
    rationale:
      "Benevolent means well-meaning and kindly. Its direct antonym is malevolent - wishing evil or harm to others.",
  ...
  q({
    ...
    subtopic: "Vocabulary",
    difficulty: "easy",
    questionText: "Which word is a synonym of EPHEMERAL?",
    choices: [
  ...
  q({
    ...
    subtopic: "Grammar",
    difficulty: "hard",
    questionText: "Choose the sentence that uses the semicolon correctly:",
    choices: [
    ...
    correctAnswer: "B",
    rationale:
      "A semicolon joins two related independent clauses. Option B has two complete sentences on either side.",
  ...
  q({
    ...
    subtopic: "Vocabulary",
    difficulty: "medium",
    questionText: "The word UBIQUITOUS most nearly means:",
    choices: [
      { label: "A", text: "Rare" },
      { label: "B", text: "Hidden" },
    ...
    correctAnswer: "C",
    rationale: "'Ubiquitous' means present, appearing, or found everywhere.",
    tags: ["vocabulary"],
  }),
  ...
  q({
    ...
    choices: [
      ...
      {
        label: "B",
        text: "Photosynthesis converts light energy into chemical energy and is essential to life.",
      },
    ...
    correctAnswer: "B",
    rationale:
      "The passage opens by defining photosynthesis and concludes with its essential role in producing oxygen and forming the base of food chains.",
  ...
  q({
    ...
    choices: [
      ...
      { label: "D", text: "Sunlight contains more green light than other colors." },
    ],
    correctAnswer: "C",
    rationale:
  ...
  q({
    ...
    choices: [
      ...
      {
        label: "B",
        text: "Atmospheric oxygen would gradually decrease and food chains would collapse.",
      },
    ...
    correctAnswer: "B",
    rationale:
      "The passage states that photosynthesis produces nearly all atmospheric oxygen and forms the base of nearly every food chain.",
  ...
  q({
    ...
    type: "passage_based",
    passageId: PASSAGE_RIZAL_ID,
    questionText: "According to the passage, where and when was Rizal executed?",
    choices: [
...
async function seed() {
  ...
  try {
    ...
    console.log(
      `→ Seed mode: ${CLEAN ? "CLEAN (drop existing first)" : "APPEND (skip if non-empty)"}\n`
    );
    ...
    if (!CLEAN) {
      ...
      if (existing > 0) {
        console.log(`⚠️ 'questions' already has ${existing} documents. Skipping seed.`);
    ...
    for (const qq of questions) {
      ...
      byDifficulty[qq.difficulty] = (byDifficulty[qq.difficulty] || 0) + 1;
    }
...
async function backfillQuestionSetIds(db) {
  ...
  if (migrateLegacy.modifiedCount > 0) {
    console.log(
      `→ Migrated ${migrateLegacy.modifiedCount} question(s) from '${LEGACY_PREDEFINED_SET_ID}' to '${PREDEFINED_SET_ID}'.`
    );
  }

  const result = await db.collection("questions").updateMany(
    {
      $or: [{ setId: { $exists: false } }, { setId: null }, { setId: "" }],
    },
    {
      $set: {
        setId: PREDEFINED_SET_ID,
...
async function ensurePredefinedQuestionSet(db) {
  ...
  if (migrateLegacySetDoc.modifiedCount > 0) {
    console.log(
      `→ Migrated ${migrateLegacySetDoc.modifiedCount} question_set document(s) from '${LEGACY_PREDEFINED_SET_ID}' to '${PREDEFINED_SET_ID}'`
    );
  }
  ...
  const bySubject = await db
    ...
    .aggregate([{ $match: match }, { $group: { _id: "$subjectArea", n: { $sum: 1 } } }])
    .toArray();
    ...
    SUBJECT_AREAS.map((subject) => {
      ...
      const timeLimit =
        totalQuestions > 0
          ? Math.max(
              1,
              Math.round((count * Number(DEFAULT_EXAM_CONFIG.timeLimit || 180)) / Math.max(1, totalQuestions))
            )
          : Math.max(
              1,
              Math.round(
                (count * Number(DEFAULT_EXAM_CONFIG.timeLimit || 180)) /
                  Math.max(1, Number(DEFAULT_EXAM_CONFIG.totalQuestions || 100))
              )
            );

      return [subject, { questions: count, timeLimit }];
    })
  );

  const byDifficulty = await db
    .collection("questions")
    .aggregate([{ $match: match }, { $group: { _id: "$difficulty", n: { $sum: 1 } } }])
    .toArray();
  ...
  const totalTimeLimit = Object.values(distribution).reduce(
    (sum, cfg) => sum + (cfg.timeLimit || 0),
    0
  );
  ...
  console.log(`→ Ensured predefined question set '${PREDEFINED_SET_ID}' is present.`);
}
...
async function seedGamificationCatalogs(db) {
  ...
  for (const a of achievements) {
    const r = await achievementsCol.updateOne(
      { id: a.id },
      {
        $set: { ...a, hidden: false, isActive: true, updatedAt: now },
  ...
  for (const c of challenges) {
    const r = await challengesCol.updateOne(
      { id: c.id },
      {
        $set: { ...c, threshold: c.threshold ?? null, isActive: true, updatedAt: now },
        $setOnInsert: { createdAt: now },
      },
      { upsert: true }
    );
...
async function seedPaymentAndSubscriptionDefaults(db) {
  ...
  console.log(
    `\nPayment defaults: ${paymentResult.upsertedCount > 0 ? "config seeded" : "config exists"}, ${promoInserted} promo code(s) inserted.`
  );
}
