...
import { DEFAULT_EXAM_CONFIG, DEFAULT_PAYMENT_CONFIG, SUBJECT AREAS } from "@upcat/shared";
...
correctAnswer: "B",
rationale:
"In semi-conservative replication, the parental DNA strands separate and each serves as a template, so each daughter molecule has one original (conserved) strand and one newly synthesized strand.",
tags: ["genetics", "DNA"],
}),
q({
subjectArea: "Science",
...
script
{
    label: "C", text: "Chlorophyll reflects green light while absorbing other wavelengths."},
    {label: "D", text: "Sunlight contains more green light than other colors."},
], correctAnswer: "C", rationale: "The passage states chlorophyll absorbs blue and red wavelengths most efficiently and reflects green light.", tags: ["detail", "biology"],
}),
q({
subjectArea: "Reading Comprehension",
subtopic: "Inference",
...
async function ensurePredefinedQuestionSet(db) {
    ...
    const distribution = Object.fromEntries(
        SUBJECT_AREAS.map((subject) => {
            const questions = totalQuestions > 0 ? Math.max(0, Number(subjectCountMap.get(subject) || 0)) : Math.max(0, Number(fallbackDistribution[subject] || 0));
            const timeLimit = totalQuestions > 0 ? Math.max(1, Math.round((questions * Number(DEFAULT_EXAM_CONFIG.timeLimit || 180)) / Math.max(1, totalQuestions))) : Math.max(1, Math.round((questions * Number(DEFAULT_EXAM_CONFIG.timeLimit || 180)) / Math.max(1, Number(DEFAULT_EXAM_CONFIG.totalQuestions || 100))));
