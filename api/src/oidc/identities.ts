...
export async function findIdentityByProvider(
    ...
    return db.collection<UserIdentityDoc>("user_identities").findOne({ provider, providerUserId });
}
...
export async function listUserIdentities(
    ...
    return db.collection<UserIdentityDoc>("user_identities").find({ userId }).toArray();
}

export async function countUserIds(db: Db, userId: ObjectId): Promise<number> {
