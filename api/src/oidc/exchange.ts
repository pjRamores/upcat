...
export async function exchangeCode(args: {
    ...
    try {
        ...
        throw new Error(`[${provider}] token endpoint returned non-JSON (${res.status})`);
    }
...
export async function normalizeProfile(args: {
    ...
    if (provider === "facebook") {
        ...
        const composed = [asString(u.first_name), asString(u.last_name)].filter(Boolean).join(" ") || null;
        return {
            provider,
            providerUserId: id,
    ...
    const needFallback = !claims.email || !claims.name || !claims.picture || claims.email_verified === undefined;
