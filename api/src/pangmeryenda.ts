...
export class PangMeryendaService {
    ...
    verifyWebhookSignature(payload: string, signature: string): boolean {
        const expected = crypto.createHmac("sha256", this.config.webhookSecret).update(payload).digest("hex");
    ...
    async getPaymentStatus(transactionId: string): Promise<{
        ...
        const response = await fetch(`${this.config.apiBaseUrl.replace(/\/$/, "")}/v1/payments/${encodeURIComponent(transactionId)}`, {
            method: "GET",
            headers: {
                "X-PangMeryenda-Key": this.config.apiKey,
            },
        ...
        const data = (await response.json()) as { status: string; paidAt?: string; amount: number };
        return {
            status: data.status,
...
async refundPayment(transactionId: string, reason: string): Promise<boolean> {
    ...
    const response = await fetch(
        `${this.config.apiUrl.replace(/\/$/, "")}/v1/payments/${encodeURIComponent(transactionId)}/refund`,
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
