...
import { PracticeCard, PracticeCardStatus, PracticeMode, PracticeRating, PracticeSessionCard } from "@upcat/shared";
import { PRCTICE_DEFAULTS, SRS_DEFAULT_EASE, SRS_MASTERY_EASE, SRS_MASTERY_INTERVAL_DAYS, SRS_MIN_EASE } from "@upcat/shared";
...
// Weak-area detection (only used for weak_areas mode).
...
accuracy: {
    $cond: [
        {$gt: [{$add: ["Scorrect", "Sincorrect"]}, 0]],
        {
            $divide: [
                "Scorrect",
                {$add: ["Scorrect", "Sincorrect"]},
            ],
            -1,
        },
    ],
...
.toArray();
weakSubjects = accuracy.map((r) => String(r.id));
...
export async function computePracticeStats(
    ...
    const [statusAgg, dueTodayCount, dueWeekCount, subjectAgg, upcomingAgg, retentionAgg, totalReviewsAgg] = await Promise.all([
        col
            .aggregate([
                {$match: {userId}},
    ...
    cards: {$sum: 1},
    mastered: {
        $sum: {$cond: [{seq: ["$status", "mastered"]}, 1, 0]},
    },
    correct: {$sum: "$correctCount"},
    incorrect: {$sum: "$incorrectCount"},
    dueToday: {
...
const retention = retentionAgg.length > 0 && retentionAgg[0].total > 0
