 ...
 *   - award their xpReward + points (xp_awarded inline, no streak multiplier)
...
async function perSubjectPerfectCounts(
  ...
  for (const s of sessions) {
    const by = (s.score?.bySubject ?? {}) as Record<string, { percentage?: number; total?: number }>;
    for (const subject in by) {
      if (by[subject].total === 0) continue;
      const percentage = (by[subject].correct / by[subject].total) * 100;
      out[subject] = out[subject] ?? 0;
      out[subject] += percentage;
    }
  }
  return out;
}
