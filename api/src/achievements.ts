...
function evaluateCondition(
    ...
    switch (cond.kind) {
        ...
        case "scoreThreshold": {
            // Count tracked via achievement.progress map keyed by `score_ge_<gte>`.
...
/**
 * Public listing for the user's achievements page. Hidden achievements that haven't been unlocked yet are returned with their description blanked out.
 */
export async function listUserAchievements(
    ...
    const unlockedSet = new Set(block.achievements.unlocked);
    const unlockedAtMap = (block.achievements as { updatedAt?: Record<string, string> }).updatedAt ?? {};
    ...
    return catalog
        .map<UserAchievement>((def) => {
            ...
            return {
                ...
                unlocked: isUnlocked,
                updatedAt: unlockedAtMap[def.id] ?? null,
...
/**
 * Increment a per-threshold counter (used by exam/submit before evaluating scoreThreshold conditions). Pass thresholds the score qualifies for, e.g. 92 -> bumps both 'score_ge_80' and 'score_ge_90'.
 */
