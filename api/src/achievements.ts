 ...
 * evaluates each one against a user's current gamification stats,
 * and unlocks any that newly qualify. Newly unlocked achievements:
...
export interface CatalogDoc
  extends Omit<AchievementDef, "_id" | "createdAt" | "updatedAt"> {
  _id: ObjectId;
  createdAt: Date;
  updatedAt: Date;
}
...
type ExamSessionDoc = {
  score?: {
    bySubject?: Record<string, { percentage?: number; total?: number }>;
  };
};
 ...
 * Hits the `exam_sessions` collection but bounded by userId so this is cheap.
 */
async function perSubjectPerfectCounts(
  ...
  const sessions = await db
    .collection<ExamSessionDoc>("exam_sessions")
    .find(
...
export async function evaluateAchievements(
  ...
  const unlockedAtPatch: Record<string, string> = {};

  let xpToAdd = 0;
  let pointsToAdd = 0;

  for (const def of catalog) {
    const { current, unlocked } = evaluateCondition(
      def.condition,
      block,
      perSubject,
    );
    ...
    if (unlocked && !unlockedSet.has(def.id)) {
      ...
      unlockedAtPatch[`gamification.achievements.unlockedAt.${def.id}`] =
        unlockedAt;
    ...
    { _id: userId },
    {
      $set: {
        ...
        "gamification.achievements.points":
          block.achievements.points + pointsToAdd,
...
export async function listUserAchievements(
  ...
  const block = ensureGamification(user);
  const catalog = await loadCatalog(db);
  const perSubject = await perSubjectPerfectCounts(db, user._id as ObjectId);
  const unlockedSet = new Set(block.achievements.unlocked);
  const unlockedAtMap = (
    block.achievements as { unlockedAt?: Record<string, string> }
  ).unlockedAt ?? {};

  return catalog
    .map<UserAchievement>((def) => {
      const isUnlocked = unlockedSet.has(def.id);
      const { current, target } = evaluateCondition(
        def.condition,
        block,
        perSubject,
      );
      ...
      return {
        ...
        description: visible
          ? def.description
          : "Hidden - keep practicing to discover this achievement.",
 ...
 * scoreThreshold conditions). Pass thresholds the score qualifies for,
 * e.g. 92 -> bumps both 'score_ge_80' and 'score_ge_90'.
 */
...
/**
 * Seed (or update) the achievements catalog with the shared default set.
 */
