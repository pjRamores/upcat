...
export async function getPlatformSettings(db: Db): Promise<PlatformSettings> {
    ...
    if (!doc) return JSON.parse(JSON.stringify(DEFAULT_PLATFORM_SETTINGS)) as PlatformSettings;
    // Strip _id so the shape matches PlatformSettings exactly.
    const { id: _ignored, ...rest } = doc as PlatformSettings & {_id: string};
    void _ignored;
...
export async function savePlatformSettings(
    ...
    const merged: PlatformSettings = {
        ...current,
        ...patch,
        examDefaults: {...current.examDefaults, ...(patch.examDefaults ?? {})},
        registration: {...current.registration, ...(patch.registration ?? {})},
        leaderboard: {...current.leaderboard, ...(patch.leaderboard ?? {})},
        maintenance: {...current.maintenance, ...(patch.maintenance ?? {})},
        contact: {...current.contact, ...(patch.contact ?? {})},
        scoring: {...current.scoring, ...(patch.scoring ?? {})},
        ads: patch.ads
            ? {
                ...{(current.ads ?? {}), ...patch.ads},
                slots: {...(current.ads?.slots ?? {}), ...(patch.ads.slots ?? {})},
                video: patch.ads.video
                    ? {...(current.ads?.video ?? {}), ...patch.ads.video}
                    : current.ads?.video,
