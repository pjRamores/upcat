...
const SECURITY_HEADERS: Record<string, string> = {
  ...
  // API responses are pure JSON. CSP is set conservatively here; the frontend ships its own CSP via the CDN/host config.
 ...
 * Returns `true` if the request has already been handled (caller must stop processing immediately).
 */
...
/**
 * Sliding-window rate limiter. Returns `true` when the request has been rejected (429 already sent) and `false` when it may proceed.
...
export async function rateLimit(
  ...
  if (opts.scope === "ip" || opts.scope === "ip+user") {
    parts.push(`ip:${clientIp(req)}`);
  }
  ...
  } as unknown as Parameters<typeof col.updateOne>;

  await col.updateOne(
    { _id: subject as never },
    update,
    { upsert: true },
  );

  const doc = await col.findOne<{ hits?: number[] }>({ _id: subject as never });
  ...
  res.setHeader(
    ...
    String(Math.max(0, opts.limit - recent.length)),
  );
...
export const RATE_LIMITS = {
  AUTH: { key: "auth", limit: 5, windowMs: 60_000, scope: "ip" as const },
  EXAM: { key: "exam", limit: 30, windowMs: 60_000, scope: "ip+user" as const },
  CONTACT: { key: "contact", limit: 3, windowMs: 60 * 60_000, scope: "ip" as const },
  STATS: { key: "stats", limit: 60, windowMs: 60_000, scope: "user" as const },
};
