...
/**
 * Atomically allocate the next `SUP-NNNNNN` number. Uses the `counters`
 * collection with `$inc` upsert to avoid races.
 */
export async function nextTicketNumber(db: Db): Promise<string> {
  // Untyped collection avoids the strict _id ObjectId requirement; we use
  // a stable string key for the counter document.
