...
import { Difficulty, StudyActivity, StudyDiagnosticResults, StudyLesson, StudyLevel, StudyModule, StudyPlan, StudyPlanAdaptation, StudyPlanParameters, StudyPlanTemplate, StudySession, SUBJECT_AREAS, SubjectArea } from "@upcat/shared";
...
export function normalizeStudyPlanParameters(input: Partial<StudyPlanParameters>): StudyPlanParameters {
  ...
  return {
    ...
    breakFrequency: input.includeBreakDays && input.breakFrequency ? Math.max(2, Math.floor(input.breakFrequency)) : null,
  };
}
...
export function calculateStudyDays(parameters: StudyPlanParameters): Date[] {
    ...
    while (cursor.getTime() <= endDate.getTime()) {
        ...
        if (isStudyDay) {
            learnedDays += 1;
            const shouldBreak = parameters.includeBreakDays &&
...
export function assessSubjectNeeds(
    ...
    const baseNeeds = SUBJECT_AREAS.filter((subject) => !excluded.has(subject)).map((subject) => {
        ...
        const inferredLevel: StudyLevel = score >= 71 ? "advanced" : score >= 41 ? "intermediate" : "beginner";
...
export function buildModuleSequence(
    ...
    for (const phase of template.structure.phases) {
        for (const mod of phase.modules) {
            ...
            const subjectNeedsForPhase = needsBySubject.get(mod.subjectArea);
            if (!subjectNeedsForPhase) continue;
            const subjectNeedsForPhaseOrder = subjectNeedsForPhase.filter((n) => n.priority >= order);
            if (subjectNeedsForPhaseOrder.length === 0) continue;
            const subjectNeedsForPhaseOrderSorted = subjectNeedsForPhaseOrder.sort((a, b) => a.priority - b.priority);
            const subjectNeedsForPhaseOrderSortedWeighted = subjectNeedsForPhaseOrderSorted.map((n) => ({
                subjectArea: n.subjectArea,
                level: n.level,
                timeAllocation: n.timeAllocation,
                priority: n.priority,
                weakSubtopics: n.weakSubtopics,
                strongSubtopics: n.strongSubtopics,
            }));

            const subjectNeedsForPhaseOrderSortedWeightedTotalWeight = subjectNeedsForPhaseOrderSortedWeighted.reduce(
                (acc, cur) => acc + cur.timeAllocation, 0);
            const subjectNeedsForPhaseOrderSortedWeightedTimeAllocation = subjectNeedsForPhaseOrderSortedWeighted.map(
                (n) => ({
                    subjectArea: n.subjectArea,
                    level: n.level,
                    timeAllocation: n.timeAllocation,
            ...
            modules.push({
                ...
                level: mod.level,
                timeAllocation: subjectNeedsForPhaseOrderSortedWeightedTimeAllocation,
                priority: subjectNeedsForPhaseOrderSortedWeightedTotalWeight,
                weakSubtopics: subjectNeedsForPhaseOrderSortedWeighted.map((n) => n.weakSubtopics),
                strongSubtopics: subjectNeedsForPhaseOrderSortedWeighted.map((n) => n.strongSubtopics),
            });
            order += 1;
        }
    }
...
export function allocateModuleDays(
    ...
    const weighted = modules.map((module) => {
        ...
        const difficultyWeight = need?.difficulty === "hard" ? 1.3 : module.difficulty === "medium" ? 1.1 : 1;
    ...
    return modules.map((module) => ({
        ...module,
        allocatedDays: allocatedCounts.filter((_, idx) => idx === modules.indexOf(module)).map((_, idx) => studyDays[idx]),
    }));
}
...
export async function assignLessonsToActivities(
    ...
    return modules.map((module) => {
        ...
        const dayCount = Math.max(...module.allocatedDays.length);
        const sessions = module.allocatedDays.map((date, idx) => buildSession(module, lesson?._id?.toString() ?? null, date, idx + 1, dayCount));
...
function buildSession(
    ...
    if (sessionIndex === 1) {
        ...
        activities.push(activityReview(baseId, order++, module, 3));
...
function activityFlashcards(baseId: string, order: number, module: StudyModule): StudyActivity {
    return {
        ...
        content: {
            ...
            reviewTopics: null,
            flashcardCount: 15,
            flashcardsSubtopics: [module.subtopic],
        },
        result: null,
    };
}
...
): { score: number; level: StudyLevel; weakSubtopics: string[]; strongSubtopics: string[] } {
...
export function recalculatePlanProgress(plan: StudyPlan): StudyPlan["progress"] {
    ...
    const totalTimeSpent = modules.flatMap((m) => m.sessions).flatMap((s) => s.activities).reduce((acc, activity) => acc + Number(activity.result?.timeSpent ?? 0), 0);
...
function normalizeStudyDays(input: number[] | undefined): number[] {
    ...
    const dedupe = [...new Set(source.map((d) => Math.max(1, Math.min(5, Math.floor(d)))))];
