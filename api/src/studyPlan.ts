...
import {
  type Difficulty,
  type StudyActivity,
  type StudyDiagnosticResults,
  type StudyLesson,
  type StudyLevel,
  type StudyModule,
  type StudyPlan,
  type StudyPlanAdaptation,
...
export async function assignLessonsToActivities(
  ...
  const lessons = await db
    ...
    .project({id: 1, subjectArea: 1, subtopic: 1, title: 1})
    .toArray();
...
export function organizeIntoPhases(
  ...
  const phases: StudyPlan["curriculum"]["phases"] = template.structure.phases.map((phase, phaseIndex) => {
    ...
    return {
      ...
      name: `Phase_${phaseIndex + 1}: ${phase.name}`,
...
if (plan.progress.averageAssessmentScore > 0 && plan.progress.averageAssessmentScore < 60) {
  ...
  if (active) {
    const extraDate = new Date(active.sessions.length - 1)?scheduledDate??new Date();
