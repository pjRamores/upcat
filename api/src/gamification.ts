 ...
 *
 * Single source of truth for XP awarding, level recomputation, daily-streak maintenance,
 * and weekly-challenge progress tracking. Every mutation of a user's gamification block
 * flows through helpers in this module so the User record, the xp_transactions collection,
 * and the achievements pipeline stay consistent.
 *
 * Achievement evaluation lives in achievements.ts to keep this file focused on the
 * numeric/state mechanics.
 */
    ...
    const block: UserGamificationBlock = {
      ...
      achievements: {
        ...
        progress:
          ...
          typeof existing.achievements.progress === "object"
            ? existing.achievements.progress
            : defaults.achievements.progress,
        pendingNotification: Array.isArray(
 ...
 *
 * Returns a structured result describing the award.
 */
export async function awardXp(
  ...
  if (!user) {
    throw new Error(`awardXp: user ${userId.toHexString()} not found`);
  }
  ...
  const premiumBoost = isPremiumActive(
    normalizeSubscription(user as Record<string, unknown>),
  )
    ? premiumMultiplier
    : 1;
  ...
  const awarded = Math.max(
    0,
    Math.round(input.baseAmount * multiplier * premiumBoost),
  );
...
function defaultDescription(reason: XpReason): string {
  switch (reason) {
    ...
    case "weekly_challenge":
      return "Weekly challenge completed";
    case "admin_grant":
 ...
 * "study_activity" entry point (login, exam submit, practice complete).
 */
export async function updateDailyStreak(
  ...
  if (!user) {
    throw new Error("updateDailyStreak: user not found");
  }
