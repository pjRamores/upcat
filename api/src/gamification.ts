 ...
 * and weekly-challenge progress tracking. Every mutation of a user's gamification block flows through helpers in this module so the User record,
 * the xp_transactions collection, and the achievements pipeline stay consistent.
 *
...
export async function awardXp(
    ...
    return {
        ...
        newXp,
        previousLevel,
        newLevel: lvl.level > previousLevel,
        newTitle: lvl.title,
    };
}
...
export async function updateDailyStreak(
    ...
    return {
        info: {
            ...
            multiplier: mult,
            hoursUntilExpiry: Math.max(0, Math.round((tomorrow - now.getTime()) / 3_600_000)),
        },
        firstOfDay,
    };
}
