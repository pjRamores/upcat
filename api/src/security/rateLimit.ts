 ...
 *
 * Stores one document per `scope`: {identifier}: {endpoint}` in
 * `rate_limit_buckets`. Each `doc` tracks the current + previous fixed
 * window; we estimate the rolling count by linearly interpolating the
 * previous window's overlap into the current — the classic "sliding
 * window counter" algorithm.
 *
 * Accuracy is intentionally approximate (race conditions can over-count
 * by a few hits under heavy parallelism); that is acceptable for abuse
 * mitigation and avoids the cost of per-request locking.
 */
 ...
 * Always increments -- even on a limited request -- so repeat offenders get
 * progressively worse `retryAfter` values until they back off.
 */
