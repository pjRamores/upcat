 ...
 *   - Prototype pollution (_proto_, constructor, prototype keys)
...
const MAX_DEPTH = 6;
const FORBIDDEN_KEYS = new Set(["_proto_", "constructor", "prototype"]);
 ...
 *   - Truncates strings to 10,000 chars (overlong content is suspicious).
 *   - Removes <script>.on= attributes, javascript: URLs from strings.
 */
...
v = v.replace(/\u0000/g, "");
v = v.replace(/<script\b[^>]*>[^<]*<\/script\b[^>]*>/gi, "");
v = v.replace(/<script\b[^>]*>/gi, "");
v = v.replace(/<[^>]+>/gi, "");
v = v.replace(/bon[a-z]+/gi, "");
