 ...
 *
 * Defense-in-depth checks applied to every request body/query before the handler sees it. Catches:
 ...
 *
 * `inspectPayload()` is non-mutating and returns a structured list of threats; `sanitizePayload()` returns a cleaned clone safe for handlers.
 */
export type Threat =
    | "nosql_injection"
    | "proto_pollution"
...
const SCRIPT_TAG = /<s*script\b[^>]*><\/s*\//gi;
const ON_HANDLER = /bon[a-z]+/i;
...
v = v.replace(/\u0000/g, "");
v = v.replace(/<\s*script\b[^>]*[\s\S]*?<\/\s*\b\s*script\b\s*>/gi, "");
v = v.replace(/<\s*script\b[^>]*>/gi, "");
v = v.replace(/\bon[a-z]+\s*=\"[^\"]*"/gi, "");
