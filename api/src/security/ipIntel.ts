 ...
 *
 * Single upserted document per IP in `ip_intelligence`. Every middleware touch is fire-and-forget (`void recordRequest(...)`) . Threat-score adjustments are applied via `adjustThreatScore()` and clamped to [THREAT_SCORE_MIN, THREAT_SCORE_MAX]. Reputation classification is recomputed on every score change.
 */
...
/**
 * Atomically nudges the threat score by `delta` (positive = worse) and reclassifies reputation. Returns the post-adjustment values for tests.
 */
...
export async function recordFailedLogin(
  ...
  try {
    ...
    await db.collection("ip_intelligence").updateOne(
      {_id: ip as never},
      {$inc: {"activity.failedLoginsTotal":1, "activity.failedLoginsToday":1},
