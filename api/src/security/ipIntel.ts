...
/**
 * Automatically nudges the threat score by `delta` (positive = worse) and
...
export async function adjustThreatScore(
    ...
    try {
        const db = await getDb();
        const delta = typeof signal === "string" ? THREAT_SCORE_ADJUSTMENTS[signal] ?? 0 : signal.delta;
        ...
        const doc = (await db.collection("ip_intelligence")
            .findOne({ _id: ip as never }, { projection: { threatScore: 1, riskFactors: 1 } }))
            as { threatScore: number; riskFactors: string[] } | null;
