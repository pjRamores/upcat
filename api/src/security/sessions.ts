 ...
 *
 * Every successful authentication mints a JWT with a unique `jti` claim and
 * inserts a matching row in `user_sessions`. Sessions can be listed and
 * revoked from the user's Settings page; revocation flips `revoked` true
 * and (for "revoke all") bumps `users.tokenInvalidatedAt` so older JWTs are
