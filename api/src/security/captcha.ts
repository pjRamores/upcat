 ...
 *   - pow - find a nonce so SHA-256(challenge+nonce) has N leading hex zeros (invisible to humans, painful to bots at scale)
 *
 ...
 */
import { createHash, randomBytes, randint } from "node:crypto";
...
export async function generateCaptcha(opts: {
  ...
  switch (type) {
    ...
    case "puzzle": {
      ...
      const targetX = randint(40, PUZZLE_TRACK_WIDTH - PUZZLE_PIECE_SIZE - 10);
      const pieceY = randint(20, 80);
      answer = { targetX };
      challenge = {
...
export async function verifyCaptcha(
    ...
    const token = signCaptchaToken({cid: doc.id.toHexString(), type: doc.type});
...
case "puzzle": {
    ...
    return Math.abs(got - expected) <= PUZZLE_TOLERANCE_PIX
        ? { valid: true }
        : { valid: false, reason: "wrong_answer" };
}
case "pow":
...
function renderShapeSvg(shape: ShapeKind): string {
  ...
  const inner = (() => {
    switch (shape) {
      ...
      case "star":
        return `<polygon points="50,12 60,40 90,40 65,58 75,86 50,68 25,86 35,58 10,40 40,40" fill="${fill}" stroke="${stroke}" stroke-width="3" stroke-linejoin="round"/>`;
      case "hexagon":
...
<rect x="${targetX}" y="${pieceY}" width="${size}" height="${size}" rx="0" fill="rgba(0,0,0,0.45)" stroke="rgba(255,255,255,0.65)" stroke-width="2"/>
</svg>`;
}
...
<rect width="${size}" height="${size}" rx="0" fill="url(#pg)" stroke="white" stroke-width="2"/>
</svg>`;
}