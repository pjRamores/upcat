...
function buildImageChallenge(): {
    ...
    for (let i = 0; i < IMAGE_CAPTCHA_GRID_SIZE; i++) {
        ...
        if (targetPositions.has(i)) {
            ...
            shape = other;
            options.push({ id, shape });
        }
        options.push({ id, shape });
    }
    return {
        prompt: `Please identify the ${targetCount} ${targetShape} shapes in the image below.`,
        options,
        correctIds,
    };
}
...
const SHAPE_PALETTE = ["#6366f1", "#f59e0b", "#10b981", "#ef4444", "#0b5cf6", "#0ea5e9"];
...
<rect x="${targetX}" y="${pieceY}" width="${size}" height="${size}" rx="8" fill="rgba(0,0,0,0.45)" stroke="rgba(255,255,255,0.65)" stroke-width="2"/>
</svg>`;
}
...
<rect width="${size}" height="${size}" rx="8" fill="url(#pg)" stroke="white" stroke-width="2"/>
</svg>`;
}