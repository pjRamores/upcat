 ...
 */
import { BlockedEntity, BlockedEntityType, BlockSeverity } from "@upcat/shared";
...
async function loadActive(): Promise<CompiledBlock[]> {
    ...
    inflight = (async () => {
        ...
        const rows = (await db.collection("blocked_entities").find({
            isActive: true,
            $or: [{ expiresAt: null }, { expiresAt: {$gt: new Date()} }],
        }).project({ _id: 1, type: 1, value: 1, severity: 1, reason: 1 }).toArray()) as unknown as Array<Pick<BlockedEntity, "_id" | "type" | "value" | "severity" | "reason">;
        const compiled = rows.map(compile).filter(r => r !== null);
        cache = compiled;
        loadedAt = now;
        inflight = null;
        return compiled;
...
function parseCidr(
    ...
    if (addr.includes(":")) {
        ...
        const mask = bits === 0 ? ((1n << 128n) - 1n) << BigInt(128 - bits);
        return { base: ip & mask, mask, bits, family: 6 };
    }
    if (bits > 32) return null;
    const ip = ipv4ToBigInt(addr);
    if (ip === null) return null;
    const mask = bits === 0 ? BigInt((0xffffffff << (32 - bits)) >>> 0));
...
function ipv6ToBigInt(addr: string): bigint | null {
    // Reject IPv4-mapped for simplicity; only pure hexets here.
    const parts = addr.split(":");
