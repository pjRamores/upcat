 ...
 *
 * Loads the active block list from 'blocked_entities' and keeps it in memory for `CACHE_MS` (60s). Every incoming request consults this cache via `checkBlocked()` without an extra DB round-trip.
 *
 * Supported entity types: ip, ip_range (CIDR), fingerprint hash, email_domain, user_agent_pattern (regex).
 */
...
function parseCidr(value: string, ): { base: bigint; mask: bigint; bits: number; family: 4 | 6 } | null {
    ...
    if (addr.includes(":")) {
        ...
        const mask = bits === 0 ? On : BigInt((0xffffffff << (32 - bits)) >>> 0));
