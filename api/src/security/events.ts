 ...
 *
 * Centralized append-only log for every security-relevant signal. Inserts
 * are intentionally fire-and-forget (we never want logging to break a
 * legitimate request) — callers may `await` the returned promise for tests
 * or audit guarantees, but production handlers usually don't.
 */
