 ...
 *
 * Pure utility helpers that handlers (or `withSecurity` extensions) can call to score bot risk. Each helper returns a structured result that the caller can act on — these helpers don't write to the database or issue responses themselves.
 *
 * Includes:
 * - analyzeHeaders() --- header presence + UA consistency
 * - issueTimingToken() / verifyTimingToken() -- HMAC-signed form load timestamps used to detect "submitted in <2s after load" bots
 * - checkHoneypot() --- true if the hidden field was filled
 * - requireCaptcha() --- gates a handler on a valid X-Captcha-Token, auto-generating a fresh challenge URL when missing
 */
...
/**
 * Pure inspection. We never block on UA alone, but missing or impossible combinations push the score upward.
 */
...
/**
 * Returns an opaque token encoding (issuedAt, purpose). Embed in form markup or HTML response; client echoes it on submit so we can verify minimum-time-since-render.
 *
...
export function verifyTimingToken(
    ...
    opts: { minMs: number; purpose: string; maxAgeMs?: number },
): TimingVerification {
...
/**
 * Returns true when any of the honeypot field names is non-empty in the body. Common field names: "website", "phone2", "company_url".
 */
...
/**
 * Synchronous gate. Returns true and writes a 428 response when the required CAPTCHA token is missing or invalid. Handler must `return` when this returns true.
 */
