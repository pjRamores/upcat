...
export function applySecurityHeaders(res: VercelResponse, cfg: SecurityConfig): void {
    const h = cfg.headers;
    if (h.hsts.enabled) {
...
export function buildCsp(csp: SecurityConfig["headers"]["csp"]): string {
    ...
    const add = (name: string, vals: readonly string[]) => {
...
export function applyCors(
    ...
    const allowed = opts.allowedOrigins.includes(origin);
    if (!allowed) {
        if (req.method === "OPTIONS") {
            res.status(403).end();
            return "blocked";
    ...
    res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Request-Id, X-Device-Fingerprint, X-Captcha-Token");
