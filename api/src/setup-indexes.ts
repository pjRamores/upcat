...
await tickets.createIndex({"verification.status": 1}, {name: "verification.status"});
...
await db
  ...
  .createIndex({emailHash: 1}, {name: "email_hash_lookup"});
await db
  .collection("deletion_log")
  .createIndex({executedAt: -1}, {name: "executed_recent"});

// — Phase 11: counters (ticket numbering, etc.) —
...
setupIndexes().catch((err) => {
  console.error("X Failed to create indexes:", err);
  process.exit(1);
});