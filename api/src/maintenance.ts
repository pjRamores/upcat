...
export function defaultMaintenanceConfig(): MaintenanceWindowDoc["config"] {
  return {
    type: "full",
    affectedServices: ["all"],
    advanceNoticeMinutes: Number(
      process.env.MAINTENANCE_ADVANCE_NOTICE_DEFAULT || "60"
    ),
    showCountdown: true,
    sessionHandling: {
      ...
      extensionMinutes: Number(
        process.env.MAINTENANCE_TIMER_EXTENSION_DEFAULT || "15"
      ),
      gracePeriodMinutes: Number(
        process.env.MAINTENANCE_GRACE_PERIOD_DEFAULT || "30"
      ),
...
export async function getActiveMaintenanceWindow(
  db: Db
): Promise<MaintenanceWindowDoc | null> {
  const now = new Date();
  ...
  const state = await db
    .collection<MaintenanceStateDoc>("maintenance_state")
    .findOne({ _id: "global" });
...
export async function activateMaintenanceWindow(
  db: Db,
  id: ObjectId
): Promise<{
  ...
  if (!(["scheduled", "warning"] as MaintenanceStatus[]).includes(win.status)) {
    throw new Error(
      `Maintenance window cannot be activated from status ${win.status}`
    );
  }
...
export async function buildMaintenanceStatus(db: Db): Promise<any> {
  ...
  const showBanner = Boolean(
    upcoming &&
      upcoming.scheduledStart.getTime() - now <=
        upcoming.config.advanceNoticeMinutes * 60_000
  );
  ...
  return {
    ...
    showBanner,
    bannerMessage:
      active?.config.messaging.bannerMessage ||
      upcoming?.config.messaging.bannerMessage ||
      null,
