 ...
 *
 * Storage strategy: We avoid an external object store. The export payload is placed on the `data_requests` document under `export.blob` (base64), and the "download_URL" is an absolute API URL routed back to the download endpoint (auth-gated). For large exports (>15 MB) we fall back to GridFS.
 */
...
if (options.format === "csv") {
    ...
    if (bundle.user) {
        parts.push(`# profile\n`);
        parts.push(toCsv({user as Record<string, unknown>}));
        parts.push("\n\n");
    }
...
export async function readExportPayload(
    ...
    if (exp.gridFsId) {
        ...
        await new Promise<void>((resolve, reject) => {
            bucket
                .openDownloadStream(exp.gridFsId!)
