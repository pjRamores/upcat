 ...
 *
 * Responsibilities:
 *   - runDataExport: packages a user's data into a JSON/CSV blob, stores it on the data_requests document (gridfs-free, fits well under 16 MB for typical accounts), and sets a 24-hour expiry on the inline download URL.
 *   - executeDeletion: full or data-only deletion with audit tombstone.
 *
 * Storage strategy: We avoid an external object store. The export payload is placed on the 'data_requests' document under 'export.blob' (base64), and the "download URL" is an absolute API URL routed back to the download endpoint (auth-gated). For large exports (>15 MB) we fall back to GridFS.
 */
...
function toCsv(rows: Record<string, unknown>[]): string {
    ...
    const escape = (v: unknown): string => {
        ...
        const s = typeof v === "string" ? JSON.stringify(v);
...
const activityLog = options.includeActivityLog ? await db.collection("activity_log").find({actorId: userId}).sort({createdAt: -1}).limit(2000).toArray() : null;
...
await db.collection("data_requests").updateOne({_id: requestId}, {$set: set});
...
const r = await db.collection("exam_sessions").updateMany({userId}, {$set: {userId: null, anonymizedAt: now}});
...
await db.collection("question_flags").updateMany({userId}, {$set: {userId: null, anonymizedAt: now}});
retained.push("question_flags (anonymized)");

await db.collection("activity_log").updateMany({actorId: userId}, {$set: {actorId: null, anonymizedAt: now}});
