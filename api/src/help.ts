...
export const HELP_CATEGORIES: HelpCategoryMeta[] = [
 { category: "getting-started", name: "Getting Started", description: "Start here for account setup, dashboard basics, and your first study session.", icon: "rocket" },
 { category: "practice-test", name: "Practice Tests", description: "Learn how to configure, take, and review flexible practice sessions.", icon: "clipboard-list" },
 { category: "mock-exam", name: "Mock Exams", description: "Understand strict exam simulation, scoring, and readiness signals.", icon: "target" },
 { category: "gamification", name: "Gamification & Progress", description: "XP levels, badges, weekly challenges, and leaderboard behavior.", icon: "trophy" },
 { category: "study-plan", name: "Study Plan", description: "Diagnostic setup, daily sessions, assessments, and adaptive pacing.", icon: "book-open" },
 { category: "account", name: "Account & Settings", description: "Manage security, profile details, linked accounts, and preferences.", icon: "settings" },
 { category: "payment", name: "Premium & Payments", description: "Learn premium benefits, billing flow, and subscription lifecycle.", icon: "credit-card" },
 { category: "troubleshooting", name: "Troubleshooting", description: "Fix common issues quickly and know when to contact support.", icon: "wrench" },
];
...
export function highlightExcerpt(text: string, term: string, radius = 80): string {
 if (!text) return "";
...
export async function resolveOptionalUser(
 ...
 const user = await db.collection("users").findOne({ _id: new ObjectId(payload.userId) }) as (AuthedUser & { _id: ObjectId }) | null;
