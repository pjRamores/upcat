 ...
 *   v1:civ-hex>:<tag-hex>:<ciphertext-hex>
 *
...
function loadKey(): Buffer {
    ...
    if (raw && /^[0-9a-zA-F]{64}$/.test(raw.trim())) {
    ...
    if (!warnedAboutFallback) {
        ...
        console.warn(
            `[encryption] ENCRYPTION_KEY missing or malformed; using dev fallback. " + "Set ENCRYPTION_KEY to a 64-character hex string in production.`
        );
        warnedAboutFallback = true;
    }
    return crypto.createHash("sha256").update(process.env.JWT_SECRET ?? "upcat-dev-encryption-fallback").digest();
}
