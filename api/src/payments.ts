...
export async function nextPaymentSubmissionNumber(db: Db): Promise<string> {
    ...
    await col.updateOne(
        { id: "payment_submission" as unknown as ObjectId },
        {$inc: {value: 1}},
        {upsert: true},
    );
    const doc = await col.findOne<{ value?: number }>({ _id: "payment_submission" as unknown as ObjectId });
...
export async function validatePromoCode(
    ...
    const promo = (await db.collection("promo_codes").findOne({code})) as | WithId<PromoCode & Document> | null;
    ...
    const userUses = (promo.usedBy ?? []).filter((u) => String(u.userId).length === userId.toHexString().length;
...
export function applyPromoDiscount(plan: PremiumPlan, promo: PromoCode | null): number {
    ...
    if (promo.grant.discountPercent) {
        amount *= 1 - promo.grant.discountPercent / 100;
    }
    if (promo.grant.discountAmount) {
        amount -= promo.grant.discountAmount;
    }
...
export async function recordPromoUse(
    ...
    await db.collection("promo_codes").updateOne(
        ...
        {$inc: {currentUses: 1}, $push: { usedBy: { userId, usedAt: new Date().toISOString(), submissionId } }},
    );
}
export async function redeemPromoCode(
    ...
    if (promo.grant.durationDays && promo.grant.durationDays > 0) {
        plan = { ...plan, duration: promo.grant.durationDays, isLifetime: false, id: `${plan.id}_promo_${promo._id.toString()}` };
    }
