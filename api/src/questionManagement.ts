...
const RICH_BLOCK_TYPES = new Set<RichContentBlock["type"]>([
  "paragraph",
  "image",
  "audio",
  "video",
...
export function estimateSimilarity(a: string, b: string): number {
  const tokensA = new Set(
    normalizeText(a).split(/[^a-z0-9]+/).filter(Boolean)
  );
  const tokensB = new Set(
    normalizeText(b).split(/[^a-z0-9]+/).filter(Boolean)
  );
...
export function normalizeRichContentBlocks(input: unknown): RichContentBlock[] {
  ...
  input.forEach((raw, idx) => {
    ...
    if (!RICH_BLOCK_TYPES.has(type as RichContentBlock["type"])) return;
...
export function inferDuplicateTier(input: {
  ...
  if (
    ...
    input.existingFingerprint === input.candidateFingerprint
  ) {
    return "exact";
  }

  const similarity = estimateSimilarity(
    input.existingQuestionText ?? "",
    input.candidateQuestionText
  );
