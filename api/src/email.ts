...
const FROM = "UPCAT Simulator <noreply@upcat-sim.com>";
  ...
  return `
    <div style="font-family:-apple-system,Segoe UI,sans-serif;max-width:560px;margin:0 auto;padding:32px;background:#ffffff">
      ...
      <p style="color:#94a3b8;font-size:12px;margin-top:32px">
        UPCAT Simulator · automated message — please do not reply directly.
      </p>
    </div>
  `;
}
...
export async function sendVerificationEmail(
  ...
  await safeSend({
    to,
    subject: "Verify your email — UPCAT Simulator",
    html: panel(`
...
export async function sendPasswordResetEmail(
  ...
  await safeSend({
    to,
    subject: "Reset your password — UPCAT Simulator",
    html: panel(`
...
export async function sendContactNotification(payload: {
  ...
  await safeSend({
    ...
    html: panel(`
      <h2 style="color:#4f4e65">New contact-form message</h2>
      <table style="font-size:14px;color:#334155">
        ...
        ${
          payload.ip
            ? `<tr><td style="padding:4px 12px 4px 0"><strong>IP</strong></td><td>${escape(payload.ip)}</td></tr>`
            : ""
        }
      </table>
...
/**
 * Notifies a user that their account has been deactivated or reactivated by an admin.
 * Silently no-ops if Resend isn't configured (dev/.CI).
 */
export async function sendAccountStatusEmail(
  ...
  await safeSend({
    to,
    subject: isDeact
      ? "Your UPCAT Simulator account has been deactivated"
      : "Your UPCAT Simulator account has been reactivated",
    html: panel(
      `
        ...
        <p style="color:#334155">
          ${
            isDeact
              ? "Your account has been deactivated by an administrator. You will no longer be able to sign in."
              : `Your account has been reactivated. You can now sign in again at <a href="${APP_URL}/login">${APP_URL}/login</a>.`
          }
        </p>
      `,
      isDeact ? "#dc2626" : "#16a34a",
    ),
  });
}
...
export async function sendDataExportReadyEmail(
  to: string,
  args: { downloadUrl: string; expiresAt: Date; format: string },
): Promise<void> {
  await safeSend({
    to,
    subject: "Your UPCAT data export is ready",
    html: panel(`
      <h2 style="color:#0f172a;margin:0 0 12px">Your data export is ready</h2>
      <p style="color:#334155">Your <strong>${escape(args.format.toUpperCase())}</strong> export is ready to download.</p>
      <p style="text-align:center;margin:24px 0">
        <a href="${args.downloadUrl}" style="background:#4f4e65;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:600">Download my data</a>
      </p>
      <p style="color:#64748b;font-size:13px">This download link expires on ${args.expiresAt.toUTCString()}.</p>
    `),
  });
}
...
export async function sendDeletionRequestedEmail(
  to: string,
  args: { confirmUrl: string; cancelUrl: string; scheduledFor: Date },
): Promise<void> {
  await safeSend({
    to,
    subject: "Confirm your account deletion request",
    html: panel(`
      <h2 style="color:#b91c1c;margin:0 0 12px">Account deletion requested</h2>
      <p style="color:#334155">We received a request to delete your UPCAT Simulator account. You must confirm via the link below within <strong>48 hours</strong>.</p>
      <p style="text-align:center;margin:24px 0">
        <a href="${args.confirmUrl}" style="background:#dc2626;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:600">Confirm deletion</a>
      </p>
      <p style="color:#334155">After confirming, your account will be permanently deleted on <strong>${args.scheduledFor.toUTCString()}</strong>. You can cancel any time before that date by signing in.</p>
      <p style="color:#64748b;font-size:13px">Didn't make this request? <a href="${args.cancelUrl}">Cancel it now</a>.</p>
    `, "#dc2626"),
  });
}

export async function sendDeletionConfirmedEmail(
  to: string,
  scheduledFor: Date,
): Promise<void> {
  await safeSend({
...
export async function sendTicketReceivedEmail(
  ...
  await safeSend({
    ...
    html: panel(`
      ...
      <p style="color:#334155">We've recorded your request - reference: <strong>${escape(args.ticketNumber)}</strong>.</p>
...
export async function sendTicketUpdateEmail(
  ...
  await safeSend({
    ...
    html: panel(`
      <h2 style="color:#0f172a;margin:0 0 12px">${titleMap[args.updateType]}</h2>
      <p style="color:#334155">${escape(args.summary)}</p>
      <p style="text-align:center;margin:24px 0">
...
export async function sendDisputeNotificationEmail(
  ...
  args: {
    ticketNumber: string;
    provider: string;
    ownerOrClaimant: "owner" | "claimant";
  },
): Promise<void> {
  const subject =
    args.ownerOrClaimant === "owner"
      ? `An identity dispute has been opened against your linked ${escape(args.provider)} account`
      : `Your identity dispute has been opened - reference ${escape(args.ticketNumber)}`;
  ...
  await safeSend({
    ...
    html: panel(`
      ...
      <p style="color:#334155">Reference: <strong>${escape(args.ticketNumber)}</strong></p>
      <p style="color:#334155">Provider: <strong>${escape(args.provider)}</strong></p>
      <p style="color:#334155">
        ${
          args.ownerOrClaimant === "owner"
            ? "A dispute was opened involving your linked identity provider account."
            : "Your dispute has been submitted and will be reviewed by an administrator."
        }
      </p>
    `),
  });
...
export async function sendDisputeResolvedEmail(
  ...
  await safeSend({
    ...
    html: panel(`
      ...
      <p style="color:#334155">Reference: <strong>${escape(args.ticketNumber)}</strong></p>
      <p style="color:#334155">${escape(args.outcome)}</p>
    `),
  });
}
...
export async function sendPaymentSubmissionReceivedEmail(
  ...
  await safeSend({
    ...
    html: panel(`
      ...
      <p style="color:#64748b;font-size:13px">You can track your submission status in Settings → My Payments.</p>
    `),
  });
}
...
export async function sendPaymentApprovedEmail(
  ...
  await safeSend({
    ...
    html: panel(`
      ...
      <p style="color:#334155">${
        args.endDate
          ? `Premium active until <strong>${new Date(args.endDate).toUTCString()}</strong>.`
...
export async function sendPaymentRejectedEmail(
  ...
  await safeSend({
    ...
    html: panel(`
      ...
      <p style="color:#64748b;font-size:13px">Please review your submission and try again if needed.</p>
    `, "#dc2626"),
  });
}
