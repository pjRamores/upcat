        ...
        html: `
            <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
                ...
                <p style="text-align:center;margin:32px 0">
                    <a href="${verifyUrl}" 
                        style="background:#4f46e5;color:#fff;padding:12px 32px;border-radius:8px;text-decoration:none;font-weight:600">Verify Email</a>
                </p>
    ...
    await resend.emails.send({
        ...
        html: `
            <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
                ...
                <p style="text-align:center;margin:32px 0">
                    <a href="${resetUrl}" 
                        style="background:#4f46e5;color:#fff;padding:12px 32px;border-radius:8px;text-decoration:none;font-weight:600">Reset Password</a>
                </p>
...
await resend.emails.send({
    ...
    subject: `[UPCAT Contact] ${payload.subject} â€” ${payload.name}`,
    html: `
...
```typescript
/**
 * Notifies a user that their account has been deactivated or reactivated by an admin. Silently no-ops if Resend isn't configured (dev/.CI).
 */
export async function sendAccountStatusEmail(
    ...
    await resend.emails.send({
        ...
        html: `
            <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
                <h2 style="color:${isDeact ? "#dc2626" : "#16a34a"}">Account ${isDeact ? "deactivated" : "reactivated"}</h2>
                <p style="color:#334155">${isDeact
                    ? "Your account has been deactivated by an administrator. You will no longer be able to sign in."
                    : "Your account has been reactivated. You can now sign in again at "
                    + `<a href="${APP_URL}/login">${APP_URL}/login</a>`.
                }
            </p>
        </div>
    });
...
export async function sendDataExportReadyEmail(
    ...
    await safeSend({
        ...
        html: panel(`
            ...
            <p style="color:#334155">Your <strong>${args.format.toUpperCase()}</strong> export is ready to download.</p>
...
export async function sendDisputeResolvedEmail(
    ...
    await safeSend({
        ...
        html: panel(`
            <h2 style="color:#0f172a;margin:0 0 12px">${escape(args.ticketNumber)}</h2>
            <p style="color:#334155">Reference: <strong>${args.ticketNumber}</strong>. Please respond via the support ticket so an admin can investigate.</p>,
        `),
    });
}
