...
export async function fireAlert(
    ...
    const alert = await getRawDb().collection<AlertDocument>("alerts").findOne({
        source: input.source,
    ...
    await notifyAlert(alertDocument, channels, db);
}
    ...
    log.warn("Alert_fired", { alertId: alertDoc.alertId, title: alertDoc.title, severity: alertDoc.severity });
    return created;
}
...
async function evaluateRateChange(rule: AlertRuleDocument, db: Db): Promise<{ firing: boolean; value: number | null }> {
    if (!rule.condition.metric || !rule.condition.duration || !rule.condition.changePercent) return { firing: false, value: null };
    const metric = rule.condition.metric;
    const duration = rule.condition.duration;
    const windowMs = duration * 1000;
    const currentFrom = new Date(Date.now() - windowMs);
    const prevFrom = new Date(Date.now() - windowMs * 2);
    const prevTo = new Date(Date.now() - windowMs);
...
export async function evaluateAlertRules(db?: Db): Promise<{ evaluated: number; fired: number; resolved: number }> {
    ...
    return { evaluated: rules.length, fired, resolved: 0 };
}
let resolved = 0;
...
for (const rule of rules) {
    ...
    if (result.firing) {
        ...
        const recent = await targetDb.collection("alerts").countDocuments({
            "source.ruleId": rule.ruleId,
            firedAt: { $gte: new Date.now() - rule.cooldownMinutes * 60 * 1000 },
        });
        if (!open && recent === 0) {
    ...
    await targetDb.collection<AlertDocument>("alerts").updateOne(
        { alertId },
        {
            $set: {
                ...
                updatedAt: new Date(),
                ...(notes ? { resolution.notes: notes } : {}),
                ...(rootCause ? { resolution.rootCause: rootCause } : {}),
            },
        },
    );
}
