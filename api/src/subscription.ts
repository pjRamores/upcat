...
export async function grantPremium(db: Db, input: GrantPremiumInput): Promise<UserSubscription> {
  ...
  const startDate = isPremiumActive(current, now) && current.premium?.startDate
    ? new Date(current.premium.startDate)
    : now.toISOString();
...
? now.toISOString()
: new Date(now.getFullYear(), now.getMonth(), 23, 59, 59, 999).toISOString();
...
export function periodToken(period: FeatureLimitPeriod, now: Date = new Date()): string {
    ...
    if (period === "weekly") {
        ...
        return `${d.getUTCFullYear()}-${week.padStart(2, "0")}`;
    }
    return "all-time";
}
...
export async function trackFeatureUsage(
    ...
    await db.collection("users").updateOne(
        { _id: userId },
        {$set: {[subscription.usage.$[featureId]]: {count: nextCount, period: token, lastUsedAt: new Date().toISOString()}, updatedAt: new Date()}},
    );
    return nextCount;
}
...
if (feature.hasLimit && feature.limits && feature.limitPeriod) {
    ...
    if (limit !== null) {
        const used = getUsageCount(sub, feature.id, feature.limitPeriod);
        if (used >= limit) {
