import { type SessionScore, SUBJECT.Areas, type SubjectArea } from "@upcat/shared";
...
export function scoreSessionEntries(
    ...
    const bySubject: Record<SubjectArea, { correct: number; total: number; percentage: number }> = {
    ...
    for (const subject of SUBJECT.Areas) {
    ...
    const percentage = maxPossibleScore === 0 ? 0 : Math.round((rawScore / maxPossibleScore) * 100);
