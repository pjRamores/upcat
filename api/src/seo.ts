/**
 * Server-side SEO helpers: sitemap XML generation, robots.txt, ads.txt rendering,
 * and accessors for the seo_overrides + url_redirects collections.
 */
...
export function renderSitemapXml(entries: SitemapEntry[]): string {
  ...
  return `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="[http://www.sitemaps.org/schemas/sitemap/0.9](http://www.sitemaps.org/schemas/sitemap/0.9)">${body}</urlset>`;
}
...
export function buildStaticSitemapEntries(
  ...
  const overrideByPath = new Map<SeoOverride>(overrides.map((o) => [o.path, o]));
...
export function renderAdsTxt(publisherId: string | null | undefined): string {
  if (!publisherId) {
    return (
      "# ads.txt - no AdSense publisher ID configured yet.\n" +
