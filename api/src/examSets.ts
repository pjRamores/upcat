...
async function normalizeUserSetAssignments(db: Db, userId: ObjectId): Promise<UserSetAssignmentDoc[]> {
    const assignments = await db.collection<UserSetAssignmentDoc>("exam_set_assignments")
        .find({ userId })
        .toArray();
    ...
    for (const [normalizedSetId, group] of grouped) {
        ...
        const mergedAssignedCount = group.reduce((total, assignment) => total + Math.max(0, Number(assignment.assignedCount ?? 0)), 0);
        const mostRecentDoc = group.reduce((latest, assignment) => {
            ...
            return latestTime > toComparableTime(assignment.lastAssignedAt) ? assignment : latest;
        });
        ...
        if (currentTime > toComparableTime(mostRecentDoc.lastAssignedAt)) {
            bulkOps.push({
                updateOne: {
                    filter: { _id: mostRecentDoc._id },
                    update: { $set: { lastAssignedAt: now } },
                },
            });
        }
        normalizedAssignments.push(canonical);
    }

    if (bulkOps.length > 0) {
        await db.collection<UserSetAssignmentDoc>("exam_set_assignments").bulkWrite(bulkOps);
    }
...
const mergedCreatedAt = group.reduce((earliest, assignment) => {
  ...
  for (const duplicate of group) {
    if (!duplicate.id || duplicate._id.equals(canonical._id)) continue;
    bulkOps.push({
      deleteOne: {
...
export async function ensureQuestionSetConfigs(db: Db): Promise<QuestionSetConfigDoc[]> {
    ...
    const now = new Date();
    const existingSetIds = new Set(allExisting.map((doc) => setDocIdValue(doc as QuestionSetConfigDoc & { _id?: unknown })));
...
export async function pickQuestionSetForUser(
    ...
    for (const assignment of userAssignments) {
        ...
        assignmentCountBySet.set(
            normalizedSetId,
            (assignmentCountBySet.get(normalizedSetId) ?? 0) + Math.max(0, Number(assignment.assignedCount ?? 0)),
        );
    }
    ...
    if (mostRecentSetId && candidates.length > 1) {
        const mostNotRecent = candidates.filter((s) => normalizeSetId(s.setId) !== mostRecentSetId);
