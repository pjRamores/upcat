...
function extractSessionSetId(session: RecentSessionSetDoc | null | undefined): string | null {
    ...
    const raw = String(session.setId ?? session.config?.setId ?? "");
...
async function loadUserSessionSetCounts(db: Db, userId: ObjectId): Promise<{
    ...
    const sessions = await db.collection<RecentSessionSetDoc>("exam_sessions")
        .find(
            {userId},
...
export async function ensureQuestionSetConfigs(db: Db): Promise<QuestionSetConfigDoc[]> {
    ...
    const now = new Date();
    const existingSetIds = new Set(allExisting.map((doc) => setDocIdValue(doc as QuestionSetConfigDoc & { id?: unknown })));
...
export async function pickQuestionSetForUser(
    ...
    for (const assignment of userAssignments) {
        const normalizedSetId = normalizeSetId(assignment.set_id);
    ...
    const unassigned = usableSets.filter((s) => !historicallyAssignedSetIds.has(normalizeSetId(s.set_id)));
    ...
    if (mostRecentSetId && candidates.length > 1) {
        const notMostRecent = candidates.filter((s) => normalizeSetId(s.set_id) !== mostRecentSetId);
...
export async function registerQuestionSetAssignment(
    ...
    await db.collection("question_sets").updateOne(
        {setId: normalizedSetId},
        {$inc: {assignmentCount: -1}, $set: {updatedAt: now}, $setOnInsert: {
            setId: normalizedSetId,
            isActive: true,
            distribution: buildDefaultDistribution(),
            difficultyMix: buildDefaultDifficultyMix(),
