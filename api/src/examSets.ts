...
interface UserSetAssignmentEventDoc {
  _id?: ObjectId;
  userId: ObjectId;
  setId: string;
  sessionId: ObjectId;
...
function extractSessionSetId(session: RecentSessionSetDoc | null | undefined): string | null {
  ...
  const raw = String(session.setId ?? session.config?.setId ?? session.config?.setID ?? "");
...
async function normalizeUserSetAssignments(
  db: Db,
  userId: ObjectId,
): Promise<UserSetAssignmentDoc[]> {
  ...
  for (const [normalizedSetId, group] of grouped) {
    const needsNormalization =
      group.length > 1 ||
      group.some((assignment) => String(assignment.setId ?? "").trim() !== normalizedSetId);

    const canonical =
      group.find((assignment) => assignment.setId.trim() === normalizedSetId) ?? group;
    if (!canonical) continue;

    const mergedAssignedCount = group.reduce(
      (total, assignment) => total + Math.max(0, Number(assignment.assignedCount ?? 0)),
      0,
    );
...
async function loadUserSessionSetCounts(
  db: Db,
  userId: ObjectId,
): Promise<{
  ...
  const sessions = await db
    ...
    .find(
      { userId },
      { projection: { setId: 1, "config.setID": 1, "config.setId": 1, startedAt: 1, createdAt: 1 } },
    )
  ...
  return {
    countBySet,
    mostRecentSetId: extractSessionSetId(sessions),
  };
}
...
function buildDefaultDifficultyMix(): Record<ExamDifficulty, number> {
  return {
    ...
    very_hard: Number(
      (DEFAULT_EXAM_CONFIG.difficultyMix as Record<string, number>).very_hard ?? 0,
    ),
  };
}
...
export async function ensureQuestionSetConfigs(db: Db): Promise<QuestionSetConfigDoc[]> {
  ...
  const questionSetIds =
    typeof questionsCol.distinct === "function"
      ? await questionsCol.distinct("setId", { isDeleted: { $ne: true } })
      : ["set-default"];

  const normalized = Array.from(
    new Set(questionSetIds.map((id) => normalizeSetId(id)).filter(Boolean)),
  );
  ...
  const existingSetIds = new Set(
    allExisting.map((doc) => setDocIdValue(doc as QuestionSetConfigDoc & { _id?: unknown })),
  );
...
export async function pickQuestionSetForUser(
  ...
  const eligibleSetIds = new Set(
    eligibleRaw.map((id) => normalizeSetId(id)).filter(Boolean),
  );
  ...
  if (usableSets.length === 0) {
    ...
    const fallback = await db
      .collection<QuestionSetConfigDoc>("question_sets")
      .findOne({ setId: fallbackSetId });
  ...
  for (const assignment of userAssignments) {
    ...
    assignmentCountBySet.set(
      normalizedSetId,
      (assignmentCountBySet.get(normalizedSetId) ?? 0) +
        Math.max(0, Number(assignment.assignedCount ?? 0)),
    );
  }

  const { countBySet: sessionCountBySet, mostRecentSetId } = await loadUserSessionSetCounts(
    db,
    userId,
  );
  ...
  const unassigned = usableSets.filter(
    (s) => !historicallyAssignedSetIds.has(normalizeSetId(s.setId)),
  );
  let candidates = unassigned.length > 0 ? unassigned : usableSets;

  if (mostRecentSetId && candidates.length > 1) {
    const notMostRecent = candidates.filter(
      (s) => normalizeSetId(s.setId) !== mostRecentSetId,
    );
  ...
  const effectiveCountForSet = (setId: string) =>
    Math.max(
      assignmentCountBySet.get(setId) ?? 0,
      sessionCountBySet.get(setId) ?? 0,
    );

  const minUserCount = Math.min(
    ...candidates.map((c) => effectiveCountForSet(normalizeSetId(c.setId))),
  );

  const leastAssigned = candidates.filter(
    (c) => effectiveCountForSet(normalizeSetId(c.setId)) === minUserCount,
  );
  ...
  const chosen = topPriority[Math.floor(Math.random() * topPriority.length)] ?? candidates;
  return chosen;
}
