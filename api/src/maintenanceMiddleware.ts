...
export async function maintenanceMiddleware(req: Request, res: Response, next: NextFunction) {
    ...
    if (!status.isActive) {
        if (status.upcoming) {
            res.setHeader("X-Upcoming-Maintenance", JSON.stringify({
    ...
    const alwaysAllowed = path.startsWith("/api/health") || path.startsWith("/api/maintenance/") || path.startsWith("/api/sync/") || isAdmin;
    ...
    if (active.sessionHandling.allowActiveExamsToFinish) {
        const sessionRoute = /\/api\/(exam|practice|study-plan)\/.+\/(answer|submit|complete|rate)/.test(path);
        if (sessionRoute) return next();
    }

    const blockStarts = /\/(exam|practice|study-plan)\/(start|generate)/.test(path);
    if (blockStarts) {
        const retryAfter = Math.max(30, Math.ceil((new Date(active.scheduledEnd).getTime() - Date.now()) / 1000));
    ...
    if (active.type === "read_only" && req.method === "GET") return next();
