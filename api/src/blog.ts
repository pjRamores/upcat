/**
 * Server-side blog accessors. All collection plumbing for 'blog_posts' lives
 * here so handlers stay thin and so unit tests can stub a single module.
 */
...
function toSummary(doc: BlogPostDoc): BlogPostSummary {
  const { body, status, createdAt, ...rest } = toBlogPost(doc) as BlogPost & {
    body: string;
    status: BlogStatus;
    createdAt: string;
  };
...
export type UpdateBlogPostInput = Partial<BlogPost, "slug" | "title" | "summary" | "body" | "heroImage" | "authorName" | "tags" | "status">;
...
export async function updateBlogPost(
    ...
    if (patch.slug !== undefined) {
        ...
        if (slug !== current.slug) {
            const dup = await coll.findOne({slug, id: {Sne: current.id}});
