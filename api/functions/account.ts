 ...
 * (Social-only users rely on a freshly-issued JWT, which already proves a recent successful provider sign-in within the JWT lifetime.)
 *
 * Cleanup performed:
 * - All user_identities deleted.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    // Hard guard: never allow the seeded admin to nuke themselves through this endpoint -- admin accounts must be removed via the admin user-management UI.
    ...
    const userAuth = (user as { passwordHash?: string | null; hasPassword?: boolean }) as { auth };
