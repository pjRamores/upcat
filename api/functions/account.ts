 ...
 * 3. (Social-only users rely on a freshly-issued JWT, which already proves
 * 4. a recent successful provider sign-in within the JWT lifetime.)
 *
 * Cleanup performed:
 * 1. All user identities deleted.
 * 2. Exam sessions anonymized (userId set to null) so aggregate stats survive.
 * 3. Contact messages authored by the user are deleted.
 * 4. Tombstone written to deletion_log (sha-256 of email).
 * 5. User document deleted; tokens invalidated first.
 */
