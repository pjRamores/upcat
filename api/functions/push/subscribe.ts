 ...
 * The uniqueness key is 'endpoint' -- re-subscribing from the same device
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const body = (req.body ?? {}) as Partial<PushSubscribePayload>;
    if (
        !body.endpoint ||
    ...
    if (
        ...
        return res
            .status(400)
            .json({success: false, error: "Missing subscription keys"});
    }
    const reminderTime = typeof body.reminderTime === "string" && REMINDER_TIME_REGEX.test(body.reminderTime) ? body.reminderTime : "19:00";
    const timezone = typeof body.timezone === "string" && body.timezone.length <= 64 ? body.timezone : null;
    const userAgent = typeof body.userAgent === "string" && body.userAgent.length <= 512 ? body.userAgent : (req.headers["user-agent"] as string | undefined) ?? null;
