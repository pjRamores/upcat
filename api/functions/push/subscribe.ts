...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const reminderTime = typeof body.reminderTime === "string" && REMINDER_TIME_REGEX.test(body.reminderTime)
    ? body.reminderTime
    : "19:00";
  const timezone = typeof body.timezone === "string" && body.timezone.length <= 64
    ? body.timezone
    : null;
  const userAgent = typeof body.userAgent === "string" && body.userAgent.length <= 512
    ? body.userAgent
