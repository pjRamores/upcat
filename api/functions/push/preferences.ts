...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "PATCH") {
    const body = (req.body ?? {}) as Partial<PushPreferencesPayload>;
    ...
    if ((result.matchedCount ?? 0) === 0) {
      return res.status(404).json({ success: false, error: "No subscriptions matched" });
    }
  }
  ...
  const data: PushPreferencesResponse = {
    subscriptions: docs.map((d) => {
      return publicSubscription({
        ...
        lastUsedAt: d.lastUsedAt instanceof Date
