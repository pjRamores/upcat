...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    for (const s of sessions) {
        if (s.completedAt) {
            ...
            days.add(`${d.getUTCFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`);
        }
    }
    ...
    if (sorted.length > 0) {
        ...
        const today = `${todayUTC.getUTCFullYear()}-${String(todayUTC.getMonth() + 1).padStart(2, "0")}-${String(todayUTC.getDate()).padStart(2, "0")}`;
        const yesterdayDate = new Date(Date.now() - 86_400_000);
        const yesterday = `${yesterdayDate.getUTCFullYear()}-${String(yesterdayDate.getMonth() + 1).padStart(2, "0")}-${String(yesterdayDate.getDate()).padStart(2, "0")}`;
