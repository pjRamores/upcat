...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const overallAccuracy = totalQuestionsAnswered === 0 ? 0 : Math.round((totalCorrect / totalQuestionsAnswered) * 100);
    ...
    if (sorted.length > 0) {
        ...
        const today = `${todayUTC.getFullYear()}-${String(todayUTC.getMonth() + 1).padStart(2, "0")}-${String(todayUTC.getDate()).padStart(2, "0")}`;
