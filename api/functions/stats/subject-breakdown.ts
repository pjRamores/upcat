...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const sessionSubject: Map<string, Record<SubjectArea, { correct: number; total: number }>> = new Map();
    ...
    const result = SUBJECT_AREAS.map((subject) => {
        ...
        return {
            ...
            averageTimePerQuestion: s.timedAnswered === 0 ? 0 : Math.round(s.totalTimeSec / s.timedAnswered),
            trend: s.trend,
        };
    }).sort((a, b) => {
