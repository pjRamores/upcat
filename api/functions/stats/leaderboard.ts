...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!data.some((r) => r.isMe)) {
    ...
    if (myRow.length > 0) {
      const myAvg = Math.round((myRow.averageScore as number) ?? 0);
      ...
      const meUser = await db.collection("users").findOne(
        { _id: new ObjectId(meId) },
        { projection: { firstName: 1, lastName: 1 } }
      );

      me = {
        rank: ((higher?.n as number) ?? 0) + 1,
        firstName: meUser?.firstName ?? "You",
        lastInitial:
          typeof meUser?.lastName === "string" && meUser.lastName.length > 0
            ? `${meUser.lastName.toUpperCase()}.`
            : "",
        averageScore: myAvg,
        examsCompleted: myRow.examsCompleted as number,
        isMe: true,
      };
    }
  }
