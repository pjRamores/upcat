...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const sessions = (await loadCompletedSessions(db, new ObjectId(payload.userId))).filter((s) =>
    s.completedAt ? s.completedAt.getTime() >= cutoff : false
  );

  const allIds: ObjectId[] = [];
  for (const s of sessions) {
    for (const q of s.questions) {
      allIds.push(q.questionId);
    }
  }
