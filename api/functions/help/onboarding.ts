...
async function shouldServeFlow(
  ...
  const flow = await db.collection<OnboardingFlowDoc>("onboarding_flows").findOne({ _id: flowId, isActive: true });
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "POST") {
    ...
    await db.collection("users").updateOne(
      ...
      {
        $set: {
          [userHelp.onboardingCompleted.S(flowId)]: completedEntry(completed, stepsCompleted),
        },
      }
    );
  }
...
export async function checkOnboarding(req: VercelRequest, res: VercelResponse) {
    ...
    const db = await getDb();
    const page = normalizeContextualPage(String(req.query.page ?? "/"));
    const userHelp = normalizeUserHelp({ help: unknown }).help;

    if (userHelp.helpPreferences.showOnboarding === false) {
        return res.status(200).json({ success: true, data: { items: [] } });
    }

    const activeFlows = await db.collection<OnboardingFlowDoc>("onboarding_flows").find({ isActive: true }).project({ _id: 1, triggerCondition: 1, canBeReplayed: 1 }).toArray();
