...
function triggerMatchesPage(triggerCondition: string, page: string): boolean {
  if (triggerCondition === "first_login") {
    return page === "/dashboard" || page === "/";
  }
  if (triggerCondition === "first_practice") {
    return page === "/practice-test/configure" || page === "/practice";
  }
  if (triggerCondition === "first_mock") {
    return page.startsWith("/mock-exam") || page === "/dashboard";
  }
  if (triggerCondition === "first_study_plan") {
    return page.startsWith("/study-plan");
  }
  if (triggerCondition === "first_xp_earned") {
    return page === "/profile" || page === "/results";
  }
...
async function shouldServeFlow(
  ...
  if (!flow) return { flow: null, reason: "flow_not_found" as const };
  ...
  if (!manual && userHelp.helpPreferences.showOnboarding === false) {
    return { flow: null, reason: "preferences_disabled" as const };
  }

  if (!manual && completion?.completedAt && !flow.canBeReplayed) {
    return { flow: null, reason: "already_completed" as const };
  }

  if (!manual && completion?.skippedAt && !flow.canBeReplayed) {
    return { flow: null, reason: "already_skipped" as const };
  }

  if (!manual && !triggerMatchesPage(String(flow.triggerCondition ?? "manual"), currentPage)) {
    return { flow: null, reason: "trigger_not_met" as const };
  }

  return { flow, reason: "eligible" as const };
}
...
export default async function handler(
  ...
  const user = (await requireUser(req, res)) as AuthedUser | null;
  if (!user) return;
  ...
  if (req.method === "GET") {
    ...
    const manual =
      manualParam === "" ? true : !(manualParam === "0" || manualParam === "false");
  ...
  if (req.method === "POST") {
    ...
    const stepsCompleted = Number(
      (req.body as { stepsCompleted?: number } | undefined)?.stepsCompleted ?? 0,
    );
        ...
        await applyRewards(db, user._id, [
          {
            reason: "admin_grant",
            baseAmount: 15,
            description: `Onboarding completed: ${flowId}`,
          },
        ]);
      } catch {
    ...
    return res.status(200).json({
      success: true,
      data: completed ? { recorded: true } : { skipped: true },
    });
  }
