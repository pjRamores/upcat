 ...
 *
 * Returns ranked users by XP (all_time) or by XP earned within the rolling window
 * (weekly/monthly). The window variant aggregates xp_transactions.
 * Always returns up to LEADERBOARD_PAGE_SIZE entries plus the current user's row
 * (rank may be outside the top page).
 *
...
function displayName(first: string, last: string, showFullName: boolean): string {
  const safeFirst = first.trim();
  const safeLast = last.trim();

  if (showFullName) {
    return `${safeFirst} ${safeLast}`.trim() || "Anonymous";
  }

  return `${safeFirst}${safeLast ? ` ${safeLast.charAt(0).toUpperCase()}.` : ""}`.trim() || "Anonymous";
}

function userIdVariants(userId: ObjectId): Array<string | ObjectId> {
  return [userId, userId.toString(), userId.toHexString()];
}
    ...
    if (!entries.some((e) => e.isCurrentUser)) {
      const myAgg = await db
        ...
        .aggregate([
          {
            $match: {
              userId: { $in: userIdVariants(user._id) },
