 ...
 * 
 * Returns ranked users by XP (all_time) or by XP earned within the rolling window (weekly/monthly) -- the window variant aggregates xp_transactions.
 * Always returns up to LEADERBOARD_PAGE_SIZE entries plus the current user's row (rank may be outside the top page).
 * 
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (scope === "all_time") {
        ...
        entries = rows.map((u, i) => ({
            ...
            title: u.gamification?.title ?? titleForLevel(),
        ...
        if (!entries.some(e => e.isCurrentUser)) {
            ...
            if (myXp > 0) {
                ...
                currentUserEntry = {
                    ...
                    title: user.gamification?.title ?? titleForLevel(),
                    xp: user.gamification?.xp ?? 0,
                    streak: user.gamification?.streak?.current ?? 0,
                    achievements: user.gamification?.achievements?.unlocked?.length ?? 0,
                    isCurrentUser: true,
                };
                entries = entries.filter(e => e.isCurrentUser);
            }
        }
    }
    res.status(200).json({ success: true, data: { entries, currentUserEntry } });
}
    .xp: myXP,
    .streak: user.gamification?.streak?.current ?? 0,
    .achievements: user.gamification?.achievements?.unlocked?.length ?? 0,
