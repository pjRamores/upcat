...
if (req.method === "POST") {
    if (!existing.completed) {
        return res.status(400).json({ success: false, error: "Challenge not yet completed." });
    }
    if (existing.rewardClaimed) {
        return res.status(400).json({ success: false, error: "Reward already claimed." });
    }
    const def = (await db.collection("weekly_challenges_catalog").findOne({ id: existing.challengeId })) as unknown as WeeklyChallengeDef | null;
    if (!def) {
...
return res.status(200).json({
    ...
    data: {
        ...
        progressPct: existing.target > 0 ? Math.min(100, Math.round((existing.progress / existing.target) * 100)) : 0,
