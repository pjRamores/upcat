 ...
 * any celebration UI), the active weekly challenge, and the last 10 XP
 * transactions.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const recent = (await db
        ...
        .find({userId: user.id.toHexString()})
        .sort({createdAt: -1})
        .limit(10)
        .toArray()) as unknown as XpTransaction[];
        ...
        if (def) {
            ...
            weeklyChallenge = {
                challenge: {
                    ...
                    createdAt: typeof def.createdAt === "string" ? new Date(def.createdAt).toISOString(),
                    updatedAt: typeof def.updatedAt === "string" ? new Date(def.updatedAt).toISOString(),
                },
                ...
                progressPct: block.weeklyChallenge.target > 0
                    ? Math.min(100, Math.round((block.weeklyChallenge.progress / block.weeklyChallenge.target) * 100))
                    : 0,
