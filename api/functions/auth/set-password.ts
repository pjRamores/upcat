 ...
 * Body: {newPassword: string, confirmNewPassword: string, currentPassword?: string}
 *
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const existingHash =
    userAuth?.passwordHash ?? (
      (user as {passwordHash?: string}).passwordHash ?? null;
    );
  const hasPassword = !!(userAuth?.hasPassword || existingHash);

  if (hasPassword) {
    if (!currentPassword) {
      return res.status(400).json({success: false, error: "Current password is required to change it."});
    }
    if (!existingHash) {
