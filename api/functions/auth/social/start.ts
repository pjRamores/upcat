 ...
 *
 * Rewritten by vercel.json from /api/auth/social/:provider/start -> /api/functions/auth/social/start?provider=:provider
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (!cfg.enabled || !cfg.clientId || !cfg.clientSecretEnc || !cfg.redirectUri) {
        return res.status(403).json({success: false, error: `Sign in with ${provider} is not enabled right now.`});
    }
    ...
    const safeRedirectPath = typeof body.redirectPath === "string" && body.redirectPath.startsWith("/")
