 ...
 * verifies the OIDC id_token (where applicable), normalizes the profile,
 * and either:
 * - logs the user in (purpose=login), creating an account if needed;
 * or
...
if (linkedExisting) {
    ...
    if (!user) {
        ...
        await db.collection("user_identities").deleteOne({_id: linkedExisting._id});
    }
}
