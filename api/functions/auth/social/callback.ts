 ...
 * Validates the state, exchanges the authorization code for tokens, verifies the OIDC id_token (where applicable), normalizes the profile, and either: logs the user in (purpose=login), creating an account if needed; or links the identity to the currently-authenticated user (purpose=link).
 * Rewritten by vercel.json from /api/auth/social/:provider/callback -> /api/functions/auth/social/callback?provider=:provider
 */
...
try {
    ...
    return res
        .status(400)
        .json({success: false, error: (err as Error).message || "Token exchange failed."});
}
...
try {
    ...
    return res
        .status(400)
        .json({success: false, error: (err as Error).message || "Could not load profile."});
}
...
if (user && profile.email) {
    ...
    if (candidate) {
        ...
        if (!candidate.isVerified || !profile.emailVerified) {
            return res.status(409).json({
                success: false,
                error: "An account with this email already exists but is unverified. " +
...
if (user) {
    const email = profile.email?.toLowerCase() ?? `social-${provider}-${profile.providerUserId}@no-email.local`;
