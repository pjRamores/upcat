 ...
 *
 * POST /api/auth/security-questions/set -- auth_required
...
async function set(req: VercelRequest, res: VercelResponse) {
  ...
  for (const q of questions) {
    if (!q || typeof q.question !== "string" || typeof q.answer !== "string") {
      return res.status(400).json({ success: false, error: "Each entry must include question and answer." });
    }
  ...
  if (seenQs.size !== questions.length) {
    return res.status(400).json({ success: false, error: "All three questions must be different." });
  }
...
async function lookup(req: VercelRequest, res: VercelResponse) {
  ...
  const user = await requireUser(req, res);
  if (!user) return;
  const storedQs = await db.collection("users").findOne({ _id: user._id });
  if (!storedQs) return res.status(404).json({ success: false, error: "User not found." });
  const { securityQuestions } = storedQs;
  const questions = securityQuestions.map((hashed) => {
    const question = SECURITY_QUESTION_BANK[SECURITY_QUESTION_BANK.indexOf(hashed)];
    return { question, answer: signRecoveryToken(question, hashed) };
  });
