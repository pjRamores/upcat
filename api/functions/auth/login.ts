 ...
 * the first time a user logs in on any given UTC day. Subsequent logins within the same day are no-ops.
 * Returns the gamification payload for the login response (or null if nothing happened).
 */
async function buildLoginGamification(db: Db, userId: ObjectId): Promise<{ xp: [], achievements: [], streakUpdated: boolean }> {
