...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (existingLock && new Date(existingLock).getTime() > Date.now()) {
    const isHard = new Date(existingLock).getTime() >= new Date(LOGIN_LOCKOUT.hardUntil).getTime() - 1000;
    return res.status(423).json({
      success: false,
      error: isHard ? "Account locked. Use account recovery to regain access." : "Too many failed attempts. Try again after ${new Date(existingLock).toUTCString()}.",
    });
  }
  ...
  if (!storedHash) {
    return res.status(401).json({ success: false, error: "This account has no password set. Sign in with your linked social provider, or use 'Forgot password' to set one." });
  }
  ...
  if (!valid) {
    ...
    if (nextCount >= LOGIN_LOCKOUT.softThreshold) {
      const lock = new Date(Date.now() + LOGIN_LOCKOUT.softDurationMs);
      await db.collection("users").updateOne(
        { _id: user._id },
        { $set: { "security.loginAttempts.lockedUntil": lock } }
      );
    }
    return res.status(401).json({ success: false, error: "Invalid credentials." });
  }

  const token = signTokenWithSession(user);
  const clientIp = extractClientIp(req);
  const activity = logActivity(user, "login", { clientIp });
  await db.collection("users").updateOne(
    { _id: user._id },
    { $set: { "security.loginAttempts.count": nextCount } }
  );
  res.status(200).json({ success: true, token, activity });
}
...
if (prevCount < LOGIN_LOCKOUT.mediumThreshold && nextCount >= LOGIN_LOCKOUT.mediumThreshold && newLock) {
...
const [{token}, gamification, onboarding] = await Promise.all([
    ...
    buildLoginGamification(db, user.id).catch((err) => {
        // eslint-disable-next-line no-console
        console.error("[auth/login] gamification update failed", err);
        return null;
    }),
    (async () => {
        const items: Array<{ flowId: string; triggerCondition: string; reason: string }> = [];
        try {
