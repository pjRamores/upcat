...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const userAuth = (user as { auth?: { hasPassword?: boolean; passwordHash?: string | null } }).
    .auth;
  const hasPassword = !!(
  ...
  if (!hasPassword) {
    ...
    if (total <= 1) {
      return res.status(400).json({
        success: false,
        error:
          "Set a password first or keep at least one linked provider — otherwise you'll be locked out.",
      });
    }
  }
