 ...
 * Body: { action: "reset_password" | "set_password", newPassword, confirmPassword }
 *
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { action, newPassword, confirmPassword } = (req.body ?? {}) as {
  ...
  if (!newPassword || !confirmNewPassword || newPassword !== confirmPassword) {
    return res
      .status(400)
