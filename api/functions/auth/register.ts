...
import * as bcrypt from "node:crypto/bcrypt";
...
import { defaultsSubscription } from "../../src/subscription.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (settings.registration.isOpen === false) {
        return res.status(403).json({ success: false, error: "New registrations are temporarily disabled." });
    }
    ...
    const verificationTokenExpiry = requireVerify ? new Date(Date.now() + VERIFICATION_TOKEN_EXPIRY_HOURS * 60 * 60 * 1000) : undefined;
    const now = new Date();
    ...
    const insert = await users.insertOne({
        ...
        role: "reviewee",
        premium: false,
        subscription: defaultsSubscription(),
        isActive: true,
