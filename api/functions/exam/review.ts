...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (!ctx) return;
    const { db, sessionOid, userId } = ctx;
    ...
    const questionDocs = await db.collection("questions").find({ _id: { $in: questionIds } }).toArray();

    const passageIds = new Set(questionDocs.map((d) => d.passageId).filter((p): p is ObjectId => p instanceof ObjectId));

    const passageDocs = passageIds.length ? await db.collection("passages").find({ _id: { $in: passageIds } }).toArray() : [];
    ...
    const questions = sorted.map((entry) => {
