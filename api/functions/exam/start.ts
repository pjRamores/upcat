...
function buildOrderedQuestionMeta(allQuestions: QuestionSamplingDoc[]): { _id: ObjectId; subjectArea: SubjectArea }[] {
  ...
  for (const subject of orderedSubjects) {
    ...
    for (const q of subjectQuestions) {
      const passageKey = q.passageId?.toString() : null;
      if (!passageKey) {
...
const randomizedById = new Map<QuestionSamplingDoc, QuestionChoice[] | null>(
    sampledQuestionDocs
        .map((doc) => {
        ...
        .filter((entry): entry is readonly [string, QuestionChoice[] | null] => Boolean(entry)),
);
