...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const session = await db.collection("exam_sessions").findOne({_id: sessionOid, userId: userOid}, {projection: {status: 1, "timerState.pausedAt": 1}});
    if (!session) {
    ...
    const now = new Date();
    const ops = answers.filter((a) => a && ObjectId.isValid(a.questionId))
