...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (session.status !== "in_progress") {
        return res.status(400).json({ success: false, error: "Only in-progress sessions can be paused or resumed" });
    }

    const now = new Date();
    const eventAt = clampEventDate((req.body as Record<string, unknown> | undefined)?.at);
    const currentTotalPausedMs = Math.max(0, Number(session.timerState?.totalPausedMs ?? 0));
        ...
        if (currentPausedAt) {
            return res.status(200).json({
                ...
                data: {
                    ...
                    timerExtensionMs: currentTotalPausedMs + Math.max(0, Date.now() - currentPausedAt.getTime()),
                },
            });
        }
