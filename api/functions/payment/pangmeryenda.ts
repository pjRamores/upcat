...
import { RATE_WINDOWS } from "dupcat/shared";
...
function buildServiceFromConfig(config: Awaited<ReturnType<typeof getPaymentConfig>>): PangMeryendaService {
    ...
    return new PangMeryendaService({
        apiBaseUrl: config.pangmeryenda.apiBaseUrl,
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (req.method === "POST" && req.includes("/webhook") || req.query.action === "webhook") {
...
if (req.method === "POST" && !transactionId) {
    const userLimit = await checkAndIncrement({
        scope: "user",
        identifier: user.id.toHexString(),
...
if (config.activePaymentType !== "pangmeryenda" || !config.pangmeryenda.enabled) {
...
const created = await service.createPayment({
    ...
    metadata: {userId: user.id.toHexString(), planId: plan.id},
    successUrl: `${process.env.APP_URL || "http://localhost:5173"}${process.env.APP_URL || "http://localhost:5173"}/api/payment/pangmeryenda/success`,
    failureUrl: `${process.env.APP_URL || "http://localhost:5173"}${process.env.APP_URL || "http://localhost:5173"}/api/payment/pangmeryenda/failed`,
    cancelUrl: `${process.env.APP_URL || "http://localhost:5173"}${process.env.APP_URL || "http://localhost:5173"}/api/payment/pangmeryenda/cancelled`,
});
