...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    return res.status(200).json({
        ...
        data: {
            ...
            type: promo.type,
            grant: {
                description: promo.type === "discount" ? `Discount code (${promo.grant.discountPercent ?? 0}% off)` : promo.type === "extended_trial" ? `Extended trial (${promo.grant.durationDays ?? 0} days)` : "Premium grant",
