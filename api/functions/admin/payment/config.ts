...
async function confirmAdminPassword(
  ...
  const hash =
    (admin as unknown as {auth?: {passwordHash?: string | null}}).auth?.passwordHash ??(
    (admin as unknown as {passwordHash?: string | null}).passwordHash;
...
const patch = {
  manual: {
    ...
    autoDisableThreshold:
      typeof body.autoDisableThreshold === "number" ? body.autoDisableThreshold : undefined,
...
if (hasPath(url, "/config/pangmeryenda") || body.action === "pangmeryenda") {
  ...
  const next = {
    ...config.pangmeryenda,
    apiBaseUrl:
      typeof body.apiBaseUrl === "string" ? body.apiBaseUrl : config.pangmeryenda.apiBaseUrl,
    apiKey: typeof body.apiKey === "string" ? body.apiKey : config.pangmeryenda.apiKey,
    apiSecretEnc:
      typeof body.apiSecret === "string" && body.apiSecret
        ? encrypt(body.apiSecret)
        : config.pangmeryenda.apiSecretEnc,
    webhookSecret:
      typeof body.webhookSecret === "string"
        ? body.webhookSecret
        : config.pangmeryenda.webhookSecret,
    merchantId:
      typeof body.merchantId === "string" ? body.merchantId : config.pangmeryenda.merchantId,
    planMapping: Array.isArray(body.planMapping)
        ? (body.planMapping as never)
        : config.pangmeryenda.planMapping,
    successRedirectUrl:
      typeof body.successRedirectUrl === "string"
        ? body.successRedirectUrl
        : config.pangmeryenda.successRedirectUrl,
    failureRedirectUrl:
      typeof body.failureRedirectUrl === "string"
        ? body.failureRedirectUrl
        : config.pangmeryenda.failureRedirectUrl,
    cancelRedirectUrl:
      typeof body.cancelRedirectUrl === "string"
        ? body.cancelRedirectUrl
        : config.pangmeryenda.cancelRedirectUrl,
  };
