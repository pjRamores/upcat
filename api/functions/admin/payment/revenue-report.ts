...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    return res.status(200).json({
        ...
        data: {
            revenue: {
                total: revenueTotal,
                byPlan: Array.from(byPlan.entries(), ([planId, total]) => ({planId, total})),
                byChannel: Array.from(byChannel.entries(), ([channelId, total]) => ({channelId, total})),
                bySource: Array.from(bySource.entries(), ([source, total]) => ({source, total}))
            },
            subscribers: {
