...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "POST" && resource === "grant-xp") {
    const body = (req.body || {}) as AdminGrantXpPayload;
    ...
    if (typeof body.reason !== "string") return badRequest(res, "reason is required");
    ...
    await logActivity(db, {
      actorId: admin._id,
      actorRole: "admin",
      action: "admin.gamification.grant_xp",
targetType:"user",
...
if (resource === "achievements") {
  ...
  if (req.method === "DELETE") {
    ...
    await db.collection("achievements_catalog").updateOne({id}, {$set: {isActive: false, updatedAt: new Date}});
    await logActivity(db, {
      actorId: admin._id,
      actorRole: "admin",
