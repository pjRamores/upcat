...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const [
        ...
        questions.countDocuments({ isDeleted: { $ne: true }, createdAt: { $gte: startOfWeek } }),
        sessions.countDocuments({}),
        sessions.countDocuments({ status: "completed", createdAt: { $gte: startOfDay } }),
        sessions.countDocuments({ status: "completed", createdAt: { $gte: startOfWeek } }),
        sessions.aggregate([
            { $match: { status: "completed", "score.percentage": { $type: "number" } } },
            { $group: { _id: null, avg: { $avg: "$score.percentage" } } },
        ]),
        sessions.aggregate([{ $group: { _id: "$status", n: { $sum: 1 } } }])
            .toArray(),
    ...
    const toMap = (arr: { _id: string; n: number }[]) => {
...
return res.status(200).json({
    ...
    data: {
        ...
        questions: {
            total: questionTotal,
            bySubject: toMap<QuestionBySubject>({ _id: string; n: number }[]),
            byDifficulty: toMap<QuestionByDifficulty>({ _id: string; n: number }[]),
