/**
 * GET /api/admin/data-requests - list (filters: type/status/userId)
 * PUT /api/admin/data-requests/:id - admin override (cancel/expedite/note)
 */
import { VercelRequest, VercelResponse } from "@vercel/node";
import { ObjectId } from "mongodb";
import { DataRequestStatus, DataRequestType } from "@upcat/shared";
...
async function list(req: VercelRequest, res: VercelResponse) {
    ...
    const ids = Array.from(
        new Set(items.map((d) => d.userId?.toString()).filter((s): s is string => !!s)),
        (s) => new ObjectId(s);
    );
    const users =
        ids.length === 0
