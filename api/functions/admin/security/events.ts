...
export default withSecurity({ endpoint: "GET /api/admin/security/events" })(async (
  ...
  if (reviewMatch && req.method === "PUT") {
    ...
    if (body.action === "block_user" && ev.source?.userId) {
      await db.collection("users").updateOne({ _id: ev.source.userId }, {$set: {isActive: false}});
    }
  ...
  if (detailMatch && req.method === "GET") {
    ...
    const related = await db.collection("security_events").find({
      _id: {$ne: id},
      "source_ip": ev.source?.ip,
      timestamp: {
        $gte: new Date(ev.timestamp.getTime() - hourMs),
        $lte: new Date(ev.timestamp.getTime() + hourMs),
      },
