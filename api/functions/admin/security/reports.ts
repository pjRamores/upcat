...
export default withSecurity({endpoint: "GET /api/admin/security/reports"})(async (req: VercelRequest, res: VercelResponse) => {
    ...
    const [byType, bySeverity, topCountries, topIps, blocksTotal, eventsTotal] = await Promise.all([
        ...
        db.collection("security_events")
            .aggregate([
                {$match: {timestamp: {$gte: since}, "source.ip": {$ne: null}}},
                {$group: {_id: "$source.ip", count: {$sum: 1}}},
                {$sort: {count: -1}},
                {$limit: 20},
            ])
            .toArray(),
        db.collection("blocked_entities")
