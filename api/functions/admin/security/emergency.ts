 ...
 *
 * When lockdown is on, `withSecurity` rejects every non-admin request with 503. Admins remain able to manage the system.
 */
...
export default withSecurity({endpoint: "ADMIN/api/admin/security/emergency"})(async (req: VercelRequest, res: VercelResponse) => {
    ...
    const fullAdmin = await db.collection("users").findOne({_id: admin._id}, {projection: {passwordHash: 1}});
