...
export default withSecurity({ endpoint: "ADMIN /api/admin/security/blocked" })(async (req: VercelRequest, res: VercelResponse) => {
    ...
    if (delMatch && req.method === "DELETE") {
        ...
        const r = await db.collection("blocked_entities").updateOne({ _id: id }, { $set: { isActive: false } });
        if (!r.matchedCount) {
