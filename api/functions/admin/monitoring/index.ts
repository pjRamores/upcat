...
async function getDashboard(req: VercelRequest, res: VercelResponse) {
  ...
  const range = queryString(req.query.range, "lh") as TimeRangeKey;
  ...
  const levelMap = (logsByLevel as Array<{id: string; count: number}>).reduce<Record<string, number>>((acc, row) => {
...
async function getLogs(req: VercelRequest, res: VercelResponse) {
  ...
  if (search) filter.Sort = [{message: new RegExp(search, "i")}, {error.message: new RegExp(search, "i")}];
...
async function getMetrics(req: VercelRequest, res: VercelResponse) {
  ...
  const docs = await db.collection("metrics").find({
    name,
    timestamp: {$gte: since}
  }).sort({timestamp: 1}).limit(2000).toArray();
...
async function alertRules(req: VercelRequest, res: VercelResponse, adminId: ObjectId) {
  const db = awaitgetDb();
if (req.method === "GET") {
...
async function healthChecks(req: VercelRequest, res: VercelResponse) {
    ...
    if (req.method === "PUT") {
        const body = (req.body ?? {}) as { checkId?: string; config?: Record<string, unknown> };
...
async function configResource(req: VercelRequest, res: VercelResponse, adminId: ObjectId) {
    const db = await getDb();
    if (req.method === "GET") {
        const config = await db.collection<{} & Record<string, unknown>>("monitoring_config").findOne({_id: "global"});
...
if (resource === "alerts") {
    ...
    if (req.method === "PUT") {
        await mutateAlert(req, res, admin.id);
        return;
    }
    if (req.method === "POST") {
        await testAlert(res);
...
if (resource === "alert-rules") {
    await alertRules(req, res, admin.id);
    return;
}
...
if (resource === "config") {
    await configResource(req, res, admin.id);
    return;
}
