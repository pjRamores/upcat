...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const [items, total] = await Promise.all([
        ...
        db.collection("users")
            .aggregate([
                { $match: { _id: { $in: items.map(item => item.userId) } } },
    ...
    .toArray(),
    db.collection("question_flags").countDocuments(filter),
  ]);
}
...
return res.status(200).json({
  ...
  data: {
    items: items.map((f) => ({
      ...
      question: f.question
        ? {
            _id: f.question._id?.toString?().toString(),
      ...
      user: f.user
        ? {
            _id: f.user._id?.toString?().toString(),
