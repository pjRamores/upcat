/**
 * Admin SEO management -- seo_overrides + url_redirects + sitemap_meta.
 *
 * * GET ... /api/admin/seo/overrides ... -> list all overrides
 * * PUT ... /api/admin/seo/overrides ... -> upsert override (body: { path, ... })
...
const saved = await upsertSecOverride(db, {
    path,
...
if (req.method === "DELETE") {
    ...
    const removed = await deleteSecOverride(db, path);
    await logActivity(db, {
        actorId,
        actorRole: "admin",
...
async function handleSitemapStatus(
  ...
  const overrides = await listSecOverrides(db);
