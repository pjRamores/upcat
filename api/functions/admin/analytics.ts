  ...
  $group: {
    ...
    avgPerQuestion: {
      $avg: {
        $cond: [
          {$gt: ["$totalQuestions", 0]},
          {$divide: ["$durationSec", "$totalQuestions"]}, null
        ]
      }
    },
...
return res.status(200).json({
  ...
  data: {
    ...
    userMetrics: {
      ...
      retentionRate:
        totalUsers > 0
          ? Math.round((((repeatedUsers[0] as { total?: number } | undefined)?.total ?? 0) / totalUsers) * 1000) / 10
          : 0,
...
function questionPerformance(db: Awaited<ReturnType<typeof getDb>>, since: Date) {
    const stats = await db.collection("exam_sessions")
        .aggregate([
            {$match: {status: "completed", completedAt: {$gte: since}}},
            {$unwind: "$questions"},
            {
    ...
    const userIds = new Set(filtered.map((s) => s.questionId.toString()));
    const allQs = await db.collection("questions")
        .find({isDeleted: {$ne: true}})
        .project({subjectArea: 1, difficulty: 1, questionText: 1, flagCount: 1})
        .limit(2000)
        .toArray();
    const neverUsed = allQs
        .filter((q) => !userIds.has(q._id.toString()))
        .slice(0, 25)
        .map((q) => ({
            _id: q._id.toString(),
            subjectArea: q.subjectArea,
