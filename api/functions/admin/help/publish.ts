...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        const articles = await db.collection("help_articles").find({status:"published"}).project({
            _id: 0,
            slug: 1,
            title: 1,
            subtitle: 1,
            ...
            helpfulCount: 1,
            notHelpfulCount: 1
        }).sort({category: 1, order: 1, title: 1}).toArray();

        // Fetch all categories
        const categoryDocs = await db.collection("help_categories").find({}).project({_id: 0, category: 1, name: 1, description: 1, icon: 1}).toArray();
        ...
        const enrichedArticles = articles.map((article) => {
            const body = String({content?: {body?: string}}.content?.body ?? "");
