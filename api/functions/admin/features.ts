...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (req.method === "GET") {
        ...
        for (const user of users) {
            const usage = (user.subscription?.usage ?? {}) as Record<string, { count?: number }>;
            for (const [key, value] of Object.entries(usage)) {
                const stat = usageStats.get(key);
                if (!stat) continue;
                const c = Number(value.count || 0);
                if (c > 0) stat.totalUsersHitting += c;
                stat.avgUsage += c;
                stat.count += 1;
            }
        }
    ...
    if (url.includes("/preview") || body.userId || body.tier) {
        if (body.userId) {
            const target = await db.collection("users").findOne({ _id: new (await import("mongodb")).ObjectId(body.userId) });
    ...
    if (url.includes("/bulk") || Array.isArray(body.features)) {
        ...
        for (const update of updates) {
            const feature = config.featureGating.features.find(f => f.id === update.id);
            if (!feature) continue;
            const limit = feature.limits ? feature.limits[tier] : null;
            const accessible = feature.accessLevel === "all" || (feature.accessLevel === "premium" && tier === "premium");
            if (!accessible) continue;
            const remaining = limit - update.used;
            if (remaining < 0) continue;
            feature.limits[tier] = remaining;
            feature.limitPeriod = update.period;
        }
        await savePaymentConfig(db, config);
        return res.status(200).json({ success: true, data: { tier, features } });
    }
}
