 ...
 *
 * The endpoint accepts a JSON body in two forms (we deliberately avoid multipart parsing so this works identically on Vercel and AWS Lambda - the frontend reads the file client-side and sends the parsed text):
 *
...
function normalizeRow(raw: Record<string, unknown>): Record<string, unknown> {
  ...
  const existingChoices = raw["choices"];
  if (Array.isArray(existingChoices)) return raw;

  const choices: { text: string }[] = [];
  for (const k of ["choiceA", "choiceB", "choiceC", "choiceD"] as const) {
    if (raw[k] !== undefined) {
...
function parseCsv(text: string): Record<string, unknown>[] {
  ...
  const headers = parseCsvRow(lines).map((h) => h.trim());
