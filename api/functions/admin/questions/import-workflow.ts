    ...
    const existing = fingerprints.length
        ? await questions
            .find(
                ...
                {_id: 1, dedupFingerprint: 1, questionText: 1}
            )
            .toArray()
        : [];
...
for (const { fingerprint, rowsForFingerprint } of validFingerprintToRow) {
    ...
    for (const row of rowsForFingerprint) {
        ...
        const nearDuplicate = await questions
            .find(
                { subjectArea: row.payload.subjectArea, subtopic: row.payload.subtopic, isDeleted: {$ne: true} },
...
return res.status(200).json({
    success: true,
    data: {
        batchSize: insert.insertedId.toString(),
        totalRows: batchRows.length,
        validRows,
        duplicateRows,
        invalidRows,
        passagesDetected: passageDocs.length,
        rows: batchRows.slice(0, 500).map((row) => serializeBatchRow(row)),
    },
});
...
function normalizeRow(row: Record<string, unknown>): Record<string, unknown> {
    ...
    if (row.choices !== null && typeof row.choices === "object" && !Array.isArray(row.choices)) {
        const c = row.choices as Record<string, unknown>;
        raw = {
            ...raw,
    ...
    if (Array.isArray(row.choices)) return raw;
    const choices: { text: string }[] = [];
    for (const k of ["ChoiceA", "ChoiceB", "ChoiceC", "ChoiceD"]) {
        if (row[k] !== undefined) choices.push({ text: String(row[k] ?? "") });
    }
