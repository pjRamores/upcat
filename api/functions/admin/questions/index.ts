...
async function listQuestions(
  ...
  const [items, total, subjectCounts, difficultyCounts] = await Promise.all([
    ...
    col.countDocuments({subjectArea: {$exists: true}}),
    col.countDocuments({difficulty: {$exists: true}}),
  ]);
...
async function createQuestion(
  ...
  const { questionText, correctAnswer, difficulty, subjectArea, subtopic } = req.body;
  if (!questionText || !correctAnswer || !difficulty || !subjectArea || !subtopic) {
  ...
  const question = {
    ...
    flagCount: 0,
    usageCount: 0,
    publicationStatus: QuestionPublicationStatus.DRAFT,
  };

  const insertedQuestion = await db.collection("questions").insertOne(question);
  if (!insertedQuestion.insertedId) {
    return res.status(500).json({ success: false, error: "Failed to insert question" });
  }
  ...
  res.status(201).json({ success: true, questionId: insertedQuestion.insertedId });
}
contentBlocks: 1,
isDeleted: 1,
createdAt: 1,
...
export function validateQuestionPayload(
    ...
    const labels = ["A", "B", "C", "D"] as ["A", "B", "C", "D"];
    ...
    if (type === "passage_based") {
        if (!body.passageId || !ObjectId.isValid(String(body.passageId))) {
            return { ok: false, error: "passageId is required for passage_based questions"};
        }
