...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (!includeDeleted) filter.isDeleted = {one: true};
    ...
    if (topic) {
        const tags = topic
            ...
            .map(t => t.trim())
            .filter(Boolean);
        if (tags.length > 0) filter.tags = {sin: tags};
    }
...
const passageDocs = uniquePassageIds.length > 0 ? await db.collection("passages").find({
_id: {$in: uniquePassageIds.map((id) => new ObjectId(id))}, isDeleted: {$ne: true},
{projection: {_id: 1, title: 1, subjectArea: 1, source: 1, content: 1}},
).toArray() : [];
