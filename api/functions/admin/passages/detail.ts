...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "GET") {
    ...
    const linked = await db.collection("questions").find({ passageId: oid, isDeleted: { $ne: true } }).project({ subjectArea: 1, subtopic: 1, difficulty: 1, questionText: 1, correctAnswer: 1 }).toArray();
  ...
  if (req.method === "DELETE") {
    const dependents = await db.collection("questions").find({ passageId: oid, isDeleted: { $ne: true } }).project({ _id: 1, subtopic: 1 }).toArray();
    ...
    const result = await col.updateOne({ _id: oid }, { $set: { isDeleted: true } });
    if (result.matchedCount === 0) {
      return res.status(404).json({ success: false, error: "Passage not found" });
    }
    await logActivity(db, {
