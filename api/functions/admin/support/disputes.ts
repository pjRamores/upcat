...
import {
  IdentityDispute,
  IdentityDisputeAction,
  IdentityDisputeStatus,
  SOCIAL_PROVIDERS,
  SocialProvider,
} from "@upcat/shared";
...
async function create(
  ...
  const db = await getDb();
  const ticket = await db.collection("support_tickets").findOne({ _id: new ObjectId(body.supportTicketId) });
  if (!ticket) {
    return res.status(404).json({ success: false, error: "Linked support ticket not found." });
  }
  const owner = await db.collection("users").findOne({ _id: new ObjectId(body.currentOwnerUserId) });
  if (!owner) {
    return res.status(404).json({ success: false, error: "Current owner not found." });
  }
  const now = new Date();
  const doc: Omit<IdentityDisputeDoc, "_id"> = {
    supportTicketId: new ObjectId(body.supportTicketId),
    claimantUserId: body.claimantUserId && ObjectId.isValid(body.claimantUserId) ? new ObjectId(body.claimantUserId) : null,
  ...
  const r = await db.collection<IdentityDisputeDoc>("identity_disputes").insertOne(doc as IdentityDisputeDoc);
  ...
  return res.status(201).json({ success: true, data: { id: r.insertedId.toString() } });
}
...
if (action === "transfer_identity") {
    if (!dispute.claimantUserId) {
        return res
            .status(400)
            .json({success: false, error: "Claimant user id is required to transfer."});
    }
...
const finalStatus: IdentityDisputeStatus = body.status ??
...
const ticket = await db
    ...
    .findOne({id: dispute.supportTicketId});
