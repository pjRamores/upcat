...
return res.status(200).json({
    ...
    data: {
        items: docs.map((d) => toSupportTicketDTO(d, {
            ...
            assignedToName: d.assignedTo?.nameById.get(d.assignedTo.toString()) ?? null,
        })),
        total,
        page,
        limit,
...
const db = await getDb();
const doc = await db.collection<SupportTicketDoc>("support_tickets")
    .findOne({ticketNumber});
if (!doc) return res.status(404).json({success: false, error: "Ticket not found."});
const now = new Date();
const message = {
...
async function updateStatus(
    ...
    if (push.length > 0) {
        update.$push = {messages: push};
    }
    const r = await db.collection<SupportTicketDoc>("support_tickets")
