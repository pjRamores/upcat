...
async function dashboard(_req: VercelRequest, res: VercelResponse) {
  ...
  const [
    ...
    col.find({ status: {$in: ["open", "in_progress", "awaiting_user"]} })
      .sort({ createdAt: 1 })
      .limit(1)
      .toArray(),
    col.find({ "resolution.resolvedAt": {$ne: null} })
    ...
    col.aggregate([
      ...
      },
      {
        $facet: {
          ...
          resolved: [{ $group: { _id: "$resolvedDay", c: {$sum: 1}} }],
        },
      },
    ]),
  ]);
    ...
    resolvedSamples.length === 0
        ...
        : Math.round(
            {resolvedSamples.reduce((sum, doc) => {
                ...
                const resolved = (doc as { resolution: { resolvedAt: Date } }).resolution.resolvedAt.getTime();
                return sum + (resolved - created) / (60 * 60_000);
            }, 0) / (resolvedSamples.length * 10, ) / 10;
        );
...
async function listTickets(req: VercelRequest, res: VercelResponse) {
    ...
    else if (assignedTo && ObjectId.isValid(assignedTo)) filter.assignedTo = new ObjectId(assignedTo);
...
const assigneeIds = Array.from(
    new Set(docs.map((d) => d.assignedTo?.toString()).filter((id): id is string => !!id)),
...
return res.status(200).json({
    success: true,
    data: {
        items: docs.map((d) => toSupportTicketDTO(d, {
...
async function getOne(req: VercelRequest, res: VercelResponse, ticketNumber: string) {
    ...
    if (doc.assignedTo) {
        ...
        if (a) {
            assignedToName = `${(a as { firstName?: string }).firstName ?? ""}${(a as { lastName?: string }).lastName ?? ""}`.trim() ||
            ((a as { email?: string }).email ?? "Admin");
        }
    }
    return res
...
async function updateStatus(
  ...
  if (status) {
    ...
    if (status === "resolved" || status === "rejected") {
      ...
      set["resolution.resolvedBy"] = admin.id;
  ...
  const r = await db
    .collection<SupportTicketDoc>("support_tickets")
    .findOneAndUpdate({ ticketNumber }, update, { returnDocument: "after" });

  // mongodb v6 driver returns the document directly (not wrapped in {value}).
  const doc = (r as unknown as SupportTicketDoc | { value: SupportTicketDoc | null } | null).value
