...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!primaryUserId || !secondaryUserId || primaryUserId === secondaryUserId) {
    return res.status(400).json({ success: false, error: "Two distinct user ids are required." });
  }
  ...
  if (!adminHash || !(await bcrypt.verify(adminPassword, adminHash))) {
    return res.status(401).json({ success: false, error: "Admin password is incorrect." });
  }
  ...
  if (secondary.role === "admin") {
    return res.status(400).json({ success: false, error: "Cannot merge an admin account away." });
  }

  const now = new Date();

  // 1) Identities - transfer; skip duplicates.
  const secondaryIdentities = await db.collection("user_identities").find({ userId: secondaryId }).toArray();
  let movedIdentities = 0;
  for (const ident of secondaryIdentities) {
    const dupe = await db.collection("user_identities").findOne({ userId: primaryId, provider: ident.provider });
    if (dupe) {
  ...
  if (mergeStrategy === "merge_all") {
    const r = await db.collection("exam_sessions").updateMany({ userId: secondaryId }, {$set: { userId: primaryId, mergedAt: now}});
