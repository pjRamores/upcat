 ...
 * Sends an announcement-type push to every subscription whose preferences.announcement is true. Optionally filtered by role.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (body.role === "admin" || body.role === "reviewee") {
        const users = (await db.collection("users").find({ role: body.role, isActive: {$ne: false} }).project({ _id: 1 }).toArray()) as Array<{ _id: ObjectId }>;
        userFilter = { userId: {$in: users.map((u) => u._id)} };
    }

    const subs = (await db.collection("push_subscriptions").find({ ...userFilter, [preferences.$(type)]: true }).project({ endpoint: 1, keys: 1 }).toArray()) as unknown as Array<{
        _id: ObjectId;
        endpoint: string;
