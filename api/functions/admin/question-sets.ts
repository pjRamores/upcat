  ...
  try {
    ...
    const sets = await Promise.all(
      docs.map(async (doc: any) => ({
        ...doc,
        usageCount: await db.collection("exam_sessions").countDocuments({
          "config.setId": doc.setId || doc._id?.toString(),
        }),
      })),
    );
...
async function getDetail(
  ...
  try {
    ...
    const usageCount = await db.collection("exam_sessions").countDocuments({
      "config.setId": doc.setId || id,
    });
  ...
  try {
    ...
    const priority =
      rawPriority === undefined
        ? 1
        : Math.min(100, Math.max(1, Math.floor(Number(rawPriority) || 1)));

    if (
      rawPriority !== undefined &&
      (!Number.isFinite(Number(rawPriority)) || Number(rawPriority) < 1 || Number(rawPriority) > 100)
    ) {
    ...
    if (totalTimeLimit <= 0) {
      return res.status(400).json({
        success: false,
        error: "Total time limit must be greater than 0",
      });
    }
  ...
  try {
    ...
    if (distribution !== undefined) {
      ...
      const mergedDistribution = Object.fromEntries(
        SUBJECT_AREAS.map((subject) => {
          const existingEntry = (
            existing.distribution as Record<string, { questions?: unknown; timeLimit?: unknown }> | undefined
          )?.[subject];
    ...
    if (totalTimeLimit !== undefined) {
      if (typeof totalTimeLimit !== "number" || totalTimeLimit <= 0) {
        return res.status(400).json({
          success: false,
          error: "totalTimeLimit must be a positive number",
        });
      }
    ...
    if (priority !== undefined) {
      const p = Math.floor(Number(priority) || 1);
    ...
    await syncQuestionSetPublishedCounts(db, existing.setId || id);
    const refreshedDoc = await col.findOne({ _id: objectId });

    await logActivity(db, {
      actorId: adminId,
      actorRole: "admin",
      action: "question_set.updated",
      targetType: "question_set",
      targetId: id,
