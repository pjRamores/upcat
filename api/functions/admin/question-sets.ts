...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (req.method === "POST" && !id) return createSet(req, res, db, admin.id);
    if (req.method === "PUT" && id) return updateSet(req, res, db, admin.id, id);
    if (req.method === "DELETE" && id) return deleteSet(req, res, db, admin.id, id);
