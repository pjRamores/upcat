...
export default withSecurity({ endpoint: "ACCOUNT/api/account/security/sessions" })(async (req: VercelRequest, res: VercelResponse, ctx) => {
    ...
    if (revokeMatch && req.method === "POST") {
        const session = await db.collection("user_sessions").findOne({ _id: new ObjectId(revokeMatch[1]!), userId: user._id });
        if (!session) {
    ...
    const sessions = await db.collection("user_sessions").find({ userId: user._id, revoked: false }).sort([{ lastActiveAt: -1 }]).limit(200).toArray();
