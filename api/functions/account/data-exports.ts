 ...
 *
 * POST /api/account/data-export - request a new export
 * GET /api/account/data-export/:requestId - status / download URL
 * GET /api/account/data-export/:requestId/download - stream the bytes
 */
...
function toExportRequestShape(doc: Record<string, unknown>): {
    ...
    export: {
        format: ExportFormat.format === "csv" ? "csv" : "json",
        includeExamHistory: doc.exportBlock.includeExamHistory !== false,
        includeStats: doc.exportBlock.includeStats !== false,
        includePersonalInfo: doc.exportBlock.includePersonalInfo !== false,
        includeActivityLog: !doc.exportBlock.includeActivityLog,
        fileUrl: (doc.exportBlock.fileUrl as string | null) ?? null,
        fileSizeBytes: (doc.exportBlock.fileSizeBytes as number | null) ?? null,
        generatedAt: toIso(doc.exportBlock.generatedAt),
        expiresAt: toIso(doc.exportBlock.expiresAt),
    },
...
async function listRequests(res: VercelResponse, userId: ObjectId): Promise<void> {
    const db = await getDb();
    const docs = await db.collection("data_requests").find({ userId, type: "export" }).sort({ requestedAt: -1 }).limit(20).toArray();
...
runDataExport(db, {
    requestid: insert.insertedId,
    userId,
    options,
    appUrl: APP_URL,
})
.then(async () => {
    const fresh = await db.collection("data_requests").findOne({ _id: insert.insertedId });
...
async function getStatus(
    ...
    const db = await getDb();
    const doc = await db.collection("data_requests").findOne({ _id: new ObjectId(requestId), userId, type: "export" });
