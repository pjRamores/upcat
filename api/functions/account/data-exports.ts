...
async function createRequest(
    ...
    if (recent) {
        return res.status(400).json({ success: false, error: "Rate limit exceeded" });
    }
    const exportId = await db.collection("data_requests").insertOne({
        userId,
        type: "export",
        requestedAt: new Date(),
        expiresAt: new Date(Date.now() + DATA_EXPORT_TTL_HOURS * 60 * 60 * 1000),
        status: "pending",
        export: {
            format,
            includeExamHistory: options.includeExamHistory,
            includeStats: options.includeStats,
            includePersonalInfo: options.includePersonalInfo,
            includeActivityLog: options.includeActivityLog,
        },
    });
    const exportData = await runDataExport(userEmail, options, exportId);
    await db.collection("data_requests").updateOne(
        { _id: exportId },
        { $set: { status: "success", expiresAt: new Date(Date.now() + DATA_EXPORT_TTL_HOURS * 60 * 60 * 1000), export: exportData } },
    );
    await sendDataExportReadyEmail(userEmail, exportData);
    return res.status(200).json({ success: true, data: { exportId } });
}
if (recent) {
