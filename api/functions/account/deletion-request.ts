        ...
        data: {
            request: {
                ...
                deletion: {
                    ...
                    cancelledBy: doc.deletion?.cancelledBy === "admin" || doc.deletion?.cancelledBy === "user"
                        ? null
...
async function create(req: VercelRequest, res: VercelResponse) {
    ...
    const userAuth = (user as { auth?: { passwordHash?: string | null; hasPassword?: boolean; } })
        .auth;
    const passwordHash = userAuth?.passwordHash ?? (user as { passwordHash?: string }).passwordHash ?? null;
    const hasPassword = !!userAuth?.hasPassword || passwordHash;
    if (hasPassword) {
        const { salt, hash } = await bcrypt.hash(password, 10);
        const updatedUser = await userAuth.updateOne({
            $set: {
                passwordHash: hash,
                hasPassword: true,
            },
    ...
    await logActivity(user, "created deletion request", request);
    return res.status(201).json({ success: true, data: { request: request } });
}
if (!password) {
...
await db.collection("users").updateOne(
    {_id: user_id},
    {$set: {"dataRequests.pendingDeletionId": insert.insertedId}},
);

const confirmUrl = `${APP_URL}/account/deletion-confirm?id=${insert.insertedId.toString()}&token=${confirmationToken}`;
...
return res.status(202).json({
    ...
    data: {
        ...
        message: "Check your email to confirm. Deletion will execute 7 days after confirmation.",
    },
});
...
async function cancel(req: VercelRequest, res: VercelResponse, id: string) {
    ...
    if (isAdmin) filter.userId = user.id;
