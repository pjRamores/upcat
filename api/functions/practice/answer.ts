 ...
 * Mutates the session.card's 'userAnswer' / 'isCorrect' / 'timeSpent', but does NOT yet advance SM-2 state (that happens on /rate). Returns the correctAnswer + rationale so the UI can reveal them.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (userAnswer !== null && !["A", "B", "C", "D"].includes(userAnswer)) {
        return res
            .status(400)
            .json({ success: false, error: "userAnswer must be A, B, C, D, or null" });
    }
    const timeSpent = typeof body.timeSpentSeconds === "number" && body.timeSpentSeconds >= 0 ? Math.min(3600, Math.floor(body.timeSpentSeconds)) : null;
    ...
    const idx = sessionCards.findIndex((c) => c.cardId.toString() === cardId.toHexString());
    if (idx === -1) {
