...
const passages = passageIds.length ? await db.collection("passages").find({ _id: { $in: passageIds } }).toArray() : [];
...
const responseCards: PracticeStartResponse["cards"] = sessionCards.map(({ sc, idx }) => {
    const card = allCards[idx];
    const q = byQuestion.get((card.questionId as ObjectId).toString());
    if (!q) return null;
    const passage = q.passageId ? passageById.get((q.passageId as ObjectId).toString()) : null;
    return {
