 ...
 * Creates a new spaced-repetition practice session. Selects due + (optionally) new cards according to the requested mode, persists a `practice_sessions` doc with pre-SM-2 snapshots, and returns a sanitized question payload (no correctAnswer ./ rationale).
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (questionLimitFeature && questionLimitFeature.enabled) {
    const limit = questionLimitFeature.limit;
    if (limit && limit < baseMaxQuestions) {
      return res.status(429).json({
        success: false,
        error: `You've reached the maximum number of questions allowed for this session. Please try again later.`,
        limit: limit,
      });
    }
  }

  const practiceSession = await selectCardsForSession(db, user._id, mode, body.subjectArea, newCardsLimit, baseMaxQuestions);
  const practiceSessionDoc = await db.collection("practice_sessions").insertOne(practiceSession);
  const practiceSessionId = practiceSessionDoc.insertedId.toString();
  const practiceSessionDocWithId = {
    ...practiceSession,
    _id: practiceSessionId,
  };
  const sanitizedQuestionPayload = await defaultPracticeSessionCard(practiceSessionDocWithId, body.subjectArea);
  res.status(200).json({
    success: true,
    practiceSessionId,
    questionPayload: sanitizedQuestionPayload,
  });
}
...
const questionDocs = await db.collection("questions").find({id: {$in: questionIds}}).toArray();
...
passage: passage
    ? {
        id: (passage._id as ObjectId).toString(),
        title: passage.title,
