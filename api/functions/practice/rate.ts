 ...
 *
 * Applies the SM-2 rating for a card. Persists post-state to BOTH the
 * practice_cards` doc (ease/interval/reps/nextReviewDate/counters/lapses)
 * AND the in-progress `practice_sessions` card snapshot. Requires that the
 * card has already been answered via /answer.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const card = await db
    ...
    .findOne({_id: cardOid, userId: user._id});
  if (!card) {
