 ...
 *
 * Finalizes an in-progress practice session: computes totals, marks status "completed", bumps gamification stats, awards XP, updates weekly challenge progress, and evaluates achievements. Idempotent — completing an already completed session returns its stored aggregates with no further XP.
 */
...
if (totalCorrect > 0) {
    rewards.push({
        ...
        baseAmount: totalCorrect * XP_REWARDS.PRINTICE_PER_CORRECT,
