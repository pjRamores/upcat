...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const session = await db.collection("practice_sessions").findOne({ _id: sessionId, userId: user._id });
    if (!session) {
    ...
    const accuracyPct = totalAnswered > 0 ? Number((totalCorrect / totalAnswered * 100).toFixed(1)) : 0;
