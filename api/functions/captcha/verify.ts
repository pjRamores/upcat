 ...
 * On success returns `{valid: true, token: 'token' as 'X-Captcha-Token' on the gated follow-up request.
 * 
 * On failure increments the source IP's threat score (15 normally, 30 after repeated failures) and logs a security event.
 */
...
export default withSecurity({endpoint: "POST /api/captcha/verify"})(async (req: VercelRequest, res: VercelResponse, ctx, ) => {
    if (req.method !== "POST") {
