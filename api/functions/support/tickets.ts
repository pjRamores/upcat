...
async function create(req: VercelRequest, res: VercelResponse) {
  ...
  const finalPriority: SupportTicketPriority = priority && SUPPORT_TICKET_PRIORITIES.includes(priority) ? priority : "medium";
  const db = await getDb();
  ...
  const effectivePriority: SupportTicketPriority = isPremium && prioritySupportFeature?.accessLevel !== "disabled" ? "high" : finalPriority;
