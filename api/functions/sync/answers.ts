...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const body = (req.body ?? {}) as {
        ...
        deviceId?: string;
        syncedAt?: string;
        offlinePeriods?: Array<{ startedAt: string; endedAt: string; durationMs: number }> | null;
    };
    ...
    for (const ans of body.answers) {
        ...
        const existingQuestion = (session.questions || []).find(
            (q: { questionId: ObjectId; answeredAt?: Date | null }) => q.questionId.toString() === questionId.toString(),
        );
