...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const body = (req.body ?? {}) as { sessionId?: string; deviceId?: string; timestamp?: string };
    ...
    if (body.sessionId && ObjectId.isValid(body.sessionId)) {
        await db.collection("exam_sessions").updateOne(
            { _id: new ObjectId(body.sessionId), userId: user._id },
            {
                $set: {
                    lastHeartbeatAt: new Date(body.timestamp || Date.now()),
                    updatedAt: new Date(),
                },
                $addToSet: {
                    devices: {
