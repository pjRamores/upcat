  ...
  try {
    if (req.method === "GET" && action === "templates") {
      const templates = await db
        ...
        .toArray();
      return res.status(200).json({ success: true, data: templates.map(getTemplateSummary) });
    }
    ...
    if (req.method === "POST" && action === "generate") {
      ...
      const active = await db
        ...
        .countDocuments({ userId: user._id, status: { $in: ["active", "paused"] } });
      if (active >= cfg.maxActive) {
        return res.status(409).json({ success: false, error: "You already have an active study plan." });
      }
      ...
      if (!template) {
        return res.status(500).json({ success: false, error: "No active study plan template found" });
      }
      ...
      if (!premium && template.targetDuration > 4) {
        return res.status(403).json({
          success: false,
          error: "Free users can only create study plans up to 4 weeks. Upgrade to Premium for longer plans.",
          upgradeUrl: "/pricing",
        });
      }

      if (!premium && parameters.targetExamDate) {
        const diffDays = Math.ceil((new Date(parameters.targetExamDate).getTime() - new Date(parameters.startDate).getTime()) / 86_400_000);
        if (diffDays > 26) {
          return res.status(403).json({
            success: false,
      ...
      // Further logic for generating the personalized study plan
    }
  } catch (error) {
    console.error(error);
    ...
    if (body.diagnosticId && ObjectId.isValid(body.diagnosticId)) {
        ...
        if (!diagnostic || diagnostic.status !== "completed") {
            return res.status(400).json({success: false, error: "Diagnostic must be completed"});
        }
...
if (req.method === "GET" && action === "calendar") {
    ...
    const days = plan.curriculum.phases.flatMap((phase) => phase.modules.flatMap((module) =>
    ...
    return res.status(200).json({success: true, data: {planId: plan._id, days}});
}
...
if (req.method === "GET" && planId && action === "today") {
    ...
    if (!today.session) {
        return res.status(200).json({success: true, data: {planComplete: true}});
    }
        ...
        .find({
            id: {
                $in: found.session.activities
                    ...
                    .filter((id): id is string => typeof id === "string" && id.length > 0 && ObjectId.isValid(id))
...
if (activity.result) {
    ...
    activity.result = {
        ...
        practiceSessionId: body.result?.practiceSessionId ?? activity.result.practiceSessionId,
        assessmentSessionId: body.result?.assessmentSessionId ?? activity.result.assessmentSessionId,
    };
}

const allDone = found.session.activities.every((a) => a.status === "completed" || a.status === "skipped");
if (allDone) {
...
if (activity.type === "assessment") {
    ...
    if (passed) {
        ...
        found.module.assessment.passedAt = now;
...
if (req.method === "POST" && planId && action === "adapt") {
  ...
  return res.status(200).json({ success: true, data: { adapted: true, changes: adapted.changes } });
}
...
if (req.method === "PUT" && planId && action === "reschedule") {
  ...
  if (body.newTargetDate) plan.parameters.targetExamDate = new Date(body.newTargetDate).toISOString();
  if (typeof body.newHoursPerDay === "number") plan.parameters.availableHoursPerDay = body.newHoursPerDay;
  if (Array.isArray(body.newStudyDays)) plan.parameters.studyDaysPerWeek = body.newStudyDays;

  const now = new Date();
  if (body.pauseUntil) {
    const pauseDate = new Date(body.pauseUntil);
    const shiftDays = Math.max(0, Math.ceil((pauseDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000)));
...
if (req.method === "POST" && planId && action === "pause") {
  ...
  plan.status = "paused";
  await persistPlan(db, planId, plan);
  return res.status(200).json({ success: true, data: { paused: true } });
}
...
if (req.method === "GET" && planId && action === "analytics") {
    ...
    const avgAssessmentScore = performanceScores.length > 0 ? Math.round((performanceScores.reduce((acc, cur) => acc + cur, 0) / performanceScores.length) * 100) / 100 : 0;
    ...
    return res.status(200).json({
        ...
        data: {
            ...
            performance: {
                ...
                assessmentPassRate: plan.progress.completedAssessments > 0 ? Math.round((plan.progress.passedAssessments / plan.progress.completedAssessments) * 100) : 0,
            ...
            readinessEstimate: {
                ...
                recommendation:
                    avgAssessmentScore >= .75 && plan.progress.overallProgress >= 70 ? "Take a mock exam" : "Keep studying",
            },
        },
    });
}
...
res: VercelResponse,
  ...
  const doc = await db.collection<DbStudyPlan>("study_plans").findOne({ _id: new ObjectId(planId), userId });
  if (!doc) {
