...
import { awardDp, updateDailyStreak } from "../../../../src/gamification.js";
...
if (req.method === "GET" && action === "calendar") {
    ...
    const days = plan.curriculum.phases.flatMap((phase) => phase.modules.flatMap((module) => module.sessions.map((session) => ({
...
const activityXp = activity.type === "lesson" ? 15 : activity.type === "practice" ? 20 : activity.type === "assessment" && activity.result?.passed ? activity.result.attempts <= 1 ? 75 : 50 : 5;
await awardXp(db, user._id, {
    reason: "admin_grant",
...
if (req.method === "GET" && planId && action === "analytics") {
    ...
    const assessmentHistory = modules.flatMap((m) => m.assessment.attempts.map((a) => ({
        module: m.name,
