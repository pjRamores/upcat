...
const doc: Omit<DbStudyPlanAssessment, '_id'> = {
    ...
    moduleId: found.module.assessment.id,
...
if (req.method === "GET" && assessmentSessionId && action === "assessment-questions") {
    ...
    const assessment = await db.collection<DbStudyPlanAssessment>("study_plan_assessments").findOne({id: new ObjectId(assessmentSessionId), userId: user._id});
    if (!assessment) {
        return res.status(404).json({success: false, error: "Assessment session not found"});
    }

    const questionDocs = await db.collection("questions").find({_id: {$in: assessment.questions.map((q) => q.questionId)}}).toArray();
...
if (req.method === "POST" && assessmentSessionId && action === "assessment-submit") {
    ...
    const strengths = { ...bySubtopic.entries() }
        .filter(([k, v]) => Math.round((v.correct / Math.max(1, v.total)) * 100) >= 75)
        .map(([k]) => k);
    const weaknesses = { ...bySubtopic.entries() }
        .filter(([k, v]) => Math.round((v.correct / Math.max(1, v.total)) * 100) < 60)
        .map(([k]) => k);
...
.findOne({ id: assessment.studyPlanId, userId: user._id });
if (!planDoc) {
    return res.status(404).json({ success: false, error: "Study plan not found" });
}
const plan = toApiStudyPlan(planDoc);
...
const assessment = await db
    .collection<DbStudyPlanAssessment>("study_plan_assessments")
    .findOne({ _id: new ObjectId(assessmentSessionId), userId: user._id, status: "completed" });
if (!assessment) {
...
const qdocs = await db
    .collection("questions")
    .find({ _id: { $in: assessment.questions.map((q) => q.questionId) } })
    .project({ _id: 1, questionText: 1, choices: 1, correctAnswer: 1, rationale: 1, subtopic: 1 })
    .toArray();
const byId = new Map(qdocs.map((q) => [String(q._id), q]));
