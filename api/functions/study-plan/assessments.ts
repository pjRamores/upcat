...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const assessmentSessionId =
    typeof req.query.assessmentSessionId === "string" ? req.query.assessmentSessionId : null;
  ...
  try {
    if (req.method === "POST" && planId && moduleId && action === "module-assessment-start") {
      ...
      if (prereqIncomplete) {
        return res.status(400).json({
          success: false,
          error: "Complete or skip all sessions first",
        });
      }
      ...
      const usedQuestionIds = found.module.sessions
        ...
        .flatMap((activity) =>
          activity.result?.practiceSessionId ? [activity.result.practiceSessionId] : [],
        );
      ...
      const insert = await db
        ...
        .insertOne(doc as DbStudyPlanAssessment);
    ...
    if (req.method === "GET" && assessmentSessionId && action === "assessment-questions") {
      if (!ObjectId.isValid(assessmentSessionId)) {
        return res.status(400).json({
          success: false,
          error: "Invalid assessment session id",
        });
      }
    ...
    if (req.method === "POST" && assessmentSessionId && action === "assessment-answer") {
      ...
      const questionIndex = assessment.questions.findIndex(
        (q) => q.questionId.toString() === body.questionId,
      );
      if (questionIndex === -1) {
        return res.status(404).json({
          success: false,
    ...
    if (req.method === "POST" && assessmentSessionId && action === "assessment-submit") {
      ...
      assessment.questions = assessment.questions.map((row) => {
        ...
        if (!bySubtopic.has(subtopic)) {
          bySubtopic.set(subtopic, { correct: 0, total: 0 });
        }
      ...
      const planDoc = await db
        ...
        .findOne({ _id: assessment.studyPlanId as never, userId: user._id });
      ...
      found.module.assessment.bestScore = Math.max(
        found.module.assessment.bestScore ?? 0,
        percentage,
      );
      ...
      await db.collection("study_plans").updateOne(
        { _id: assessment.studyPlanId as never },
        {
          $set: {
      ...
      await awardXp(db, user._id, {
        ...
        baseAmount: xpEarned,
        description: passed
          ? "Passed study plan assessment"
          : "Completed study plan assessment",
        metadata: {
      ...
      return res.status(200).json({
        ...
        data: {
          ...
          attemptsRemaining: Math.max(
            0,
            found.module.assessment.maxAttempts - found.module.assessment.attempts.length,
          ),
          moduleUnlocked: passed
            ? plan.curriculum.phases
                .flatMap((phase) => phase.modules)
                .find((m) => m.status === "active")?.id ?? null
            : null,
          xpEarned,
        },
      });
    ...
    if (req.method === "GET" && assessmentSessionId && action === "assessment-review") {
      ...
      const assessment = await db
        .collection<DbStudyPlanAssessment>("study_plan_assessments")
        .findOne({
          _id: new ObjectId(assessmentSessionId),
          userId: user._id,
          status: "completed",
        });
