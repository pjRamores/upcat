...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const channels = config.manual.channels.map((c) => ({
        ...
        autoDisabled: c.autoDisabled && String(c.autoDisabledReason || "").toLowerCase().includes("daily") ? false : c.autoDisabled,
        autoDisabledReason: c.autoDisabled && String(c.autoDisabledReason || "").toLowerCase().includes("daily") ? null : c.autoDisabledReason,
        autoDisabledAt: c.autoDisabled && String(c.autoDisabledReason || "").toLowerCase().includes("daily") ? null : c.autoDisabledAt,
    }));
