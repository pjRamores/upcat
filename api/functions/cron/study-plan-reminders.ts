...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const sent = await Promise.all(
        subscriptions.map(async (sub) => {
            ...
            const allSessions = (plan.curriculum?.phases ?? []).flatMap((phase: any) => (phase.modules ?? []).flatMap((mod: any) => mod.sessions ?? []));
            const next = allSessions.find(
                (session: any) => {
                    (session.status === "available" || session.status === "in_progress") && String(session.scheduledDate ?? "").slice(0, 10) <= today,
                };
