 ...
 *
 * The job is intentionally conservative: it adds `riskFactors` strings and
 * lets the threat-score adjuster handle scoring on next request.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const farmIps = await db
    ...
    .find({associatedUserIds: {$exists: true}})
    .project({_id: 1})
    .toArray();
  let farmFlagged = 0;
