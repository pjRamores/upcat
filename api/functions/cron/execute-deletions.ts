 ...
 *
 * Picks deletion requests that are processing (confirmed) and whose
 * `scheduledFor` has passed, then runs the full deletion executor.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  for (const doc of due) {
    try {
      const owner = await db.collection("users").findOne({id: doc.userId});
