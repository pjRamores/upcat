...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const channels = config.manual.channels.map((c) => ({
        ...
        autoDisabled: c.autoDisabled && String(c.autoDisabledReason || "").toLowerCase().includes("monthly") ? false : c.autoDisabled,
        autoDisabledReason: c.autoDisabled && String(c.autoDisabledReason || "").toLowerCase().includes("monthly") ? null : c.autoDisabledReason,
        autoDisabledAt: c.autoDisabled && String(c.autoDisabledReason || "").toLowerCase().includes("monthly") ? null : c.autoDisabledAt,
    }));
