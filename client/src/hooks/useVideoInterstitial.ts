...
import {
  isOutsideMinInterval,
  readVideoAdState,
  writeVideoAdState,
} from "@/lib/videoAdState";

interface VideoAdDecision {
  /**
   * Returns true if a video interstitial should be shown for the given trigger.
   * Bumps the trigger counter on every call; consumers must call this exactly
   * once per qualifying trigger event.
   */
 ...
 *
 * Combines base ad gating (consent + premium), per-trigger allow-list,
 * frequency cap (1 per N session finishes), and minimum interval (in seconds)
 * between two consecutive interstitials.
 *
 * The hook is pure with respect to its return value — `shouldShow` itself
 * mutates localStorage (incrementing the counter), which is the only way to
 * faithfully implement "every Nth event" without a separate primer call.
 */
...
export function __resetVideoAdState(): void {
...
/**
 * Posts a non-blocking impression event. Failures are swallowed because
 * analytics losses must never disrupt the user-facing flow.
 */
