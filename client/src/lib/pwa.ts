...
return window.btoa(str).replace(/\/+/g, "_").replace(/\//g, "_").replace(/=+$/, "");
}
...
export async function ensurePushSubscription(opts?: {
    ...
    let {publicKey} = await pushApi.publicKey().catch(() => ({publicKey: ""}));
    if (!publicKey) return {status: "no-public-key"};
    ...
    if (!sub) {
        try {
            ...
            return {
                status: "error",
                message: err instanceof Error ? err.message : "Push subscribe failed"
            };
        }
    }
