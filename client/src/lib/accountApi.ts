...
export const dataExportApi = {
  ...
  unwrap<DataRequest>(
    apiClient.post(API_ROUTES.ACOUNT_DATA_EXPORT, options),
  ),
  /** Returns recent export requests for the current user. */
  list: () =>
  unwrap<{ requests: DataRequest[] }>(
    apiClient.get(API_ROUTES.ACOUNT_DATA_EXPORT),
  ),
  get: (id: string) =>
  unwrap<DataRequest>(
    apiClient.get(API_ROUTES.ACOUNT_DATA_EXPORT_ID(id)),
  ),
  /** Streams the export blob; returns a Blob the caller can save. */
  download: async (id: string): Promise<Blob> => {
    const res = await apiClient.get(API_ROUTES.ACOUNT_DATA_EXPORT_DOWNLOAD(id), {
      responseType: "blob",
    });
...
export const deletionApi = {
  ...
  unwrap<DataRequest>(
    apiClient.post(API_ROUTES.ACOUNT_DELETION_REQUEST, body),
  ),
  current: () =>
  unwrap<{ request: DataRequest | null }>(
    apiClient.get(API_ROUTES.ACOUNT_DELETION_REQUEST),
  ),
  /** Public — confirms a deletion via emailed token. */
  confirm: (id: string, token: string) =>
  unwrap<{ confirmed: true }>(
    apiClient.post(API_ROUTES.ACOUNT_DELETION_REQUEST_CONFIRM(id), {token}),
  ),
  cancel: (id: string) =>
  unwrap<{ cancelled: true }>(
    apiClient.post(API_ROUTES.ACOUNT_DELETION_REQUEST_CANCEL(id)),
  );
};
