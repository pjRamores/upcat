...
export const userSecurityApi = {
  listSessions: () =>
    unwrap<{ sessions: UserSessionRow[] }>(
      apiClient.get(API_ROUTES_V15.ACCOUNT.SESSIONS),
    ),

  revokeSession: (id: string) =>
    apiClient.post(API_ROUTES_V15.ACCOUNT.SESSION_REVOKE(id)),

  revokeAll: () =>
    apiClient.post(API_ROUTES_V15.ACCOUNT.SESSIONS_REVOKE_ALL),

  activity: (limit = 20) =>
    unwrap<{ items: UserSecurityEvent[] }>(
...
export const adminSecurityApi = {
  dashboard: () =>
    unwrap<AdminDashboard>(
      apiClient.get(API_ROUTES_V15.ADMIN.DASHBOARD),
    ),
  ...
  blockIp: (
    ip: string,
    body: { severity?: string; reason?: string; duration?: number },
  ) => apiClient.post(API_ROUTES_V15.ADMIN.IP_BLOCK(ip), body),

  unblockIp: (ip: string) =>
    apiClient.post(API_ROUTES_V15.ADMIN.IP_UNBLOCK(ip)),
  ...
  blockRange: (body: {
    ...
    reason?: string;
    duration?: number;
  }) => apiClient.post(API_ROUTES_V15.ADMIN.IP_BLOCK_RANGE, body),
  ...
  listBlocked: (q: Record<string, string | undefined> = {}) => {
    ...
    return unwrap<{
      items: Array<Record<string, unknown>>;
      total: number;
    }>(apiClient.get(`${API_ROUTES_V15.ADMIN.BLOCKED}?${qs}`));
  },
  ...
  removeBlock: (id: string) =>
    apiClient.delete(API_ROUTES_V15.ADMIN.BLOCKED_ITEM(id)),

  getConfig: () =>
    unwrap<Record<string, unknown>>(
      apiClient.get(API_ROUTES_V15.ADMIN.CONFIG),
    ),

  updateConfig: (config: Record<string, unknown>, password: string) =>
    unwrap<Record<string, unknown>>(
      apiClient.put(API_ROUTES_V15.ADMIN.CONFIG, { config, password }),
    ),

  enableLockdown: (password: string) =>
    apiClient.post(API_ROUTES_V15.ADMIN.LOCKDOWN_ENABLE, {
      confirmCode: "LOCKDOWN",
      password,
    }),

  disableLockdown: (password: string) =>
    apiClient.post(API_ROUTES_V15.ADMIN.LOCKDOWN_DISABLE, { password }),

  report: (period: "24h" | "7d" | "30d" = "24h") =>
    unwrap<{
      period: string;
      since: string;
      totalEvents: number;
