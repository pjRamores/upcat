...
export const adminSecurityApi = {
    dashboard: () => 
        unwrap<AdminDashboard>(apiClient.get(API_ROUTES_V15.ADMIN.DASHBOARD)),
    listEvents: (q: Record<string, string | number | undefined> = {}) => {
        const qs = new URLSearchParams();
        for (const [k, v] of Object.entries(q)) if (v !== undefined && v !== "") 
            qs.set(k, String(v));
        return unwrap<{ items: Array<Record<string, unknown>>; total: number; page: number; limit: number; totalPages: number; }>(apiClient.get(`${API_ROUTES_V15.ADMIN.EVENTS}?${qs}`));
    },
    getEvent: (id: string) => 
        unwrap<{
    ...
    listIps: (q: Record<string, string | number | undefined> = {}) => {
        ...
        for (const [k, v] of Object.entries(q)) if (v !== undefined && v !== "") 
            qs.set(k, String(v));
        return unwrap<{ items: Array<Record<string, unknown>>; total: number; page: number; totalPages: number; }>(apiClient.get(`${API_ROUTES_V15.ADMIN.IPS}?${qs}`));
    },
    getIp: (ip: string) => 
        unwrap<{
