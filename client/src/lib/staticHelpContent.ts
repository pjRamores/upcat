  ...
  inFlightLoad = (async () => {
    try {
      ...
      if (!response.ok) {
        ...
        loadError = new Error(
          `Failed to load static content: ${response.status}`
        );
        loadAttempted = true;
        return null;
      }
      ...
      loadAttempted = true;
      console.warn(
        "[HelpContent] Failed to load static content, will use API fallback",
        loadError.message
      );
      return null;
    } finally {
  ...
  for (const article of content.articles) {
    ...
    const body = String(
      (article as Record<string, unknown>).content instanceof Object &&
        ...
        ? ((article as Record<string, unknown>).content as Record<
            string,
            unknown
          >).body
        : ""
...
> {
  const items = content.articles
    ...
    .map((a) => ({
      ...
      category: a.category,
      estimatedReadingMinutes: a.estimatedReadingMinutes || 1,
    }));
