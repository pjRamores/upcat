...
export function getStaticArticlesByCategory(
    ...
    return content.articles.filter((article) => article.category === categoryId).slice(0, limit);
}
): Array<HelpArticle & { "slug": string; "title": string; "subtitle": string; "category": string } & { estimatedReadingMinutes: number; }>;
> {
