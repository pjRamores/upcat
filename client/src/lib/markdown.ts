 ...
 * - Horizontal rule (----)
 *
 * All input is HTML-escaped before tokens are recognised, so a hostile post body can not inject raw HTML or javascript: URLs. The output is trusted only because of this escaping pass.
 */
...
function renderInline(line: string): string {
    ...
    s = s.replace(/\![(!note|warning|info)\]/gi, (_m, kind: string) => `<strong>${kind.toUpperCase()}:</strong>`);
    // Inline code first so its contents aren't re-parsed.
    s = s.replace(/`([^"]+)`/g, (_, m, code: string) => `<code>${code}</code>`);
    // Bold then italic (longest match first).
    s = s.replace(/\*{1}\*{1}(\*+|\*)\*/g, '<strong>$1</strong>');
    s = s.replace(/(^|[\s()_]{1})_(^|_)+(_?=[\s.,,!?]|$)/g, "$1<em>$2</em>");
    s = s.replace(/\*{1}(\*+|\*)\*/g, "<em>$1</em>");
    // Images ! [alt] (src) — http/https and absolute-root relative paths only.
    s = s.replace(/!\[([^\]]+)\]\((?:https?:\/\/|\/)([^)\s]+)\)/g,
        (_, _m, alt: string, src: string) => `<img src="${src}" alt="${alt}" loading="lazy" />`,
    );
    // Links [text] (href) — http/https only.
    s = s.replace(/\[([^\]]+)\]\((?:https?:\/\/|\/)([^)\s]+)\)/g,
        (_, _m, text: string, href: string) => `<a href="${href}" rel="noopener noreferrer">${href.startsWith("http") ? 'target="_blank"' : ""}>${text}</a>`,
    );
    return s;
}
...
export function renderMarkdown(markdown: string): string {
    ...
    function isTableSeparator(line: string): boolean {
        ...
        return cells.every((cell) => /^:?-{3,}:?$/i.test(cell));
    }
