 ...
 *
 * 1. Server-side snapshot** — periodically POSTs the full runtime state
 *      to /api/sync/session-snapshot so a snapshot is always available on the
 *      server even if the client's sessionStorage was wiped (e.g. private
 *      browsing, cross-device continuation).
 *
 * 2. Status check on re-init** — before trusting a cached local runtime,
 *      we check /api/sync/session-status/:id to verify the session is still
 *      active and learn of any server-side answer count or timer adjustments
 *      (e.g. maintenance extension applied while offline).
 */
