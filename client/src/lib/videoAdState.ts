 ...
 *
 * Why client-side state: the interstitial decision happens before any server round-trip and must be instant. Server-side analytics (impressions) are still recorded via '/api/ads/video/impression', but the cap is enforced here to avoid blocking the result UI on a network call.
 */
...
export function readVideoAdState(): VideoAdLocalState {
    ...
    try {
        ...
        return {
            ...
            triggersSinceShown: typeof parsed.triggersSinceShown === "number" && parsed.triggersSinceShown >= 0 ? Math.floor(parsed.triggersSinceShown) : 0,
        };
    } catch {
