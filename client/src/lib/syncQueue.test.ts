...
vi.mocked(apiClient.post).mockResolvedValue({data: {data: {answersAccepted: 1}}});
...
let resolvePost!: (value: {data: {data: {answersAccepted: number}}}} => void;
const inFlight = new Promise<{data: {data: {answersAccepted: number}}}}>(resolve) => {
resolvePost = resolve;
});
vi.mocked(apiClient.post).mockImplementation(
() => inFlight as Promise<{data: {data: {answersAccepted: number}}}}),
);
...
expect(apiClient.post).toHaveBeenCalledTimes(1);
resolvePost({data: {data: {answersAccepted: 1}}});
await Promise.resolve();
});
...
expect(setTimeoutMock).toHaveBeenCalledWith(expect.any(Function), 4_000);
...
vi.mocked(apiClient.post).mockResolvedValue([]);
// Only one pause action should remain (last-write-wins)
return flushSessionActionQueue("s1").then((sent) => {
  expect(sent).toBe(1);
  expect(apiClient.post).toHaveBeenCalledTimes(1);
});
