...
/**
 * Generic list response for admin endpoints that do not yet have shared types.
 */
...
export const adminApi = {
  // ├── Dashboard / Analytics
  dashboardSummary: () =>
    unwrap<AdminDashboardSummary>(apiClient.get(API_ROUTES.ADMIN.DASHBOARD)),

  dashboardActivity: (limit = 100) =>
    unwrap<{ items: ActivityLogEntry[] }>(
      apiClient.get(`/admin/dashboard/activity?limit=\${limit}`),
    ).then((d) => d.items),

  analytics: (period: "week" | "month" | "year" | "all" = "month") =>
    unwrap<Record<string, unknown>>(
      apiClient.get(`\${API_ROUTES.ADMIN.ANALYTICS}?period=\${period}`),
    ),
  ...
  createQuestion: (body: Partial<Question> & { passageId?: string | null }) =>
    unwrap<Question>(apiClient.post(API_ROUTES.ADMIN.QUESTIONS, body)),
  ...
  bulkDeleteQuestions: (ids: string[]) =>
    unwrap<{ deleted: number }>(
      apiClient.post(API_ROUTES.ADMIN.QUESTIONS_BULK_DELETE, { ids }),
    ),

  importQuestions: (format: "json" | "csv", data: string) =>
    unwrap<{
      imported: number;
      skipped: number;
      errors: Array<{ row: number; reason: string }>;
    }>(apiClient.post(API_ROUTES.ADMIN.QUESTIONS_IMPORT, { format, data })),
  ...
  previewQuestionImport: (
    ...
    unwrap<{
      ...
      apiClient.post(API_ROUTES.ADMIN.QUESTIONS_IMPORT_PREVIEW, {
        format,
        data,
        setId,
      }),
  ...
  confirmQuestionImport: (
    ...
    unwrap<{ inserted: number; updated: number; skipped: number; mode: string }>(
      apiClient.post(API_ROUTES.ADMIN.QUESTIONS_IMPORT_CONFIRM, {
        batchId,
        mode,
        setId,
      }),
  ...
  getQuestionImportBatch: (id: string) =>
    unwrap<QuestionImportBatch>(apiClient.get(API_ROUTES.ADMIN.QUESTIONS_IMPORT_BATCH(id))),
  ...
  exportQuestions: async ({ params = {} }: { params?: QuestionExportParams }) => {
  ...
  exportQuestionsUrl: (params: QuestionExportParams) => {
  ...
  deleteQuestionMediaAsset: (id: string) =>
    unwrap<{ deleted: boolean }>(
      apiClient.delete(API_ROUTES.ADMIN.QUESTION_MEDIA_ASSET(id)),
    ),
  ...
  createPassage: (body: Partial<Passage>) =>
    unwrap<Passage>(apiClient.post(API_ROUTES.ADMIN.PASSAGES, body)),

  updatePassage: (id: string, body: Partial<Passage>) =>
  ...
  deactivateUser: (id: string, reason?: string) =>
    unwrap<{ ok: boolean }>(apiClient.post(API_ROUTES.ADMIN.USER_DEACTIVATE(id), { reason })),
  ...
  extendUserPremium: (id: string, body: { days: number; reason?: string }) =>
    unwrap<{ extended: boolean; newEndDate: string | null }>(
      apiClient.post(API_ROUTES.ADMIN.USER_EXTEND(id), body),
    ),

  exportUsersUrl: () =>
    `\${import.meta.env.VITE_API_URL || "/api"}\${API_ROUTES.ADMIN.USERS_EXPORT}`,
  ...
  updateAnnouncement: (id: string, body: Partial<Announcement>) =>
    unwrap<{ updated: boolean }>(
      apiClient.put(API_ROUTES.ADMIN.ANNOUNCEMENT(id), body),
    ),

  deleteAnnouncement: (id: string) =>
    unwrap<{ deleted: boolean }>(
      apiClient.delete(API_ROUTES.ADMIN.ANNOUNCEMENT(id)),
    ),
  ...
  // ─── Settings
  getSettings: () =>
    unwrap<PlatformSettings>(apiClient.get(API_ROUTES.ADMIN.SETTINGS)),
  ...
  listRedirects: () =>
    unwrap<UrlRedirect[]>(apiClient.get("/admin/seo/redirects")),
  ...
  getBlogPost: (id: string) =>
    unwrapRaw<BlogPost>(apiClient.get(`/admin/blog/\${id}`)),
  ...
  listHelpArticles: (params: Record<string, string | number | undefined> = {}) =>
    unwrap<ListResponse<Record<string, unknown>>>(
      apiClient.get(API_ROUTES.ADMIN.HELP_ARTICLES, { params }),
    ),

  createHelpArticle: (body: Record<string, unknown>) =>
    unwrap<{ created: boolean; slug: string }>(
      apiClient.post(API_ROUTES.ADMIN.HELP_ARTICLES, body),
    ),

  updateHelpArticle: (slug: string, body: Record<string, unknown>) =>
    unwrap<{ updated: boolean }>(
      apiClient.put(API_ROUTES.ADMIN.HELP_ARTICLE(slug), body),
    ),
  ...
  publishHelpContent: () =>
    unwrap<{ exported: boolean; contentSize: number; payload: Record<string, unknown> }>(
      apiClient.post("/admin/help/publish"),
    ),

  listContextualHelp: () =>
    unwrap<{ items: Array<Record<string, unknown>> }>(
      apiClient.get(API_ROUTES.ADMIN.HELP_CONTEXTUAL),
    ),

  createContextualHelp: (body: Record<string, unknown>) =>
    unwrap<{ created: boolean; id: string }>(
      apiClient.post(API_ROUTES.ADMIN.HELP_CONTEXTUAL, body),
    ),

  updateContextualHelp: (id: string, body: Record<string, unknown>) =>
    unwrap<{ updated: boolean }>(
      apiClient.put(API_ROUTES.ADMIN.HELP_CONTEXTUAL_ID(id), body),
    ),

  listOnboardingFlows: () =>
    unwrap<{ items: Array<Record<string, unknown>> }>(
      apiClient.get(API_ROUTES.ADMIN.HELP_ONBOARDING),
    ),
  ...
  helpAnalytics: () =>
    unwrap<Record<string, unknown>>(apiClient.get(API_ROUTES.ADMIN.HELP_ANALYTICS)),

  // Monitoring
  monitoringDashboard: (range: "1h" | "6h" | "24h" | "7d" = "1h") =>
    unwrap<Record<string, unknown>>(
      apiClient.get(`/admin/monitoring/dashboard?range=\${range}`),
    ),

  monitoringLogs: (params: Record<string, string | number | undefined> = {}) =>
    unwrap<PagedResponse<Record<string, unknown>>>(
  ...
  monitoringAcknowledgeAlert: (id: string, notes?: string) =>
    unwrap<{ success: boolean }>(
      apiClient.put(`/admin/monitoring/alerts/\${id}/acknowledge`, { notes }),
    ),

  monitoringResolveAlert: (id: string, notes?: string, rootCause?: string) =>
    unwrap<{ success: boolean }>(
      apiClient.put(`/admin/monitoring/alerts/\${id}/resolve`, { notes, rootCause }),
    ),
  ...
  monitoringAlertRules: () =>
    unwrap<Array<Record<string, unknown>>>(
      apiClient.get("/admin/monitoring/alert-rules"),
    ),

  monitoringSeedAlertRules: () =>
    unwrap<{ seeded: number }>(
      apiClient.post("/admin/monitoring/alert-rules/seed-defaults"),
    ),

  monitoringHealthChecks: () =>
    unwrap<Array<Record<string, unknown>>>(
      apiClient.get("/admin/monitoring/health-checks"),
    ),

  monitoringRunAllHealthChecks: () =>
    unwrap<Record<string, unknown>>(
      apiClient.post("/admin/monitoring/health-checks/run-all"),
    ),

  monitoringRunHealthCheck: (checkId: string) =>
    unwrap<Record<string, unknown>>(
      apiClient.post(`/admin/monitoring/health-checks/\${checkId}/run`),
    ),
  ...
  monitoringReport: (range: "1h" | "6h" | "24h" | "7d" = "24h") =>
    unwrap<Record<string, unknown>>(
      apiClient.get(`/admin/monitoring/reports?range=\${range}`),
    ),

  monitoringEvaluateRules: () =>
    unwrap<Record<string, unknown>>(
      apiClient.get("/admin/monitoring/reports/evaluate-rules"),
    ),

  // Question Sets
  getQuestionSets: (params: Record<string, string | number | boolean | undefined> = {}) =>
    unwrap<PaginatedResult<AdminQuestionSet>>(
      apiClient.get("/admin/question-sets", { params }),
    ),
  ...
  deleteQuestionSet: (id: string) =>
    unwrap<{ deleted: boolean }>(apiClient.delete(`/admin/question-sets/\${id}`)),
};
...
export const publicApi = {
  announcements: () =>
    unwrap<Announcement[]>(apiClient.get(API_ROUTES.ANNOUNCEMENTS)),

  status: () =>
    apiClient.get(API_ROUTES.STATUS).then(({ data }) => ({
