 ...
 */
async function unwrap<T>(promise: Promise<{ data: { [key: string]: any; }; }>): Promise<T> {
...
importQuestions: (format: "json" | "csv", data: string) => 
    unwrap<{ imported: number; skipped: number; errors: { row: number; reason: string }[] }>() => (
        apiClient.post(API_ROUTES.ADMIN.QUESTIONS_IMPORT, { format, data }),
    ),
previewQuestionImport: (format: "json" | "csv", data: string | Record<string, unknown>[] | {
...
confirmQuestionImport: (batchId: string, mode: "skip_duplicates" | "insert_all" | "replace_exact", setId: string) => 
...
unwrap<QuestionId; from: QuestionPublicationStatus; to: QuestionPublicationStatus; version: number>()>(
...
unwrap<Passage>(apiClient.post(API_ROUTES.ADMIN.PASSEAGES, body)),
updatePassage: (id: string, body: Partial<Passage>) =>
unwrap<{updated: boolean}>(
apiClient.put(API_ROUTES.ADMIN.PASSAGE(id), body)),
deletePassage: (id: string) =>
unwrap<{deleted: boolean}>(
apiClient.delete(API_ROUTES.ADMIN.PASSAGE(id))),
// ---- Users
...
unwrap<{updated: boolean}>(
apiClient.put(API_ROUTES.ADMIN.USER(id), body)),
...
unwrap<{ok: boolean}>(
apiClient.post(API_ROUTES.ADMIN.USER_REACTIVATE(id)),
resetUserPassword: (id: string) =>
unwrap<{ok: boolean}>(
apiClient.post(API_ROUTES.ADMIN.USER_RESET_PASSWORD(id)),
verifyUserEmail: (id: string) =>
unwrap<{ok: boolean}>(
apiClient.post(API_ROUTES.ADMIN.USER_VERIFY_EMAIL(id)),
setUserPremium: (
id: string,
...
archiveHelpArticle: (slug: string) =>
    unwrap<Archived>({ archived: boolean })(apiClient.delete(API_ROUTES.ADMIN.HELP_ARTICLE(slug))),
publishHelpContent: () =>
    unwrap<{ exported: boolean; contentSize: number; payload: Record<string, unknown> }>(apiClient.post("/admin/help/publish")),

listContextualHelp: () =>
    unwrap<Items>({ items: Array<Record<string, unknown>> })(apiClient.get(API_ROUTES.ADMIN.HELP_CONTEXTUAL)),
...
listOnboardingFlows: () =>
    unwrap<Items>({ items: Array<Record<string, unknown>> })(apiClient.get(API_ROUTES.ADMIN.HELP_ONBOARDING)),
