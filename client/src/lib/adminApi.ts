...
export const adminApi = {
  ...
  dashboardActivity: (limit = 100) =>
    unwrap<ActivityLogEntry[]>(apiClient.get(`/admin/dashboard/activity?limit=${limit}`)).then((d) => d.items),
  analytics: (period: "week" | "month" | "all" = "month") =>
...
listQuestions: (params: Record<string, string | number | boolean | undefined> = {}) => {
    ...
    return unwrap<PaginatedResult<AdminQuestionListEntry>>(apiClient.get(API_ROUTES.ADMIN.QUESTIONS, { params: sanitized }));
},
...
importQuestions: (format: 'json' | 'csv', data: string) => 
    unwrap<imported: number; skipped: number; errors: { row: number; reason: string }[]>(apiClient.post(API_ROUTES.ADMIN.QUESTIONS_IMPORT, { format, data })),
previewQuestionImport: {
    ...
    unwrap<batchId: string; totalRows: number; validRows: number; duplicateRows: number; invalidRows: number; passagesDetected: number; rows: Array<{ rowNumber: number; status: string; error?: string; duplicateQuestionId?: string }>> (apiClient.post(API_ROUTES.ADMIN.QUESTIONS_IMPORT_PREVIEW, { format, data, setId })),
...
listPracticeSessions: (params: Record<string, string | number | undefined> = {}) =>
  unwrap<PaginatedResult<Record<string, unknown>>>(apiClient.get("/admin/practice-sessions", { params })),
...
listFlags: (params: Record<string, string | number | undefined> = {}) =>
  unwrap<PaginatedResult<QuestionFlag & { question: Record<string, unknown>; user: Record<string, unknown> }>>(apiClient.get(API_ROUTES.ADMIN.FLAGS, { params })),
...
flagsSummary: () =>
  unwrap<{ byStatus: Record<string, number>; byReason: Record<string, number> }>(apiClient.get(API_ROUTES.ADMIN.FLAGS_SUMMARY)),
...
deleteSeoOverride: (path: string) =>
  unwrap<{ removed: boolean }>(apiClient.delete("/admin/seo/overrides", { params: { path } })),
...
listHelpArticles: (params: Record<string, string | number | undefined> = {}) =>
  unwrap<{ items: Array<Record<string, unknown>>; total: number; page: number; limit: number }>(apiClient.get(API_ROUTES.ADMIN.HELP_ARTICLES, { params })),
...
publishHelpContent: () =>
    unwrap< { exported: boolean; contentSize: number; payload: Record<string, unknown>; } >(apiClient.post("/admin/help/publish")),
...
updateOnboardingFlow: (flowId: string, body: Record<string, unknown>) =>
    unwrap< { updated: boolean; upserted: boolean } >(apiClient.put(API_ROUTES.ADMIN.HELP_ONBOARDING_FLOW(flowId), body)),

helpAnalytics: () => unwrap<Record<string, unknown>>>(apiClient.get(API_ROUTES.ADMIN.HELP_ANALYTICS)),

// --- Monitoring ---
monitoringDashboard: (range: "lh" | "6h" | "24h" | "7d" = "lh") =>
...
monitoringLogs: (params: Record<string, string | number | undefined> = {}) =>
    unwrap< { items: Array<Record<string, unknown>>; total: number; page: number; limit: number; totalPages: number; } >(apiClient.get("/admin/monitoring/logs", {params})),
monitoringMetrics: (params: Record<string, string | number | undefined> = {}) =>
    unwrap< { name: string; range: string; points: Array<Record<string, unknown>> } >(apiClient.get("/admin/monitoring/metrics", {params})),
monitoringAlerts: (params: Record<string, string | number | undefined> = {}) =>
    unwrap< { items: Array<Record<string, unknown>>; total: number; page: number; limit: number; totalPages: number; } >(apiClient.get("/admin/monitoring/alerts", {params})),
...
monitoringSilenceAlert: (id: string, silenceMinutes = 30, notes?: string) =>
    unwrap< { success: boolean } >(apiClient.put(`/admin/monitoring/alerts/${id}/silence`, {silenceMinutes, notes})),
monitoringTestAlert: () =>
...
monitoringReport: (range: "lh" | "6h" | "24h" | "7d" = "24h") =>
