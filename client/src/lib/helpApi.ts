...
async function unwrap<TP>(promise: Promise<{ data: { data: T } }>): Promise<T> {
...
interface OnboardingCheckCache {
  [userId: string]: {
    [page: string]: {
      at: number;
      data: { items: { flowId: string; triggerCondition: string; reason: string }[] };
    };
  };
}
    ...
    listArticles: async (params: {
        ...
        if (staticContent && hasUsableStaticContent(staticContent)) {
            try {
                ...
                // Silently fail if static content loading fails
            }
        }
    },
};
...
feedback: (slug: string, payload: { helpful: boolean; comment?: string }) => unwrap<recorded: true>(apiClient.post(API_ROUTES.HELP.ARTICLE_FEEDBACK(slug), payload)),
...
contextual: (page: string) => unwrap<items: ContextualHelpPoint[]; isNewUser: boolean>() => apiClient.get(API_ROUTES.HELP.CONTEXTUAL, {params: {page}}),
