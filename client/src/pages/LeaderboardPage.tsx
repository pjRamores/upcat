        ...
        {LEADERBOARD_SCOPES.map((s) => (
          <button
            ...
            className={`rounded-md px-4 py-1.5 text-sm font-medium transition ${
              scope === s ? "bg-indigo-600 text-white" : "text-slate-700 hover:bg-slate-50"
            }`}
          >
            {SCOPE_LABEL[s]}
          </button>
...
function LeaderboardRow({ entry }: { entry: LeaderboardEntry }) {
    ...
    return (
        <tr className={entry.isCurrentUser ? "bg-indigo-50 font-semibold" : "hover:bg-slate-50"}>
            <td className="px-4 py-3 font-mono">{medal ? <span className="text-lg">{medal}</span> : `${entry.rank}`}</td>
            <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-100 text-xs font-bold text-indigo-700">{entry.avatarInitials}</div>
            ...
            <td className="px-4 py-3 text-right font-mono text-indigo-700">{entry.xp.toLocaleString()}</td>
