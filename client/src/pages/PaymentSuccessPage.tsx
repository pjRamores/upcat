...
export default function PaymentSuccessPage() {
  ...
  return (
    <div className="mx-auto max-w-2x1 px-4 py-16 text-center">
      ...
      {summary && (
        <div
          className="mx-auto mt-6 max-w-md rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-left text-sm text-emerald-900">
          <p><strong>Tier:</strong> {summary.tier}</p>
          <p><strong>Valid</strong>
            Until:</strong> {summary.endDate ? new Date(summary.endDate).toLocaleString() : "Lifetime"}</p>
        </div>
      )}
      <div className="mt-8 flex items-center justify-center gap-3">
        <Link to="/dashboard"
          ...
          to Dashboard</Link>
        <Link to="/settings"
          className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Subscription
          Details</Link>
        </div>
      </div>
    );
