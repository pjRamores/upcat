...
import SEOHead from "@/components/Sec";
 ...
 * Single-page scroll with hero / features / how-it-works / subjects / social proof / footer-CTA
...
function StudentIllustration() {
    return (
        <div className="relative mx-auto aspect-square w-full max-w-md">
            <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-primary-500 to-primary-700 shadow-2xl shadow-primary-200">
                ...
                <div className="absolute -right-2 top-1/2 flex items-center gap-2 rounded-full bg-white py-2 pl-2 pr-4 shadow-xl ring-1 ring-black/5">
        ...
        <section id="features" className="scroll-mt-24 bg-white py-20 sm:py-24">
            <div className="mx-auto max-w-6xl px-4">
                ...
                <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    {FEATURES.map((f, i) => (
                        <div key={f.title} data-reveal style={{ transitionDelay: `${i * 60}ms` }} className="group rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-primary-200 hover:shadow-lg">
                            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary-50 text-2xl ring-1 ring-primary-100 transition-colors" onClick={() => console.log(f.icon)}></div>
...
<div className="flex items-center gap-4">
    <span className="flex h-12 w-12 items-center justify-center rounded-full bg-primary-600 text-lg font-bold text-white shadow-lg shadow-primary-200">{step.number}</span>
...
{idx < STEPS.length - 1 && (
    <div aria-hidden className="absolute right-[-22px] top-1/2 hidden -translate-y-1/2 text-primary-300 md:block">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
            <path d="M5 12h14m-6-16 6.6-6.6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
    </div>
)}
</li>
            ...
            <div className="mx-auto max-w-6xl px-4">
                ...
                <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    {SUBJECT_areas.map((subject, i) => {
                        ...
                        return (
                            <div key={subject} data-reveal style={{transitionDelay: `${i * 60}ms`}} className="group flex flex-col rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
...
function SocialProof() {
    return (
        <section className="bg-gradient-to-r.from-primary-600.to-primary-700.py-16">
            <div className="mx-auto.max-w-4xl.px-4.text-center.text-white" data-reveal>
                ...
                <p className="mx-auto.mt-4.max-w-2xl.text-base.text-primary-100">
                    From Aparri to Zamboanga — students across the Philippines are using UPCAT Simulator to sharpen their skills and walk into exam day with confidence.
                </p>
                <div className="text-center">
                    <ProofStat value="10,000+" label="Active learners"/>
                    <ProofStat value="500,004+" label="Questions answered"/>
                    <ProofStat value="4.8+" label="Average rating"/>
                </div>
            </div>
        </section>
    );
        ...
        <section id="faq" className="scroll-mt-24.bg-white.py-20.sm:py-24">
            <div className="mx-auto.max-w-3xl.px-4">
                <div className="text-center" data-reveal>
                    ...
                    <p className="mt-3.text-lg.text-gray-600">
                        Everything you need to know about the UPCAT and how to prepare with UPCAT Simulator.
                    </p>
                </div>
                <dl className="mt-12.divide-y.divide-gray-200.border-t.border-b.border-gray-200">
                    {items.map((it, i) => (
                        <details key={it.question} data-reveal style={{transitionDelay: `${i * 40}ms`}} className="group py-5">
                            <summary className="flex.cursor-pointer.list-none.items-center.justify-between.gap-4.text-base.font-semibold.text-gray-900.outline-none focus-visible:ring-2.focus-visible:ring-primary-500">
                                <dt>{it.question}</dt>
                                <span aria-hidden className="ml-2.inline-flex.h-7.w-7.shrink-0.items-center.justify-center.rounded-full.bg-primary-50.text-primary-700 transition-transform group-open:rotate-45">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
                                        <path d="M12.5v14M5.12h14"/>
                                    </svg>
                                </span>
                            </summary>
                            <dd className="mt-3.pr-9.text-sm.leading-relaxed.text-gray-600">{it.answer}</dd>
                        </details>
                    ))}
                </dl>
            </div>
