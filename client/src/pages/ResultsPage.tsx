...
export default function ResultsPage() {
  ...
  const ringColor =
    tone === "green" ? "#16a34a" : tone === "yellow" ? "#ca8a04" : "#dc2626";
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      <Seo
        title="Exam Results"
        description="View the results of your practice exam."
        noindex
      />
      ...
      {score.rawScore !== undefined && (
        <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-2 text-center text-sm text-slate-700">
            {score.correct} correct + {score.incorrect} incorrect + {score.unanswered} unanswered
            = <span className="font-semibold"> {score.rawScore} points</span>
          </p>
        </div>
      )}
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Stat label="Time Taken" value={formatDuration(totalSecondsTaken)} tone="indigo" />
        <Stat label="Time Allowed" value={formatDuration(totalSecondsAllowed)} tone="gray" />
      </div>
...
function Stat({
  ...
  const styles: Record<"green" | "red" | "gray" | "indigo", string> = {
