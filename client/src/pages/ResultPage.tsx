  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const msg = (e as { response?: { data?: { error?: string } } }).response?.data?.error || "Could not load results";
        setError(msg);
      } finally {
...
const avgPerQuestion =
    score.total > 0 ? Math.round(totalSecondsTaken / score.total) : 0;
...
return (
    <div className="mx-auto max-w-4xl px-4 py-10">
        ...
        <div className="mt-8 flex flex-col items-center">
            ...
            <p className="mt-3 text-lg font-medium text-gray-700">{score.correct} / {score.total} correct</p>
        ...
        {score.rawScore !== undefined && (
            <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-2 text-center text-sm text-slate-700">{score.correct} correct + {score.incorrect} incorrect + {score.unanswered} unanswered = <span className="font-semibold">{score.rawScore} points</span></p>
            </div>
        )}
        {/* Summary cards */}
...
<p className="mt-6 text-center text-xs text-gray-400">{questions.length} questions reviewed.</p>
</div>;
