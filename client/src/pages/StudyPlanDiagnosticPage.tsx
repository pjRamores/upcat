...
export default function StudyPlanDiagnosticPage() {
  ...
  useEffect(() => {
    if (!id) return;
    void studyPlanApi.getDiagnosticQuestions(id, section).then((r) =>
    ...
    <div className="mx-auto max-w-4xl space-y-4 px-4 py-8">
      ...
      <div className="flex justify-end">
        <button type="button" onClick={submitSection}>
          <div className="rounded-lg bg-sky-600 px-4 py-2 font-semibold text-white">{sectionIndex < SUBJECT_AREAS.length - 1 ? "Submit Section" : "Complete Diagnostic"}</div>
        </button>
      </div>
    </div>
  );
