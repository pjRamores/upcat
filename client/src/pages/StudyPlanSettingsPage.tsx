...
export default function StudyPlanSettingsPage() {
    ...
    if (!planId) {
        return <div className="mx-auto max-w-3xl px-4 py-8 text-slate-500">No active plan available to adjust.</div>;
    }
    ...
    return (
        <div className="mx-auto max-w-3xl space-y-5 px-4 py-8">
            ...
            <h1 className="text-2xl font-bold text-slate-900">Study Plan Settings</h1>

            <section className="rounded-xl border border-slate-200 bg-white p-4">
                <label className="block text-sm font-medium">Hours per day: {hours.toFixed(1)}</label>
                <input type="range" min={0.5} max={6} step={0.5} value={hours} onChange={(e) => setHours(Number(e.target.value))} className="mt-2 w-full" />
            </section>

            <section className="rounded-xl border border-slate-200 bg-white p-4">
                <label className="block text-sm font-medium">Target exam date</label>
                <input type="date" value={targetDate} onChange={(e) => setTargetDate(e.target.value)} className="mt-1 rounded border-slate-300 px-2 py-1" />
            </section>
            ...
            <div className="flex gap-2">
                <button type="button" onClick={save} className="rounded-lg bg-sky-600 px-4 py-2 font-semibold text-white">Save Changes</button>
                <button type="button" onClick={() => void studyPlanApi.pausePlan(planId)} className="rounded-lg border border-amber-500 px-4 py-2 text-amber-700">Pause Plan</button>
                <button type="button" onClick={() => void studyPlanApi.resumePlan(planId)} className="rounded-lg border border-emerald-500 px-4 py-2 text-emerald-700">Resume</button>
                <button type="button" onClick={() => void studyPlanApi.abandonPlan(planId)} className="rounded-lg border border-rose-500 px-4 py-2 text-rose-700">Abandon</button>
            </div>
        </div>
    );
}