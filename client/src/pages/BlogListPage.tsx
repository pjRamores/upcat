  ...
  <>
    {data.posts.length === 0 ? (
      ...
      <ul className="grid gap-6 sm:grid-cols-2">
        {data.posts.map((post) => (
          <li key={post._id} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            {post.heroImage && (
              <img src={post.heroImage} alt="" loading="lazy" className="mb-4 h-40 w-full rounded object-cover" />
            )}
            <h2 className="text-lg font-semibold text-slate-900">
              <Link to={`/blog/${post.slug}`} className="hover:text-primary-700">{post.title}</Link>
            </h2>
            <p className="mt-2 text-sm text-slate-600 line-clamp-3">{post.summary}</p>
            <p className="mt-3 text-xs text-slate-500">By {post.authorName} {post.publishedAt && (
              <>
                {"..."}
...
{totalPages > 1 && (
  <nav className="mt-8 flex justify-center gap-2 text-sm" aria-label="Pagination">
    {Array.from({length: totalPages}, (_, i) => i + 1).map((p) => {
      ...
      return (
        <Link key={p} to={`/blog?${next.toString()}`} className={`rounded px-3 py-1 ${p === page ? "bg-blue-600 text-white" : "border border-slate-300 text-slate-700"}`}>
          {p}
        </Link>
      );
    })}
