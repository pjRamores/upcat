...
export default function BlogPostPage() {
  ...
  if (status === 404 || !post) {
    return (
      <div className="mx-auto·max-w-xl·px-4·py-16·text-center">
        ...
        <h1 className="text-2x1·font-semibold·text-slate-900">Post not found</h1>
        <p className="mt-2·text-sm·text-slate-600">It may have been moved or unpublished.</p>
        <button onClick={() => navigate("/blog")}>
          className="mt-6·rounded·bg-primary-600·px-4·py-2·text-sm·font-medium·text-white·hover:bg-primary-700">
            Back to blog
          </button>
        </div>
      </div>
  ...
  return (
    <article className="mx-auto·max-w-3x1·px-4·py-10">
      <SEOHead
        title={post.title}
        description={post.summary}
        canonicalUrl={url}
