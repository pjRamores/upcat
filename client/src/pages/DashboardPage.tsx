...
return (
    <div className="mx-auto max-w-5xl px-4 py-10" data-tour="dashboard-main">
        ...
        <div>
            ...
            <Link to="/help" data-tour="help-link"
        ...
        {profile && (
            <Link to="/profile"
                  ...
                  className="mt-6 block rounded-xl border border-indigo-200 bg-gradient-to-br from-indigo-50 to-purple-50 p-5 shadow-sm transition hover:from-indigo-100:to-purple-100">
                <div className="flex flex-wrap items-center gap-4">
                    <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 text-2xl font-bold text-white shadow-md">{profile.level.level}</div>
                    <div className="flex-1 min-w-0">
                        <div className="text-xs font-semibold uppercase tracking-wide text-indigo-700">{profile.level.title}</div>
                        <div className="font-bold text-slate-900">{profile.level.xp.toLocaleString()} XP</div>
                        <div className="mt-1 h-2 overflow-hidden rounded-full bg-white">
                            <div className="h-full bg-gradient-to-r from-indigo-500 to-purple-500" style={{width: `${profile.level.progressPct}%`}}/>
                        </div>
                        <div className="mt-1 text-[11px] text-slate-500">{profile.level.xpToNextLevel.toLocaleString()} XP to level{" "}{profile.level.level + 1}</div>
                    </div>
                </div>
            </Link>
        )}
...
<div className="mt-5 grid gap-4 md:grid-cols-2">
  <button
    ...
    onClick={handleStart}
    data-tour="mock-card"
    className={`w-full rounded-xl border border-maroon-200 bg-white/80 p-4 text-left shadow-sm transition hover:bg-white cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-maroon-400`}
  ...
  >
    <div className="flex items-start justify-between gap-3">
      <div>
        {starting ? (
          <div
            className="flex items-center gap-2 rounded-full bg-rose-100 px-2 py-1 text-xs font-semibold text-rose-700"
          >
            <Spinner />
            Loading
          </div>
    ...
    {starting ? (
      <div className="mt-4">
        <p className="text-xs text-slate-500">{hasInProgress ? "Opening your in-progress mock exam..." : "Preparing your mock exam..."} </p>
      </div>
    ) : hasInProgress ? (
      <div className="mt-4">
  ...
  <Link to="/practice" data-tour="review-card" className="block rounded-xl border border-maroon-200 bg-white/80 p-4 shadow-sm transition hover:bg-white">
    <div className="flex items-start justify-between gap-3">
      <div>
  ...
  {studyToday && (
    <Link to={`/study-plan?from=dashboard&plan=${studyToday.planId}`} data-tour="study-plan-card" className="mt-6 block rounded-xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-teal-50 p-5 shadow-sm transition hover:from-emerald-100 + " hover:to-teal-100">
      <div className="flex flex-wrap items-center gap-4">
...
<Modal isOpen={Boolean(upgradePrompt)} onClose={() => setUpgradePrompt(null)}
  size="md" title={<span className="text-primary-700">Upgrade to Premium</span>}
...
<div className="text-sm font-semibold uppercase tracking-wide text-gray-500">{label}</p>
