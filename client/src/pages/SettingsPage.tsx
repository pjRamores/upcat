 ...
 {/* ---- Profile ---- */}
  <Section title="Profile">
    <dl className="grid grid-cols-1 gap-y-2 text-sm sm:grid-cols-2">
      <Row label="Name">{user?.firstName} {user?.lastName}</Row>
  ...
  <Section title="Linked accounts">
    {accounts && accounts.length > 0 ? (
      <ul className="divide-y divide-gray-100 rounded-lg border border-gray-200">
        {accounts.map((a) => (
          <li key={a.provider} className="flex items-center justify-between gap-4 px-4 py-3 min-w-0">
            <div>
              <p className="text-sm font-semibold text-gray-900">{SOCIAL_PROVIDER_META[a.provider].label}</p>
              <p className="truncate text-xs text-gray-500">{a.email ?? "-"}</p>
              {a.lastLoginAt ? `Last used ${new Date(a.lastLoginAt).toLocaleDateString()}` : ""}
            </div>
            <button type="button" onClick={() => onUnlink(a.provider)} disabled={lastSocialOnly} title={lastSocialOnly ? "Set a password before unlinking your only sign-in method." : ""}>
              <span className="rounded-md border border-red-200 bg-white px-3 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50">Unlink</span>
            </button>
          </li>
        ))}
      </ul>
  ...
  <Section title={hasPassword ? "Change password" : "Set a password"}>
    <PasswordSection hasPassword={!hasPassword} onChange={reload} />
  </Section>
  ...
  <Section title="Subscription & payments">
    <p className="text-sm text-gray-600">Review your current plan, payment status, and manual submission history.</p>
    <Link to="/settings/payments" className="btn-primary mt-3 inline-block">Manage subscription</Link>
  </Section>
...
function EmailPreferencesSection() {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600">
        Control which optional emails we send you. Transactional messages (email verification, password reset) are always sent regardless of this setting.
      </p>
...
class Name={
    absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${marketing ? "translate-x-4" : "translate-x-0.5"}
}
</div>
</div>
<div>
...
function Section({
    ...
    return (
        <section
            className={`rounded-xl border bg-white p-6 shadow-sm ${tone === "danger" ? "border-red-200" : "border-gray-200"}`}
        >
            <h2
                className={`mb-4 text-base font-semibold ${tone === "danger" ? "text-red-700" : "text-gray-900"}`}
            >
                {title}
            </h2>
            {children}
...
function PasswordSection({
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const msg = (err as { response?: { data?: { error?: string } } }).response?.data?.error || "Could not save password.";
            addToast("error", msg);
        } finally {
            setBusy(false);
        }
    ...
    <form onSubmit={submit} className="space-y-3">
        {hasPassword && (
            <Field label="Current password">
                <input type="password" value={current} onChange={(e) => setCurrent(e.target.value)} required className="input-field" />
            </Field>
        )}
        <Field label="New password">
            <input type="password" value={next} onChange={(e) => setNext(e.target.value)} required className="input-field" />
        ...
        <button type="submit" disabled={busy} className="btn-primary">{busy ? "Saving..." : hasPassword ? "Change password" : "Set password"}</button>
    </form>
}
...
return (
  <div className="space-y-3 text-sm text-gray-700">
    ...
    <p className="text-xs text-gray-500">
      {status.hasRecoveryCodes ? `${status.unusedCount}/${status.totalCount} codes remaining · generated ${status.generatedAt ? new Date(status.generatedAt).toLocaleDateString() : "__"}` : "You have no recovery codes yet. Generate ${RECOVERY_CODE_COUNT} codes you can store securely."}
    </p>
    ...
    <Modal isOpen={!codes} onClose={() => confirmed && setCodes(null)} title="Save your recovery codes" hideCloseButton={!confirmed}>
      <div className="space-y-3 text-sm">
        <p className="rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-800">
          These codes will only be shown once. Save them somewhere safe.
        </p>
        <pre className="grid grid-cols-2 gap-2 rounded-md bg-gray-50 p-3 font-mono text-xs">{codes?.map((c) => <span key={c}>{c}</span>)}</pre>
      </div>
      <div className="flex flex-wrap gap-2">
        <button onClick={copyAll} className="btn-secondary text-xs">Copy</button>
        <button onClick={downloadAll} className="btn-secondary text-xs">Download .txt</button>
      </div>
      ...
      <div className="flex justify-end">
        <button type="button" onClick={() => setCodes(null)} disabled={!confirmed} className="btn-primary text-xs">Done</button>
      </div>
    </Modal>
  </div>
);
...
const submit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const msg = (err as { response?: { data?: { error?: string; }; } }).response?.data?.error || "Could not save your answers.";
        addToast("error", msg);
    } finally {
        setBusy(false);
    }
...
return (
    <div className="space-y-4 text-sm">
        <p className="text-gray-600">Download a copy of your account data. Exports are limited to 1 per 24 hours.</p>
            ...
            .map(([k, l]) => (
                <label key={k} className="flex items-center gap-2 text-sm">
                    <input
                        type="checkbox"
                        checked={opts[k]}
                        onChange={(e) => setOpts({ ...o, [k]: e.target.checked })}
                    />
                    {l}
                </label>
            ))}
        ...
        <label className="block text-sm">
            ...
            <select value={opts.format} onChange={(e) => setOpts({ ...o, format: e.target.value as ExportFormat })}>
        ...
        {recent && recent.length > 0 &&
            <div className="border-t border-gray-100 pt-3">
                <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Recent exports</p>
                <ul className="divide-y divide-gray-100 rounded-md border border-gray-200">
                    {recent.map((r) => {
                        const ready = r.status === "ready";
                        return (
                            <li key={r._id} className="flex items-center justify-between gap-3 px-3 py-2 text-xs">
                                <div>
                                    <p className="font-medium text-gray-800">{r.export?.format.toUpperCase() ?? "JSON"}{" "}
                                        {new Date(r.requestedAt).toLocaleString()}
                                    </p>
                                    <p className="text-gray-500">{r.status}</p>
                                    {r.export?.expiresAt ? `Expires ${new Date(r.export.expiresAt).toLocaleString()}` : ""}
                                </div>
                                {ready && r.export &&
                                    <button onClick={() => download(r._id, r.export!.format)} className="btn-secondary text-xs">
                                        Download
                                    </button>
                                }
                            </li>
...
<div className="mt-3 rounded-md border border-red-200 bg-red-50 p-3 text-sm">
  ...
  <button type="button" onClick={cancel} className="mt-2 rounded-md border border-red-300 bg-white px-3 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-100">
    Cancel deletion
  </button>
</div>
...
return (
  <>
    <button type="button" onClick={() => setOpen(true)} className="mt-4 rounded-md border border-red-300 bg-white px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-50">
      Delete my account
    </button>
    {Modal.isOpen && (
      <Modal isOpen={open} onClose={close} title="Schedule account deletion" size="lg">
        <form onSubmit={submit} className="space-y-4">
          <div className="rounded-md bg-red-50 p-3 text-xs text-red-800">
            <p className="font-semibold">You'll have {DATA_DELETION_GRACE_DAYS} days to change your mind. After that:</p>
          ...
          <fieldset>
            ...
            <label className="mt-1 flex items-start gap-2 text-sm">
              <input type="radio" name="scope" checked={scope === "full"} onChange={() => setScope("full")}>
                <span>Full deletion</span>
              </input>
              Remove account + all associated data.
            </label>
            <label className="mt-1 flex items-start gap-2 text-sm">
              <input type="radio" name="scope" checked={scope === "data_only"} onChange={() => setScope("data_only")}>
                <span>Data only</span>
              </input>
              Keep your account but wipe exam history / personal data.
            </label>
          </fieldset>

          <label className="flex items-start gap-2 text-sm">
            <input type="checkbox" checked={retainStats} onChange={(e) => setRetainStats(e.target.checked)}>
              <span>Allow my anonymized aggregate stats to remain (helps platform improvements).</span>
            </input>
          </label>

          <Field label={`Type "${ACCOUNT_DELETE_CONFIRMATION}" to confirm`}>
            <input autoFocus value={confirmation} onChange={(e) => setConfirmation(e.target.value)} className="input-field" />
          </Field>
          {hasPassword && (
...
<div className="flex justify-end gap-2 pt-2">
    <button type="button" onClick={close} disabled={busy} className="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</button>
    <button type="submit" disabled={busy} className="rounded-md bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 disabled:opacity-60">{busy ? "Scheduling..." : "Schedule deletion"}</button>
</div>
<form>
    <Modal>
        {/* Reference to avoid "unused" for onExecuted in this layout (kept for parity with old API). */}
        <span className="hidden" data-handler={onExecuted.toString().length} />
    </Modal>
</form>