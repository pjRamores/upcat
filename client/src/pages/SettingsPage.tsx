...
export default function SettingsPage() {
  ...
  const onUnlink = async (provider: SocialProvider) => {
    try {
      ...
      const msg = (err as { response?: { data?: { error?: string } } }).response?.data?.error || "Could not unlink that provider.";
      addToast("error", msg);
    }
  };
...
<p>Manage how you sign in, your password, and your account data.</p>
</header>
...
<Section title="Profile">
  ...
  {hasPassword && (
    <p className="mt-4 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">
      You signed in with a social provider only — set a password below so you don't lose access if that provider becomes unavailable.
    </p>
  )}
</Section>
...
<Section title="Linked accounts">
  {accounts && accounts.length > 0 ? (
    <ul className="divide-y divide-gray-100 rounded-lg border border-gray-200">
      {accounts.map((a) => (
        <li key={a.provider} className="flex items-center justify-between gap-4 px-4 py-3">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{SOCIAL_PROVIDER_META[a.provider].label}</p>
            <p className="truncate text-xs text-gray-500">{a.email ?? "-"}. linked {new Date(a.linkedAt).toLocaleDateString()} {a.lastLoginAt ? lastUsed ? new Date(a.lastLoginAt).toLocaleDateString() : ""}</p>
          </div>
          <button type="button" onClick={() => onUnlink(a.provider)} disabled={lastSocialOnly} title={lastSocialOnly ? "Set a password before unlinking your only sign-in method." : ""} className="rounded-md border border-red-200 bg-white px-3 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50">
            Unlink
          </button>
        </li>
      ))}
...
<div className="mt-4">
  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Link another account</p>
...
<Section title="Support">
  <p className="text-sm text-gray-600">Manage how you sign in, your password, and your account data.</p>
</Section>
function EmailPreferencesSection() {
  ...
  const handleToggle = async (value: boolean) => {
    setSaving(true);
    try {
      const { emailPreferences } = await emailPreferencesApi.update({
        marketing: value,
      });
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600">
        Control which optional emails we send you. Transactional messages (email
        verification, password reset) are always sent regardless of this setting.
      </p>
      <label className="flex cursor-pointer items-start gap-4">
        ...
        <span className="ml-3 text-sm text-gray-600">
          Newsletters, promotions, and announcements
        </span>
      </label>
    </div>
  );
...
function HelpGuidanceSection() {
    ...
    useEffect(() => {
        const prefs = user?.helpPreferences;
        if (!prefs) return;
...
return (
    <div className="space-y-3 text-sm text-gray-700">
        <p>
            Recovery codes let you sign in if you lose access to your password and social providers. Each code works once.
        </p>
        <p className="text-xs text-gray-500">
            {status.hasRecoveryCodes
                ? `${status.unusedCount}/${status.totalCount} codes remaining · generated ${status.generatedAt ? new Date(status.generatedAt).toLocaleDateString() : "--"}`
        ...
        <Modal isOpen={!codes} onClose={() => confirmed && setCodes(null)} title="Save your recovery codes" hideCloseButton={!confirmed}>
            ...
            <div className="flex justify-end">
                <button type="button" onClick={() => setCodes(null)} disabled={!confirmed} className="btn-primary text-xs">
                    Done
                </button>
            </div>
        </Modal>
...
return (
    <div className="space-y-4 text-sm">
        ...
        <label className="block text-sm">
            ...
            <select value={opts.format} onChange={(e) => setOpts((o) => ({ ...o, format: e.target.value as ExportFormat }))}>
...
function DeletionFlow({
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const msg = (err as { response?: { data?: { error?: string } } }).response?.data?.error || "Could not schedule deletion.";
            addToast("error", msg);
        } finally {
            setBusy(false);
        }
    ...
    const cancel = async () => {
        ...
        try {
            ...
            const msg = (err as { response?: { data?: { error?: string } } }).response?.data?.error || "Could not cancel deletion.";
            addToast("error", msg);
        }
    };
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <fieldset>
                    ...
                    <label className="mt-1 flex items-start gap-2 text-sm">
                        ...
                        />
                        <span>
                            <strong>Data only</strong> – keep your account but wipe exam history / personal data.
                        </span>
                    </label>
                </fieldset>
                ...
                <Field label={`${ACCOUNT_DELETE_CONFIRMATION} to confirm`}>
                    <input
                        autoFocus
                        value={confirmation}
