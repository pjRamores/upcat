...
function addHeadingAnchors(html: string): string {
    return html.replace(/<h([2-3])>(.*?)<\/h\1>/g, (_, full, level, content) => {
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 lg:grid-cols-[1fr,260px]">
            ...
            <article>
                <nav className="text-xs text-slate-500">
                    ...
                    <span> / </span>
                    <Link to={`/help/category/${article.category}`} className="hover:text-indigo-700">{article.category}</Link>
                    <span> / </span>
...
(
    <section className="mt-5 rounded-xl border border-indigo-100 bg-indigo-50 p-4">
        ...
        <dl className="mt-2 grid gap-2 sm:grid-cols-2">
            {article.quickFacts.map(({fact: {label, value}})) => (
...
(
    <section className="mt-8 rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 space-y-2">
            {article.fags.map(({faq: {question, answer}})) => (
            ...
            <ul className="mt-3 space-y-1">
                {headings.map((heading) => (
                    <li key={heading.id} className={heading.level === 3 ? "pl-3" : ""}>
                        <a href={`#${heading.id}`} className="text-xs text-slate-600 hover:text-indigo-700">{heading.text}</a>
                    </li>
                ))}
            </ul>
        </div>
