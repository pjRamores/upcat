...
function addHeadingAnchors(html: string): string {
  return html.replace(/<h([2-3])>(.*?)<\/h\1>/g, (_, level, content) => {
...
export default function HelpArticlePage() {
  ...
  const headings = useMemo(
    () => extractHeadings(String(article?.content?.body ?? "")),
  ...
  if (!article) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16">
        ...
        <Link to="/help" className="mt-4 inline-block text-sm font-medium text-indigo-700 hover:underline">
          Back to Help Center
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 lg:grid-cols-[1fr,260px]">
      ...
      <article>
        <nav className="text-xs text-slate-500">
          ...
          <span> / </span>
          <Link to={`/help/category/${article.category}`} className="hover:text-indigo-700">
            {article.category}
          </Link>
          <span> / </span>
        ...
        {Array.isArray(article.quickFacts) && article.quickFacts.length > 0 && (
          <section className="mt-5 rounded-xl border border-indigo-100 bg-indigo-50 p-4">
            ...
            <dl className="mt-2 grid gap-2 sm:grid-cols-2">
              {article.quickFacts.map(({ fact }: any) => (
        ...
        <div className="mt-6 flex flex-wrap gap-4 text-sm">
          <Link to="/help" className="font-medium text-indigo-700 hover:underline">
            Back to Help Center
          </Link>
          {isAdmin && (
            <Link to="/admin/help/articles" className="text-slate-700 hover:underline">
              Edit this article
            </Link>
          )}
        </div>
