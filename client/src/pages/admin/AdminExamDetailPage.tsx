...
return (
  <div className="space-y-6">
    ...
    >
      <div>
        <h2 className="text-lg font-bold text-slate-900">Session {s._id.slice(-5)}</h2>
        <p className="text-sm text-slate-500">
          {data.user ? <Link to={`/admin/users/${data.user.id}`} className="text-primary-700 hover:underline">{data.user.firstName} {data.user.lastName}</Link> : "deleted user"}
        </p>
        <div className="mt-2 flex flex-wrap gap-2">
          <Badge
            variant={s.status === "completed" ? "success" : s.status === "in_progress" ? "info" : "warning"}
          >{s.status}</Badge>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/admin/exams" className="rounded-md border border-slate-200 px-3 py-1.5 text-xs hover:bg-slate-50">
            Back
          </Link>
        </div>
      </div>
      {s.score && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Stat label="Correct" value={String(s.score.correct ?? 0)}/>
          <Stat label="Incorrect" value={String(s.score.incorrect ?? 0)}/>
          <Stat label="Unanswered" value={String(s.score.unanswered ?? 0)}/>
        </div>
      )}
      {s.score?.bySubject && (
          ...
          <ul className="divide-y divide-slate-100 text-sm">
            {Object.entries(s.score.bySubject).map(([k, v]) => (
              <li key={k} className="flex items-center justify-between py-2">
                <span>{k}</span>
                <span className="text-xs text-slate-500">{v.correct} / {v.total} • {v.percentage.toFixed(1)}%</span>
              </li>
            ))}
          </ul>
        </section>
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500">
            {filteredQuestions.length === 0 ? "0 of 0" : `${pageStart + 1}-${Math.min(pageStart + QUESTIONS_PER_PAGE, filteredQuestions.length)} of ${filteredQuestions.length}`}
          </div>
        </div>
        <button
          type="button"
          onClick={() => setQuestionPage((p) => Math.max(1, p - 1))}
          disabled={clampedQuestionPage <= 1}
          className="rounded-md border border-slate-200 px-2 py-1 disabled:opacity-40"
        >
          Prev
        </button>
        <span>
          Page {clampedQuestionPage} / {totalQuestionPages}
        </span>
        <button
          type="button"
          onClick={() => setQuestionPage((p) => Math.min(totalQuestionPages, p + 1))}
          disabled={clampedQuestionPage >= totalQuestionPages}
          className="rounded-md border border-slate-200 px-2 py-1 disabled:opacity-40"
        >
          Next
        </button>
      ...
      <div className="flex flex-wrap gap-2">
        {(["", ...SUBJECT_AREAS] as const).map((subject) => (
          <button
            key={subject || "all-subjects"}
            type="button"
            onClick={() => setSubjectFilter(subject)}
            className={subjectFilter === subject ? "border-primary-600 bg-primary-600 text-white" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}
          >
        ...
        <header className="mb-2 flex flex-wrap items-center gap-2 text-xs">
            ...
            {q.question && (
                <Badge
                    ...
                    text={DIFICULTY_LABELS[q.question.difficulty as Difficulty] ?? q.question.difficulty}
                />
            )}
                    ...
                    {q.question.choices.map((c) => {
                        ...
                        return (
                            <li key={c.label} className={`flex items-start gap-2 rounded border p-2 ${cls}`}>
                                ...
                                <span className="flex-1"><MathText>{c.text}</MathText></span>
