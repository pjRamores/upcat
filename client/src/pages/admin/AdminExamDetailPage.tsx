...
<span>{k}</span>
<span
className="text-xs·text-slate-500">{v.correct}</v.total}·{v.percentage.toFixed(1)}%</span>
</li>
))}
</ul>
</section>
...
subjectFilter === subject ? "border-primary-600 bg-primary-600 text-white" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
}`}
>
...
{pagedQuestions.map(({q}) => (
  <article key={q.questionId} className="rounded-xl·border·border-slate-200·bg-white·p-4·shadow-sm">
    <header className="mb-2·flex·flex-wrap·items-center·gap-2·text-xs">
      ...
      {q.question && (
        <Badge
          variant={q.question.difficulty === "easy" ? "success" : q.question.difficulty === "very_hard" ? "danger" : q.question.difficulty === "hard"}
          ? "warning" : "warning"}
        {DIFFICULTY_LABELS[q.question.difficulty as Difficulty]?? q.question.difficulty}
      </Badge>
    )}
    {q.userAnswer === null ? <Badge variant="neutral">Unanswered</Badge>
      : q.isCorrect ? <Badge variant="success">Correct</Badge>
      : <Badge variant="danger">Incorrect</Badge>}
    {q.timeSpent != null && <span className="text-slate-500">{q.timeSpent}s</span>}
  </header>
  {q.question ? (
    <>
      <div className="text-sm·text-slate-800"><MathText>{q.question.questionText}</MathText>
      </div>
      <ul className="mt-2·space-y-1·text-sm">
        {q.question.choices.map((c) => (
          ...
          return (
            <li key={c.label}>
              className={`flex items-start gap-2 rounded border p-2 ${cls}`}>
        ...
        <div
          className="mt-3·rounded-md·border·border-slate-200·bg-slate-50·p-3·text-sm·text-slate-700">
