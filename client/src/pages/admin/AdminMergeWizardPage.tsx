...
export default function AdminMergeWizardPage() {
    ...
    const execute = async () => {
        ...
        try {
            ...
            const msg = (err as { response?: { data?: { error?: string } } }).response?.data?.error || "Merge failed.";
            addToast("error", msg);
        } finally {
            setBusy(false);
        }
...
<ol className="flex flex-wrap gap-2 text-xs">
    {["Primary", "Secondary", "Strategy", "Re-auth", "Confirm", "Done"].map(
        (label, i) => (
            <li key={label} className={`rounded-full px-3 py-1 font-semibold ${i === 0 ? "bg-primary-600 text-white" : i > 0 ? "bg-emerald-100 text-emerald-800" : "bg-gray-100 text-gray-600"}`}>
                > {i + 1}. {label}
            </li>
        )
    )}
...
{step === 1 && (
    <Card title="1. Primary (keeper) user">
        <p className="text-xs text-gray-500">
            The account that will survive. All identities, sessions and contact messages will be transferred into this account.
        </p>
        <input
    ...
    <>
        ...
        <Card title="2. Secondary (to be removed) user">
            ...
            <div className="flex gap-2">
                ...
                <button onClick={findSecondary} disabled={!secondaryQuery} className="btn-primary">
                    Look up
                </button>
            </div>
        </Card>
    ...
    <>
        ...
        <Card title="3. Merge strategy">
            <label className="flex items-start gap-2 text-sm">
                ...
                />
                <span>
                    <strong>Keep primary data</strong> - transfer identities & contacts only. Secondary's exam sessions are discarded.
                </span>
            </label>
            <label className="flex items-start gap-2 text-sm">
                <input
...
function UserSummary({ label, u, tone = "default" }: { label: string; u: AdminUserLite; tone?: "default" | "danger"; }) {
  return (
    <section
      className={`rounded-xl border p-3 text-sm ${tone === "danger" ? "border-red-200 bg-red-50" : "border-emerald-200 bg-emerald-50"}`}
    >
      <div className="flex gap-2">
