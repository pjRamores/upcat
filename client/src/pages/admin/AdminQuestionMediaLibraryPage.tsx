...
export default function AdminQuestionMediaLibraryPage() {
  ...
  const upload = async () => {
    if (!filename.trim() || !mimeType.trim() || !base64Data.trim()) {
    ...
    <div className="space-y-6">
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Upload base64 assets and reference IDs in rich content blocks.
        </p>
            ...
            {!loading &&
              items.map((asset) => (
                <tr key={asset._id}>
                  ...
                  <td className="px-3 py-2 text-xs text-slate-500">{asset.mimeType}</td>
