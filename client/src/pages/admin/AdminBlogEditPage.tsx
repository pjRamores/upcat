...
try {
    if (isNew) {
        ...
        setOriginal(updated);
        setForm(({ ...f, status: updated.status }));
    ...
    <div className="space-y-5">
        <div className="flex items-center justify-between">
            ...
            {original?.status === "published" && (
                <a href={`/blog/${original.slug}`} target="_blank" rel="noreferrer" className="text-sm text-primary-600 underline">
                    View live
                </a>
            )}
        </div>
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
            <div className="space-y-4 lg:col-span-2">
                <Section>
                    <Field label="Title">
                        <input type="text" value={form.title} onChange={(e) => onTitleChange(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-lg font-semibold"/>
                    </Field>
                    <Field label="Slug">
                        <input type="text" value={form.slug} onChange={(e) => {
                            setSlugTouched(true);
                            update("slug", e.target.value.toLowerCase());
                        }} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm font-mono"/>
                    </Field>
                    <Field label="Summary (used for meta description + cards)">
                        <textarea rows={2} value={form.summary} onChange={(e) => update("summary", e.target.value)} maxLength={280} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"/>
                    </Field>
                </Section>
                <Section>
                    <div className="flex items-center justify-between">
                        <span className="text-sm font-semibold text-slate-700">Body (Markdown) - {readMin} min read</span>
                    </div>
                    <button type="button" onClick={() => setPreview((p) => !p)} className="rounded-md border border-slate-300 px-2 py-1 text-xs hover:bg-slate-50">
