    ...
    <div className="space-y-6">
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex flex-wrap gap-1">
          {["dashboard", "Dashboard"], ["logs", "Logs"], ["alerts", "Alerts"], ["checks", "Health Checks"]}
            .map(({ id, label }) => (
              <button
                key={id}
                type="button"
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(["1h", "6h", "24h", "7d"] as const).map((r) => (
          <button
            ...
            type="button"
            onClick={() => setRange(r)}
            className={`rounded-full border px-3 py-1.5 text-xs font-medium ${range === r ? "border-primary-600 bg-primary-600 text-white" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"} `}
          >
            {r}
          </button>
        ))}
            ...
            <div className="overflow-x-auto rounded border border-slate-200">
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {logs.map((log, idx) => (
                            <tr key={idx}>
                                <td className="px-2 py-2 text-xs text-slate-500">{String(log.timestamp ?? "")}</td>
                        ...
                        <tr key={idx}>
                            ...
                            <td className="px-2 py-2 text-right">
                                <div className="flex justify-end gap-1">
                                    ...
                                    <button
                                        type="button"
                                        className="rounded border-emerald-300 px-2 py-1 text-xs text-emerald-700"
                                        onClick={() => {
...
function HealthChecksTab() {
    ...
    return (
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {checks.map((check) => (
                <div key={String(check.checkId)} className="rounded border border-slate-200 bg-white p-3">
                    <p className="text-sm font-semibold text-slate-900">{String(check.name ?? check.checkId)}</p>
