    ...
    },
    {
        ...
        render: (r) => (
            <div className="text-xs">
                <p className="font-semibold">{r.totalQuestions} configured</p>
    ...
    },
    {
        ...
        render: (r) => (
            <div className="flex justify-end gap-1">
                <Link to={`/admin/questions?setId=${encodeURIComponent(String(r._id ?? ""))}`}>
                    Questions
                </Link>
                ...
                <button type="button" onClick={() => setConfirmDeleteId(r._id ?? null)}>
                    Delete
                </button>
            </div>
        ),
...
<div className="space-y-4">
  <div className="flex items-center justify-between">
    ...
    <div className="flex gap-2">
      <Link to="/admin/questions" className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">
        Back to Questions
      </Link>
      <button type="button" onClick={openCreate} className="rounded-md bg-primary-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-primary-700">
        + New Question Set
      </button>
    </div>
  </div>
  ...
  <Modal isOpen={showModal} onClose={() => setShowModal(false)}>
    <title>{mode === "create" ? "New Question Set" : "Edit Question Set"}</title>
...
{errors.distributionTime ?? <p className="mt-1 text-xs text-red-600">{errors.distributionTime}</p>}
...
<div className="space-y-2">
  ...
  <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
    [
      ...
      {key: "very_hard", label: "Very Hard", color: "bg-rose-100 text-rose-700 border-rose-200"}
    ].map((difficulty) => (
...
<div className="flex justify-end gap-2 border-t border-slate-100 pt-3">
  <button type="button" onClick={() => setShowModal(false)} className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">
    Cancel
  </button>
</div>
</button>
