...
export default function AdminQuestionSetsPage() {
  ...
  const load = async (p = page) => {
    setLoading(true);
    try {
      const result = await adminApi.getQuestionSets({
        page: p,
        limit: 20,
        sort: sortBy,
        order: sortOrder,
      ...
      setData({
        items: result.items.map((item: AdminQuestionSet) => ({ ...item })),
        total: result.total,
  ...
  const validate = (): boolean => {
    ...
    const tTotal = SUBJECT_AREAS.reduce(
      (sum, s) => sum + (form.distribution[s]?.timeLimit ?? 0),
      0
    );

    if (tTotal <= 0) {
      next.distributionTime = "At least one subject must have a time limit > 0.";
    }
    ...
    },
    {
      ...
      render: (r) => (
        <div className="text-xs">
          <p>
            <span className="font-semibold">{r.totalQuestions}</span> configured
          </p>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-sm text-slate-500">
          Manage exam question sets. Subject item counts and difficulty percentages are
          read-only and auto-calculated from published question statuses.
        </div>
      ...
      >
        <div className="space-y-4 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="set-active" className="text-xs font-semibold text-slate-700">
              Active (visible for exam selection)
            </label>
          </div>
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-slate-700">
              ...
              <span className="ml-1 font-normal text-slate-500">
                (1-100; lower value = higher priority when selecting sets for users)
              </span>
            </label>
            <input
              type="number"
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-500">
                  Published items and difficulty mix are auto-calculated on
            ...
            <div className="overflow-hidden rounded-lg border border-slate-200">
              ...
              {SUBJECT_AREAS.map((subject) => {
                ...
                return (
                  <div
                    key={subject}
                    className="grid grid-cols-[minmax(120px,1fr)_112px_96px] items-center gap-2 border-t border-slate-100 bg-white px-3 py-2"
                  >
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDistribution(subject, "timeLimit", Number(e.target.value))
                      }
              ...
              {[
                ...
                >
                  ...
                  <div className="mt-1 flex items-center justify-between">
                    <span
                      className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${difficulty.color}`}
                    >
                      {
                        form.difficultyMix[
                          difficulty.key as keyof SetForm["difficultyMix"]
                        ]
                      }
                      %
                    </span>
