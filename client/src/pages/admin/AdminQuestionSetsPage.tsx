...
export default function AdminQuestionSetsPage() {
    ...
    useEffect(() => {
        load(page).then((result) => {
            setData(result.items.map((item: AdminQuestionSet) => ({ ...item })));
            setLoading(false);
        });
    }, [page]);
    ...
    },
    {
        ...
        render: (r) => (
            <div className="flex justify-end gap-1">
                <Link to={`/admin/questions?setId=${encodeURIComponent(String(r.id ?? ""))}`}>
                    <button type="button" onClick={() => openEdit(r)} className="rounded-md border border-slate-200 px-2 py-1 text-xs hover:bg-slate-50">Questions</button>
                </Link>
                <button type="button" onClick={() => handleToggleActive(r)} className="rounded-md border border-slate-200 px-2 py-1 text-xs hover:bg-slate-50">
                    {r.isActive !== false ? "Deactivate" : "Activate"}
                </button>
                <button type="button" onClick={() => setConfirmDeleteId(r.id ?? null)} className="rounded-md border border-primary-200 px-2 py-1 text-xs text-primary-600 hover:bg-primary-50">Delete</button>
            </div>
        ),
    },
];
return (
    <div className="space-y-4">
        ...
        <div className="flex gap-2">
            <Link to="/admin/questions" className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">Back to Questions</Link>
            <button type="button" onClick={openCreate} className="rounded-md bg-primary-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-primary-700">+ New Question Set</button>
        </div>
    </div>

    <DataTable columns={columns} rows={(data?.items ?? [])} getRowId={(r) => r._id ?? ""} isLoading={loading} onSort={onSort} sortBy={sortBy} sortOrder={sortOrder} />
    <Pagination page={page} totalPages={data?.totalPages ?? 1} total={data?.total ?? 0} onPageChange={setPage}/>

    {/* Create/Edit Modal */}
    <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={mode === "create" ? "New Question Set" : "Edit Question Set"}>
        <div className="space-y-4 text-sm">
            {/* Name */}
            <div>
                <label className="mb-1 block text-xs font-semibold text-slate-700">Name *</label>
                <input type="text" value={form.name} onChange={(e) => setForm((p) => ({...p, name: e.target.value}))} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm focus:border-primary-500 focus:outline-none"/>
            ...
            <div>
                ...
                <textarea value={form.description} onChange={(e) => setForm((p) => ({...p, description: e.target.value}))} rows={2} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm focus:border-primary-500 focus:outline-none"/>
            </div>

            {/* Active toggle */}
            <div className="flex items-center gap-2">
                <input id="set-active" type="checkbox" checked={form.isActive} onChange={(e) => setForm((p) => ({...p, isActive: e.target.checked}))} />
            ...
            <div>
                ...
                <input type="number" min={1} max={100} step={1} value={form.priority} onChange={(e) => {
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between">
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Subject
<p className="text-[11px] text-slate-500">
  Published items and difficulty mix are auto-calculated on publish/status changes.
</p>

<div className="grid grid-cols-1 gap-2 rounded-lg border border-slate-200 bg-gradient-to-r from-slate-50 to-white p-3 sm:grid-cols-2">
...
<div className="rounded-lg border border-slate-200 overflow-hidden">
  <div className="grid grid-cols-[minmax(120px,1fr)_112px_96px] items-center gap-2 bg-slate-50 px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
    <span>Subject</span>
    <span className="text-center">Time (min)</span>
    <span className="text-center">Items</span>
  </div>
  {SUBJECT_AREAS.map((subject) => {
    const itemCount = form.distribution[subject]?.questions ?? 0;

    return (
      <div key={subject} className="grid grid-cols-[minmax(120px,1fr)_112px_96px] items-center gap-2 border-t border-slate-100 bg-white px-3 py-2">
        <span className="text-xs font-medium text-slate-700">{subject}</span>
        <input
          type="number"
          min={0}
          value={form.distribution[subject]?.timeLimit ?? 0}
          onChange={(e) => setDistribution(subject, "timeLimit", Number(e.target.value))}
          className="w-full rounded border border-slate-300 px-2 py-1 text-center text-xs focus:border-primary-500 focus:outline-none"
          aria-label={`${subject} time limit in minutes`}
        />
        <input
          type="text"
          readOnly
          value={itemCount}
          className="w-full cursor-default rounded border border-slate-200 bg-slate-50 px-2 py-1 text-center text-xs text-slate-600"
          aria-label={`${subject} published items`}
        />
      </div>
    );
  })}
...
<div className="grid grid-cols-[minmax(120px,1fr)_112px_96px] items-center gap-2 border-t border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700">
  <span>Total</span>
  <span className="text-center">{totalTimeLimit} min</span>
  <span className="text-center">{totalPublishedItems}</span>
</div>

{errors.distributionTime && (
  <p className="mt-1 text-xs text-red-600">{errors.distributionTime}</p>
)}

{/* Difficulty distribution */}
<div className="space-y-2">
  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Difficulty Distribution</p>
  <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
    [
      ...
      {key: "hard", label: "Hard", color: "bg-amber-100 text-amber-700 border-amber-200"},
      {
        key: "very_hard",
        label: "Very Hard",
      ...
      <div key={difficulty.key} className="rounded-md border border-slate-200 bg-white px-3 py-2">
        <p className="text-[11px] uppercase tracking-wide text-slate-500">{difficulty.label}</p>
        <div className="mt-1 flex items-center justify-between">
          <span className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${difficulty.color}`}>{form.difficultyMix[difficulty.key as keyof SetForm["difficultyMix"]]}%</span>
        </div>
      </div>
    ))}
  </div>
...
<div className="flex justify-end gap-2 border-t border-slate-100 pt-3">
  <button type="button" onClick={() => setShowModal(false)} className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">Cancel</button>
</button>
<button
    type="button"
    onClick={handleSave}
