...
export default function AdminSupportTicketDetailPage() {
  ...
  const submitReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      ).response?.data?.error || "Could not send reply.";
      addToast("error", msg);
    } finally {
      setBusy(false);
    }
  ...
  const updateStatus = async (status: SupportTicketStatus) => {
    try {
      ...
      const msg = (
        err as { response?: { data?: { error?: string } } }
      ).response?.data?.error || "Could not update status.";
      addToast("error", msg);
    }
  };
      ...
      <div className="lg:col-span-2">
        <div className="rounded-t-xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h1 className="text-lg font-bold">{ticket.ticketNumber}</h1>
            <span
              className={`rounded-full px-2 py-0.5 text-xs font-semibold bg-${status.color}-50 text-${status.color}-700`}
            >
              {status.label}
            </span>
          </div>
...
{ticket.userId && (
    <SidePanel title="Account actions">
        <Link to={`/admin/users/${ticket.userId}`}>
            View user profile
        </Link>
        <button onClick={unlockAccount} className="btn-secondary mt-2 w-full text-xs">
            Unlock account
        </button>
        {ticket.type === "account_merge" && (
            <Link to={`/admin/support/merge?primary=${ticket.userId}&ticket=${ticket.ticketNumber}`}>
                Open merge wizard
            </Link>
        )}
    </SidePanel>
...
<VerifyIdentityModal open={verifyOpen} onClose={() => setVerifyOpen(false)} onSaved={(async () => {
    setVerifyOpen(false);
    await load();
})}>
...
<div className="flex justify-end gap-2 pt-2">
    ...
    <button type="submit" disabled={busy}.className="btn-primary text-xs">{busy ? "Saving..." : "Save verification"}</button>
</div>
</form>
</Modal>;
}