...
<label className="inline-flex items-center gap-1 text-xs text-slate-600">
  <input type="radio" name="correct" checked={form.correctAnswer === c.label} onChange={() => setForm({ ...form, correctAnswer: c.label })} />
  Correct
</label>
</div>

<Field label="Rationale">
  <FormattedTextarea value={form.rationale} onChange={(rationale) => setForm({ ...form, rationale })} rows={3} />
</Field>

<Field label="Tags (comma-separated)">
  <input type="text" value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
</Field>

<Field label="Media asset IDs (comma-separated)">
  <input type="text" value={form.mediaAssetIds} onChange={(e) => setForm({ ...form, mediaAssetIds: e.target.value })} className="w-full rounded-md border border-slate-300 px-2 py-1.5 font-mono text-xs" placeholder="ObjectId, ObjectId" />
</Field>

<Field label="Rich content blocks (JSON array)">
  <textarea value={form.contentBlocksJson} onChange={(e) => setForm({ ...form, contentBlocksJson: e.target.value })} rows={6} className="w-full rounded-md border border-slate-300 px-2 py-1.5 font-mono text-xs" placeholder='[{"id":"intro","type":"paragraph","text":"Context..."}]' />
</Field>
...
<div className="flex flex-wrap items-center justify-between gap-2 pt-3">
  ...
  <div className="flex gap-2">
    {isNew && (
      <button type="button" onClick={() => setPreview(!v)}>
        {preview ? "Hide preview" : "Preview"}
      </button>
    )}
    {!isNew && (
      <button type="button" onClick={publishNow} disabled={publishing || archiving || form.publicationStatus === "published"} className="rounded-md border border-emerald-300 px-3 py-1.5 text-sm font-semibold text-emerald-700 hover:bg-emerald-50 opacity-50">
        {publishing ? "Publishing..." : form.publicationStatus === "published" ? "Published" : "Publish now"}
      </button>
    )}
    {!isNew && (
      <button type="button" onClick={archiveNow} disabled={publishing || archiving || form.publicationStatus === "archived"} className="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 opacity-50">
        {archiving ? "Archiving..." : form.publicationStatus === "archived" ? "Archived" : "Archive"}
      </button>
    )}
    <button type="submit" disabled={saving} className="rounded-md bg-primary-600 px-4 py-1.5 text-sm font-semibold text-white hover:bg-primary-700 opacity-50">
...
{preview && (
  <aside className="space-y-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="prose primary-sm max-w-none text-slate-800">
      <MathText>{form.questionText}</MathText>
    </div>
    <ul className="space-y-2 text-sm">
      {form.choices.map((c) => (
        <li key={c.label}>
          <div className={`flex items-start gap-2 rounded-md border p-2 ${c.label === form.correctAnswer ? "border-emerald-300 bg-emerald-50" : "border-slate-200"}`}>
...
const PUBLICATION_STATUS_LABELS: Record<QuestionPublicationStatus, string> = {
  draft: "Draft",
  in_review: "In Review",
};
