      ...
      {form.type === "passage_based" && (
        <Field label="Passage">
          ...
          >
            ...
            {form.passageId && !passages.some((p) => p.id === form.passageId) && (
        ...
        {form.choices.map((c, i) => (
          <div key={c.label} className="flex items-center gap-2">
            ...
            <button
              ...
              className="ml-2 text-sm text-slate-600 font-bold"
            >
              Remove
            </button>
          </div>
...
<Field label="Media asset IDs (comma-separated)">
    <input type="text" value={form.mediaAssetIds} onChange={(e) => setForm({ ...form, mediaAssetIds: e.target.value })}
           className="w-full rounded-md border border-slate-300 px-2 py-1.5 font-mono text-xs" placeholder="ObjectId, ObjectId" />
</Field>
<Field label="Rich content blocks (JSON array)">
    <textarea
        value={form.contentBlocksJson} onChange={(e) => setForm({ ...form, contentBlocksJson: e.target.value })}
        rows={6} className="w-full rounded-md border border-slate-300 px-2 py-1.5 font-mono text-xs" placeholder='[{"id":"intro","type":"paragraph","text":"Context..."}]' />
</Field>
<div className="flex flex-wrap items-center justify-between gap-2 pt-3">
    <Link to="/admin/questions" className="text-sm text-slate-600 hover:underline">Back</Link>
    <div className="flex gap-2">
        <button type="button" onClick={() => setPreview((v) => !v)} className="rounded-md border border-slate-200 px-3 py-1.5 text-sm">
            {preview ? "Hide preview" : "Preview"}
        </button>
        {isNew && (
            <button type="button" onClick={publishNow} disabled={publishing || archiving || form.publicationStatus === "published"} className="rounded-md border border-emerald-300 px-3 py-1.5 text-sm font-semibold text-emerald-700 hover:bg-emerald-50 disabled:opacity-50">
                {publishing ? "Publishing..." : form.publicationStatus === "published" ? "Published" : "Publish now"}
            </button>
        )}
        {isNew && (
            <button type="button" onClick={archiveNow} disabled={publishing || archiving || form.publicationStatus === "archived"} className="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50">
...
{preview &&
    <aside className="space-y-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="prose p-primary-sm max-w-none text-slate-800">
            <MathText>{form.questionText}</MathText>
        </div>
        <ul className="space-y-2 text-sm">
            {form.choices.map((c) => (
                <li key={c.label} className={flex items-start gap-2 rounded-md border p-2 ${c.label === form.correctAnswer ? "border-emerald-300 bg-emerald-50" : "border-slate-200"} }>
