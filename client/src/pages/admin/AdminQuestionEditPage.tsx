      ...
      >
        <div className="grid grid-cols-2 gap-3">
          <Field label="Subject">
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, subjectArea: e.target.value as SubjectArea })
              }
          ...
          {form.type === "passage_based" && (
            <Field label="Passage">
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, passageId: e.target.value || null })
                }
                className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm"
                disabled={passagesLoading}
              >
                <option value="">
                  {passagesLoading ? "Loading passages..." : "None"}
                </option>
                {passages.map((p) => (
                ...
                {form.passageId &&
                  !passages.some((p) => p._id === form.passageId) && (
            ...
            <div key={c.label} className="flex items-center gap-2">
              ...
              <label className="inline-flex items-center gap-1 text-xs text-slate-600">
                <input
                  type="radio"
                  name="correct"
                  checked={form.correctAnswer === c.label}
        ...
        <Field label="Rich content blocks (JSON array)">
          <textarea
            ...
            onChange={(e) =>
              setForm({ ...form, contentBlocksJson: e.target.value })
            }
            rows={6}
            className="w-full rounded-md border border-slate-300 px-2 py-1.5 font-mono text-xs"
