    ...
    render: (r) => (
        <div className="flex justify-end gap-1">
            <Link to={`/admin/questions/${r._id}`}>
                <button type="button" onClick={() => setConfirmRow(r._id)} className="rounded-md border border-slate-200 px-2 py-1 text-xs text-primary-600 hover:bg-primary-50">Edit</button>
            </Link>
            {!r.isDeleted && (
                <button type="button" onClick={() => setConfirmRow(r._id)} className="rounded-md border border-slate-200 px-2 py-1 text-xs text-primary-600 hover:bg-primary-50">Delete</button>
            ))}
        </div>
    ),
},
...
return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
            <select required value={selectedSetId} onChange={(e) => { setSelectedSetId(e.target.value); setPage(1); }}>
