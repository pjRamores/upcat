...
export default function AdminQuestionsPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (requestId === latestRefreshRequestId.current) {
        const msg = (e as { response?: { error?: string } }).response?.data?.error;
      ...
      render: (r) => (
        <div className="min-w-0">
          <Link to={`/admin/questions/${r.id}`}>
            <span className="line-clamp-2 font-medium text-slate-800 hover:text-primary-700">
{
    key: "subjectArea",
    header: "Subject",
    sortable: true,
...
},
{
    ...
    header: "Flags",
    sortable: true,
    render: (r) => (r.flagCount > 0 ? <Badge variant="danger">{r.flagCount}</Badge> : <span className="text-xs text-slate-400">0</span>),
},
{
    key: "isDeleted",
    header: "Status",
...
},
{
    ...
    render: (r) => (
        <div className="flex justify-end gap-1">
            <Link to={`/admin/questions/${r.id}`} className="rounded-md border border-slate-200 px-2 py-1 text-xs hover:bg-slate-50">Edit</Link>
            {!r.isDeleted && (
                <button type="button" onClick={() => setConfirmRow(r._id)} className="rounded-md border border-primary-200 px-2 py-1 text-xs text-primary-600 hover:bg-primary-50">
                    Delete
                </button>
            )}
        </div>
...
return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
            <select required value={selectedSetId} onChange={(e) => {
...
<ConfirmDialog
    ...
    message={__("This will soft-delete ${selected.size} question(s). They can be restored from MongoDB.")}
    confirmText="DELETE"
    confirmLabel="Delete"
    variant="danger"
