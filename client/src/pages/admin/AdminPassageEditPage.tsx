...
export default function AdminPassageEditPage() {
    ...
    return (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <form className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-2" onSubmit={(e) => {
                e.preventDefault();
                save();
            }}>
                <Field label="Title">
                ...
                <Field label="Subject">
                    <select value={form.subjectArea} onChange={(e) => setForm({...form, subjectArea: e.target.value as SubjectArea})}
                            className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
        ...
        <ul className="space-y-2 text-sm">
            {linked.map((q) => (
                <li key={q.id} className="rounded-md border border-slate-200 p-2">
                    <Link to={`/admin/questions/${q.id}`} className="line-clamp-2 hover:text-primary-700">{q.questionText.slice(0, 120)}</Link>
                    <p className="mt-1 text-xs text-slate-500">{DIFFICULTY_LABELS[q.difficulty]} · {ans(q.correctAnswer)}</p>
                </li>
            ))}
        </ul>
    )}
