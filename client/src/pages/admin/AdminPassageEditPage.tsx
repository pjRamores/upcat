            ...
            <ul className="space-y-2 text-sm">
                {linked.map((q) => (
                    <li key={q._id} className="rounded-md border border-slate-200 p-2">
                        <Link to={`/admin/questions/${q._id}`} className="line-clamp-2 hover:text-primary-700">{q.questionText.slice(0, 120)}</Link>
                        <p className="mt-1 text-xs text-slate-500">{DIFFICULTY_LABELS[q.difficulty]} - {q.correctAnswer}</p>
                    </li>
                ))}
            </ul>
        )}
