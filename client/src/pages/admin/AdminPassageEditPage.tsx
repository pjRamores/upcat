...
export default function AdminPassageEditPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (isNew) {
        await adminApi.createPassage(body);
      } else {
        await adminApi.updatePassage(id!, body);
      }
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <form
        className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-2"
        onSubmit={(e) => {
          e.preventDefault();
          void save();
        }}
            ...
            {linked.map((q) => (
              <li key={q._id} className="rounded-md border border-slate-200 p-2">
                ...
                <p className="mt-1 text-xs text-slate-500">
                  {DIFFICULTY_LABELS[q.difficulty]} - {q.correctAnswer}
                </p>
              </li>
            ))}
          </ul>
