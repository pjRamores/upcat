      ...
      <nav className="flex gap-2 border-b border-slate-200">
        {(["overrides", "redirects", "sitemap"] as const).map((t) => (
          <button key={t} type="button" onClick={() => setTab(t)}>
            <span
              className={`mb-px border-b-2 px-3 py-2 text-sm font-medium capitalize ${tab === t ? "border-primary-600 text-primary-700" : "border-transparent text-slate-500 hover:text-slate-700"}`}
            >
              {t}
            </span>
          </button>
  ...
  const publishStaticOverrides = async () => {
    ...
    try {
      ...
      if (data?.payload) {
        ...
        addToast("success", `${data.contentSize} bytes. Save to client/public/data/seo-overrides.json`);
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="space-y-4">
        <section className="rounded-xl border border-amber-200 bg-amber-50 p-4">
            ...
            <p className="mt-2 text-xs text-amber-700">
                2. <strong>Next step:</strong> Save the downloaded JSON to{" "}
                        ...
                        <tr key={row.path} className="border-t border-slate-100">
                            ...
                            <td className="px-3 py-2">
                                {row.override?.noIndex ? (
                                    <span
                                        className="rounded bg-rose-100 px-2 py-0.5 text-xs font-semibold text-rose-700"
                                    >
                                        hidden
                                    </span>
                                ) : (
...
<div className="flex justify-end gap-2">
    <button type="button" onClick={onClose} className="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-medium hover:bg-slate-50">Cancel</button>
    <button type="button" disabled={saving} onClick={save} className="rounded-md bg-primary-600 px-4 py-1.5 text-sm font-semibold text-white hover:bg-primary-700 disabled:opacity-50">{saving ? "Saving..." : "Save override"}</button>
</div>
</div>
};
    ...
    return (
        <div className="space-y-4">
            <section className="space-y-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <h2 className="text-sm font-bold text-slate-700">{editingId ? "Edit redirect" : "Add redirect"}</h2>
...
<span className="rounded bg-slate-200 px-2 py-0.5 text-xs text-slate-600">off</span>
)}
</td>
<td className="px-3 py-2 text-right">
<button
    ...
    <section className="space-y-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <dl className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <Stat
          label="Sitemap URL"
          value={
            <a href={status.sitemapUrl} target="_blank" rel="noreferrer" className="text-primary-600 underline">
              {status.sitemapUrl}
            </a>
          }
        />
