    ...
    const columns: DataTableColumn<ExamRow>[] = [
        {
            ...
            header: "User",
            render: (r) => r.user
            ? <div><p className="font-medium">{r.user.firstName} {r.user.lastName}</p><p className="truncate text-xs text-slate-500">{r.user.email}</p></div>
        ...
        },
        {
            key: "status",
            header: "Status",
            render: (r) => <Badge variant={r.status === "completed" ? "success" : r.status === "in_progress" ? "info" : "warning"}>{r.status}</Badge>
        },
        {
            key: "startedAt",
            header: "Started",
            render: (r) => <span className="text-xs">{new Date(r.startedAt).toLocaleString()}</span>
        },
        {
            key: "completedAt",
            header: "Completed",
            render: (r) => <span className="text-xs">{r.completedAt ? new Date(r.completedAt).toLocaleString() : "-"}</span>
        },
        {
            key: "percentage",
            header: "Score",
            render: (r) => <span className="text-xs font-semibold">{r.percentage !== null ? `${r.percentage.toFixed(1)}%` : "-"}</span>
        },
        {
            key: "totalQuestions",
            header: "#Q",
...
<DataTable
    ...
    getRowId={(r) => r._id}
    isLoading={loading}
    onRowClick={(r) => navigate(`/admin/exams/${r.id}`)}
    page={page}
