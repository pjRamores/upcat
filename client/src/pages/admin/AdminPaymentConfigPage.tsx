    ...
    setManualConfig(prev) => {
        ...
        return {
            ...
            channels: prev.channels.map((channel) => {
                return channel.id === channelId ? { ...channel, ...patch } : channel;
            }),
        };
    });
};
...
const savePangConfig = async (e: React.FormEvent) => {
    ...
    try {
        const payload = {
            ...
            apiSecret: pangConfig.apiSecret.trim() || undefined,
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="block">
          ...
          <input
            ...
            onChange={(e) => setManualConfig(prev => ({
              ...prev,
              processingTimeMessage: e.target.value
            }))}
        ...
        <label className="block">
          ...
          <input
            ...
            onChange={(e) => setManualConfig(prev => ({
              ...prev,
              autoDisableThreshold: Number(e.target.value) || 0
            }))}
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
          />
        </label>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="block">
          ...
          <input
            ...
            onChange={(e) => setManualConfig(prev => ({
              ...prev,
              instructionsHeader: e.target.value
            }))}
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
          />
        </label>
        <label className="block">
          <span className="text-xs font-semibold text-slate-700">Instructions content</span>
          <textarea
            rows={4}
            value={manualConfig.instructionsContent}
            onChange={(e) => setManualConfig(prev => ({
              ...prev,
              instructionsContent: e.target.value
            }))}
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
          />
        </label>
...
<label className="block md:col-span-1">
  ...
  <textarea
    ...
    rows={3}
    className="mt-1 w-full rounded-md border-slate-300 px-3 py-2 text-sm"
  />
</label>
        ...
        <form onSubmit={savePangConfig} className="mt-4 space-y-4">
            <div className="grid gap-3 md:grid-cols-2">
                <label className="block">
                    ...
                    <input
                        type="text"
                        value={pangConfig.apiUrl}
                        onChange={(e) => setPangConfig(prev => prev ? { ...prev, apiUrl: e.target.value } : prev)}
                        className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                    />
                </label>
                ...
                <tbody>
                    {plans.map((p) => (
                        <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50">
                            ...
                            <td className="px-3 py-2 text-medium text-slate-900">{p.name}</td>
    ...
    <div className="mt-4 flex flex-wrap gap-2">
      {["manual", "pangmeryenda"] as const.map((mode) => (
        <button
          ...
          disabled={cursorNotAllowed} disabled:opacity-60
        >
...
<div className="flex justify-end gap-2 pt-4">
    <button type="button" onClick={closePlanModal}>
        <span className="rounded-md border border-slate-300 px-3 py-1.5 text-sm">Cancel</span>
    </button>
    <button type="submit" disabled={savingPlan}>
        <span className="rounded-md bg-primary-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-primary-700 disabled:opacity-60">{savingPlan ? "Saving..." : "Save plan"}</span>
    </button>
</div>
</form>
</Modal>
