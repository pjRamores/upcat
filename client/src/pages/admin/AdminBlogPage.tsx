...
import { BlogPostSummary, BlogStatus } from "@upcat/shared";
        ...
        <div className="space-y-4">
            ...
            {loading ? (
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
                    <table className="min-w-full text-sm uppercase text-slate-600">
                                ...
                                return (
                                    <tr key={p._id} className="border-t border-slate-100">
                                        ...
                                        <td className="px-3 py-2">
                                            {isPublished ? (
                                                <Link to={`/admin/blog/${p._id}`} className="text-slate-600 hover:text-slate-800">
                                                    View
                                                </Link>
                                            ) : (
                                                <button
...
<td className="px-3 py-2 text-right">
    ...
    <button type="button" onClick={() => remove(p.id, p.title)} className="ml-2 rounded-md border border-rose-300 px-2 py-1 text-xs font-medium text-rose-700 hover:bg-rose-50">
        Delete
    </button>
</td>
</tr>
...
<div className="flex items-center justify-between text-sm">
    {total.post(total === 1 ? "" : "s") + page + " of " + totalPages}
</div>
<div className="flex gap-1">
    <button type="button" disabled={page <= 1} onClick={() => setPage(page - 1)} className="rounded-md border border-slate-300 px-3 py-1 disabled:opacity-50">
        Prev
    </button>
    <button type="button" disabled={page >= totalPages} onClick={() => setPage(page + 1)} className="rounded-md border border-slate-300 px-3 py-1 disabled:opacity-50">
        Next
    </button>
</div>
</div>
