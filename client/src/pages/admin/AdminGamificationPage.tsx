 ...
 *
 * Tabs: Overview (stats), Achievements (list + activate/deactivate + re-seed), Challenges (list), Grant XP (form). Achievement editing uses the JSON condition format directly; full visual editor is out of scope for this phase.
 */
                ...
                .map(([k, label]) => (
                    <button
                        ...
                        className={`mb-px border-b-2 px-4 py-2 text-sm font-medium transition ${
                            tab === k ? "border-indigo-600 text-indigo-700" : "border-transparent text-slate-600 hover:text-slate-900"
                        }`}
                    >
                        {label}
                    </button>
...
function OverviewTab() {
    ...
    useEffect(() => {
        (async () => {
            try {
                setData(await adminGamificationApi.overview());
            } catch {
                addToast("error", "Failed to load overview.");
            } finally {
                setLoading(false);
            }
        })();
  ...
  return (
    <>
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm text-slate-600">{items.length} achievement{items.length === 1 ? "" : "s"}</p>
        <button
          type="button"
          onClick={reseed}
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <table className="min-w-full text-sm uppercase tracking-wide text-slate-500">
    ...
    <>
      <div className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-3.5">
        <p className="text-sm text-blue-900">
          <strong>Manual XP adjustment:</strong> Use this tool for one-off
          corrections (e.g., bug fixes, missed credits, support resolutions).
          All grants are logged in the activity audit trail with your admin ID,
          the amount, and the reason provided. Streak multipliers are not applied to manual grants.
        </p>
      </div>
