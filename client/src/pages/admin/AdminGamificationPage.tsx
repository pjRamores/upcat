...
function OverviewTab() {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <StatTile label="Total XP awarded" value={data.totalXpAwarded.toLocaleString()} />
      <StatTile label="Achievements (active)" value={data.achievementsCount.toLocaleString()} />
      <StatTile label="Weekly challenges" value={data.challengesCount.toLocaleString()} />
      <StatTile label="XP transactions" value={data.xpTransactions.toLocaleString()} />
    </div>
  );
}
                ...
                <tbody className="divide-y divide-slate-100">
                    {items.map((c) => (
                        <tr key={String(c.id)} className="hover:bg-slate-50">
                            ...
                            <td className="px-3 py-2 text-center">{c.isActive ? "✓" : "—"}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
