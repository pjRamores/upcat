...
return (
    <div className="space-y-4">
        <div className="flex flex-wrap gap-2">
            {(["open", "resolved", "dismissed", ""] as const).map((s) => (
                <button key={s} type="button" onClick={() => {
                    setStatus(s);
                    setPage();
                }} className={s === "All" ? "border-primary-600 bg-primary-600 text-white" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}>{s === "All" ? "All" : s.charAt(0).toUpperCase() + s.slice(1)}</button>
            ))}
        </div>
        ...
        <Modal isOpen={active !== null} onClose={() => {
            setActive(null);
            setResolutionNote("");
        }} title="Review flag" size="lg" footer={
            active && active.status === "open" ? (
                <div className="flex justify-end gap-2">
                    <button type="button" onClick={() => updateFlag(active._id, "dismissed")} className="rounded-md border border-slate-200 px-3 py-1.5 text-sm">Dismiss</button>
                    <button type="button" onClick={() => updateFlag(active._id, "resolved")} className="rounded-md bg-emerald-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-emerald-700">Mark resolved</button>
                </div>
            ) : (
                <button type="button" onClick={() => {
                    setActive(null);
            ...
            {active && (
                <div className="space-y-3 text-sm">
                    ...
                    <label className="block">
                        ...
                        <textarea value={resolutionNote} onChange={(e) => setResolutionNote(e.target.value)} rows={3} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
                    </label>
