...
/**
 * Admin UI for `PlatformSettings.ads` - the AdSense master config + the per-slot AdSense slot codes + the video interstitial campaign.
 */
export default function AdminAdsSettingsPage() {
  ...
  const updateAds = (patch: Partial<AdsSettings>) => {
    setSettings((s) => {
      if (!s || !s.ads) return s;
      return {...s, ads: {...s.ads, ...patch}};
    });
  };

  const updateSlot = (id: AdSlotId, key: "slot" | "format" | "enabled", value: string | boolean) => {
    setSettings((s) => {
      if (!s || !s.ads) return s;
  ...
  const updateVideo = (patch: Partial<AdsSettings["video"]>) => {
    setSettings((s) => {
      if (!s || !s.ads) return s;
      return {...s, ads: {...s.ads, video: {...s.ads.video, ...patch}}};
    });
  };
  const toggleTrigger = (t: VideoAdTrigger) => {
    setSettings((s) => {
      if (!s || !s.ads) return s;
...
Map each canonical slot id to its AdSense slot code (the value of <code className="ml-1 rounded bg-slate-100 px-1">data-ad-slot</code>). Enable a slot to render it. In test mode, enabled slots show placeholders even without a slot code.
</p>
<div className="space-y-3">
{AD_SLOT_IDS.map((id) => {
const conf = ads.slots[id] ?? {slot: "", enabled: false};
return (
<div key={id} className="grid grid-cols-1 gap-2 rounded-md border border-slate-200 p-3 md:grid-cols-4">
...
<input type="checkbox" checked={conf.enabled ?? Boolean(conf.slot)} onChange={(e) => updateSlot(id, "enabled", e.target.checked)} />
<span>Render this slot</span>
</label>
</Field>
<Field label="Slot code">
<input type="text" value={conf.slot} onChange={(e) => updateSlot(id, "slot", e.target.value.trim())} placeholder="1234567890" className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm font-mono" />
</Field>
<Field label="Format">
<select value={conf.format ?? "auto"} onChange={(e) => updateSlot(id, "format", e.target.value)} className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
...
<Toggle label="Video interstitials enabled" checked={ads.video.enabled} onChange={(v) => updateVideo({enabled: v})} />
<Field label="Video URL (MP4/WebM)">
<input type="url" value={ads.video.videoUrl} onChange={(e) => updateVideo({videoUrl: e.target.value})} className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
</Field>
<Field label="Poster URL (optional)">
<input type="url" value={ads.video.posterUrl ?? ""} onChange={(e) => updateVideo({posterUrl: e.target.value})} className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
</Field>
<Field label="Click-through URL (optional)">
<input type="url" value={ads.video.clickThroughUrl ?? ""} onChange={(e) => updateVideo({clickThroughUrl: e.target.value})} className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
</Field>
<div className="grid grid-cols-3 gap-3">
<Field label="Skip after (s)">
<input type="number" min={0} value={ads.video.skipAfterSeconds} onChange={(e) => updateVideo({skipAfterSeconds: Number(e.target.value)})} className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
</Field>
<Field label="Min interval (s)">
<input type="number" min={0} value={ads.video.minIntervalSeconds} onChange={(e) => updateVideo({minIntervalSeconds: Number(e.target.value)})} />
</Field>
</div>
</Section>
class Name="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm"
/>
</Field>
...
<div className="space-y-3">
{VIDEO_AD_TRIGGERS.map((t) => {
const conf = ads.video.triggerSettings?.[t] ?? {};
const effectiveSkip = conf.skipAfterSeconds ?? ads.video.skipAfterSeconds;
const effectiveInterval = conf.minIntervalSeconds ?? ads.video.minIntervalSeconds;
const effectiveCap = conf.frequencyCap ?? ads.video.frequencyCap;
return (
<div key={t} className="grid grid-cols-1 gap-2 rounded-md border border-slate-200 p-3 md:grid-cols-4">
...
>{saving ? "Saving..." : "Save ad settings"}</button>
</div>
...
Export the current public ad settings as a static snapshot. The client will load this file before hitting /api/ads/config.
</p>
{lastPublished && (
