    ...
    {data.topThreats.slice(0, 10).map((t, i) => (
        <li key={i} className="flex justify-between py-1.5">
            ...
            <span className="text-xs text-gray-500">
                score {String({ t as { threatScore?: number } }.threatScore ?? "?")}.{String({ t as { reputation?: string } }.reputation ?? "")}
            </span>
        </li>
    ))}
</ul>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-2 rounded bg-gray-50 p-3">
        ...
        <input
          ...
          className="rounded border border-gray-300 px-2 py-1 text-sm"
        />
        <button type="button" onClick={add} className="rounded bg-primary-600 px-3 py-1 text-sm text-white hover:bg-primary-700">
          Add block
        </button>
        <select
          value={filterActive}
                ...
                <td className="px-2 py-1 text-right">
                  {Boolean(b.isActive) && (
                    <button
                      type="button"
                      onClick={async () => {
                        await adminSecurityApi.removeBlock(b.id);
                        reload();
                      }}
...
function ConfigTab() {
    ...
    return (
        <div className="space-y-3">
            <LockdownControl/>
            <p className="text-sm text-gray-600">Edit the live security config. Saves require your admin password and take effect within 60 seconds.</p>
            <textarea
                value={json}
    ...
    return (
        <div className={enabled ? "rounded border border-red-300 bg-red-50 p-3" : "rounded border border-gray-200 bg-gray-50 p-3"}>
            <div className="flex flex-wrap items-center gap-2">
                <strong className="text-sm">Emergency lockdown: {enabled === null ? "..." : enabled ? "ENABLED" : "off"}</strong>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Admin password" className="ml-auto rounded border-gray-300 px-2 py-1 text-sm" />
                <button type="button" onClick={toggle} disabled={busy || !password || enabled === null} className={enabled ? "rounded bg-gray-700 px-3 py-1 text-sm text-white disabled:opacity-50" : "rounded bg-red-600 px-3 py-1 text-sm text-white disabled:opacity-50"}>{enabled ? "Disable" : "Enable lockdown"}</button>
            </div>
        </div>
    );
}
...
function ReportsTab() {
    ...
    return (
        <div className="space-y-4">
            <div className="flex gap-2">{[["24h", "7d", "30d"] as const].map((p) => (
                <button key={p} type="button" onClick={() => setPeriod(p)} className={period === p ? "rounded bg-primary-600 px-3 py-1 text-sm text-white" : "rounded border border-gray-300 px-3 py-1 text-sm"}>{p}</button>
            ))}
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                <Kpi label="Total events" value={data.totalEvents} />
                <Kpi label="New blocks" value={data.newBlocks} />
                <Kpi label="Critical" value={data.bySeverity.find((s) => s._id === "critical")?.count ?? 0} tone="danger" />
            </div>
        </div>
    );
}
    ...
    <ul className="space-y-1 text-sm">
      {rows.map((r) => (
        <li key={r.id} className="flex items-center gap-2">
          <span
            className={mono ? "w-40 truncate font-mono text-xs" : "w-40 truncate"}
            dangerouslySetInnerHTML={{ __html: r.id || "(unknown)" }}
          />
...
return (
    <div className="flex items-center gap-2 text-sm">
        <button type="button" disabled={page <= 1} onClick={() => onChange(page - 1)} className="rounded border border-gray-300 px-2 py-1 disabled:opacity-50">Prev</button>
        <span>{(page) / (total)}</span>
        <button type="button" disabled={page >= total} onClick={() => onChange(page + 1)} className="rounded border border-gray-300 px-2 py-1 disabled:opacity-50">Next</button>
    </div>
);