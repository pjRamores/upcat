...
import { AdminDashboard, adminSecurityApi } from "@/lib/securityApi";
        ...
        <nav className="mb-px flex flex-wrap gap-1">
          {TABS.map((t) => (
            <button
              ...
              type="button"
              onClick={() => setTab(t.id)}
              className={tab === t.id ? "border-b-2 border-blue-600 px-4 py-2 text-sm font-medium text-blue-700" : "border-b-2 border-transparent px-4 py-2 text-sm text-gray-600 hover:text-gray-900"}
            >
              {t.label}
            </button>
          ))}
            ...
            {data.recentEvents.slice(0, 10).map((e, i) => (
              <li key={i} className="py-1.5">
                <div className="flex justify-between">
                  ...
                  <span className="text-xs text-gray-500">{String((e as { severity?: string }).severity)}</span>
                </div>
                <div className="text-xs text-gray-500">
                  {String((e as { source?: { ip?: string } }).source?.ip ?? "")} · {new Date(String((e as { timestamp?: string }).timestamp)).toLocaleString()}
                </div>
              </li>
            ))}
          </ul>
    ...
    {data.topThreats.slice(0, 10).map((t, i) => (
        <li key={i} className="flex justify-between py-1.5">
            ...
            <span className="text-xs text-gray-500">
                score {String((t as { threatScore?: number }).threatScore ?? "?").toString()}
...
function EventsTab() {
    ...
    useEffect(() => {
        let cancelled = false;
        adminSecurityApi.listEvents({ ...filters, page, limit: 25 })
            .then((d) => !cancelled && setData(d))
            .catch((e) => !cancelled && setError((e as Error).message));
        return () => {
            cancelled = true;
                ...
                <>
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {data.items.map((e, i) => (
                                <EventRow key={i} ev={e} onChange={() => setFilters({ ...filters })} />
                            ))}
                        </tbody>
                    </table>
  ...
  return (
    <>
      <tr className="hover:bg-gray-50">
        <td className="px-2 py-1 text-xs">{new Date(String(ev.timestamp)).toLocaleString()}</td>
      ...
      {open && (
        <tr className="bg-gray-50">
          <td colSpan={6} className="px-2 py-2">
            <pre className="max-h-64 overflow-auto rounded bg-white p-2 text-xs">{JSON.stringify(ev, null, 2)}</pre>
          </td>
          {!ev.reviewed && (
            <div className="mt-2 flex gap-2">
              <button type="button" disabled={busy} onClick={() => review("dismiss")} className="rounded border-gray-300 px-3 py-1 text-xs">
                Dismiss
              </button>
              {src.ip && (
                <button type="button" disabled={busy} onClick={() => review("block_ip")} className="rounded border border-red-300 px-3 py-1 text-xs text-red-700">
                  Block IP
                </button>
              )}
            </div>
...
function BlockedTab() {
    ...
    const [form, setForm] = useState({ type: "", value: "", reason: "" });
                                ...
                                <td className="px-2 py-1 text-right">
                                    {Boolean(b.isActive) && (
                                        <button
                                            type="button"
                                            onClick={() => {
                                                void adminSecurityApi.deleteBlock(b.id);
                                                void reload();
                                            }}
    ...
    return (
        <div className="space-y-3">
            ...
            <p className="text-sm text-gray-600">
                Edit the live security config. Saves require your admin password and take effect within 60 seconds.
            </p>
            <textarea
                value={json}
...
function LockdownControl() {
    ...
    useEffect(() => {
        adminSecurityApi
            .getConfig()
            .then((c) => setEnabled(Boolean({ c: { lockdown?: { enabled?: boolean } } }.lockdown?.enabled)))
            .catch(() => undefined);
    }, []);

    async function toggle() {
        if (!password) return;
        if (!confirm(enabled ? "Disable lockdown?" : "ENABLE LOCKDOWN? Non-admin traffic will be blocked."))
            return;
        setBusy(true);
        try {
    ...
    return (
        <div>
            <div className={enabled ? "rounded-border-border-red-300-bg-red-50-p-3" : "rounded-border-border-gray-200-bg-gray-50-p-3">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <input
                        ...
                        onChange={(e) => setPassword(e.target.value)}
                        placeholder="Admin password"
                        className="ml-auto-rounded-border-gray-300-px-2-py-1-text-sm"
                    />
                    <button
                        type="button"
                        onClick={toggle}
                        disabled={busy || !password || enabled === null}
                        className={enabled ? "rounded-bg-gray-700-px-3-py-1-text-sm-text-white-disabled:opacity-50" : "rounded-bg-red-600-px-3-py-1-text-sm-text-white-disabled:opacity-50"}
                    >
            ...
            <div className="flex gap-2">
                {[["24h", "7d", "30d"] as const].map((p) => (
                    <button
                        ...
                        type="button"
                        onClick={() => setPeriod(p)}
                        className={period === p ? "rounded-bg-primary-600-px-3-py-1-text-sm-text-white" : "rounded-border-border-gray-300-px-3-py-1-text-sm"}
                    >
                        {p}
                    </button>
                ))}
...
<div>
    ...
    <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card title="Top_countries">
            <BarList rows={data.topCountries} />
        </Card>
    </div>
    <Card title="Top_source_IPs">
        <BarList rows={data.topIps} mono />
    </Card>
</div>
}

function BarList({ rows, mono = false, }) {
    const max = useMemo(() => Math.max(...rows.map((r) => r.count)), [rows]);
    return (
        <ul className="space-y-1 text-sm">
            {rows.map((r) => (
                <li key={r.id} className="flex items-center gap-2">
                    <span className={mono ? "w-40 truncate font-mono text-xs" : "w-40 truncate"}>{r.id || "unknown"}</span>
                    <div className="relative h-4 flex-1 rounded bg-gray-100">
                        <div className="absolute inset-y-0 left-0 rounded bg-primary-500" style={{ width: `${(r.count / max) * 100}%` }} />
                    </div>
...
function Kpi({ label, value, tone = "ok", }) {
    const cls = tone === "danger" ? "bg-red-50 border-red-200 text-red-700" : tone === "warn" ? "bg-amber-50 border-amber-200 text-amber-700" : "bg-white border-gray-200 text-gray-800";
    return (
