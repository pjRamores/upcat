...
export default function AdminHelpArticlesPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-bold text-slate-900">Help Article Manager</h1>
...
<div className="mt-3 grid gap-3 md:grid-cols-2">
    <input className="rounded border border-slate-300 px-3 py-2 text-sm" placeholder="Slug" value={String(form.slug ?? "")} onChange={(e) => setForm((prev) => ({...prev, slug: e.target.value})))} disabled={!isCreate}/>
    <input className="rounded border border-slate-300 px-3 py-2 text-sm" placeholder="Title" value={String(form.title ?? "")} onChange={(e) => setForm((prev) => ({...prev, title: e.target.value}))}/>
    <input className="rounded border border-slate-300 px-3 py-2 text-sm" placeholder="Subtitle" value={String(form.subtitle ?? "")} onChange={(e) => setForm((prev) => ({...prev, subtitle: e.target.value}))}/>
    <select className="rounded border border-slate-300 px-3 py-2 text-sm" value={String(form.category ?? "getting-started")} onChange={(e) => setForm((prev) => ({...prev, category: e.target.value}))}>
    ...
    <select className="rounded border border-slate-300 px-3 py-2 text-sm" value={String(form.status ?? "draft")} onChange={(e) => setForm((prev) => ({...prev, status: e.target.value}))}>
        <option value="draft">draft</option>
        <option value="published">published</option>
        <option value="archived">archived</option>
    </select>
    <input className="rounded border border-slate-300 px-3 py-2 text-sm" type="number" value={Number(form.order ?? 100)} onChange={(e) => setForm((prev) => ({...prev, order: Number(e.target.value})))} />
</div>

<div className="mt-3 grid gap-3 lg:grid-cols-2">
    <textarea className="min-h-[260px] rounded border border-slate-300 px-3 py-2 font-mono text-xs" value={String((form.content as { body?: string } | undefined)?.body ?? "")} onChange={(e) => setForm((prev) => ({...prev, content: {format: "markdown", body: e.target.value}, }))}
    />
    <div className="rounded border border-slate-200 bg-slate-50 p-3">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Metadata JSON</p>
        <textarea className="mt-2 min-h-[220px] w-full rounded border border-slate-300 px-3 py-2 font-mono text-xs" value={JSON.stringify({
...
<div className="mt-3 flex flex-wrap gap-2">
    <button type="button" className="rounded bg-primary-600 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-700" onClick={async () => {
        try {
            if (isCreate) {
    ...
    {!isCreate && (
        <button type="button" className="rounded border border-slate-300 px-4 py-2 text-sm" onClick={() => setEditingSlug(null)}>
            Cancel Edit
    </button>
    )}
</div>
...
(String(form.slug || "").trim() && (
    <a href={`/help/article/${String(form.slug)}`} target="blank" rel="noreferrer"
...
<section className="rounded-xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
    <div className="flex items-start justify-between">
        <div>
            <h2 className="text-sm font-semibold text-amber-900">Publish Static Content</h2>
            <p className="mt-1 text-xs text-amber-800">
                Export all published articles as static JSON to be bundled with the app. This enables
                instant page loads
                without API latency.
            </p>
            {lastPublished && (
    ...
    <p className="mt-3 text-xs text-amber-700">
        <strong>Next step:</strong> Save the downloaded JSON to <code className="bg-white px-1 py-0.5 rounded">client/public/data/help-content.json</code>
