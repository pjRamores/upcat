...
export default function AdminQuestionImportExportPage() {
  ...
  const { setSelectedId, setSelectedSetId } = useSetFilter();
...
const downloadFile = async (kind: "json" | "csv") => {
  ...
  try {
    ...
    const filename = kind === "csv" ? `questions-export-set-${setRef}-${Date.now()}.csv` : `questions-export-set-${setRef}-${Date.now()}.json`;
...
const downloadJson = () => void downloadFile("json");

const downloadCsv = () => void downloadFile("csv");
...
return (
  <div className="space-y-6">
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="text-lg font-semibold text-slate-900">Question Set</h2>
      <p className="mt-1 text-sm text-slate-600">
        This set applies to both import and export. Imported questions will be assigned to the selected set.
      </p>
      <label className="mt-4 block text-sm text-slate-700">
        Selected set
        <select required value={selectedSetId} onChange={(e) => setSelectedSetId(e.target.value)} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
          {setOptions.length === 0 ? (
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="text-lg font-semibold text-slate-900">Import Questions With Dedup</h2>
      <p className="mt-1 text-sm text-slate-600">
        Paste JSON array or CSV content, preview duplicate matches, then confirm with a dedup strategy. Imported rows are assigned to the selected set above.
      </p>

      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <label className="text-sm text-slate-700">
          Format
          <select value={format} onChange={(e) => setFormat(e.target.value as "json" | "csv")} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
        ...
        <label className="text-sm text-slate-700">
          Confirm mode
          <select value={mode} onChange={(e) => setMode(e.target.value as "skip_duplicates" | "insert_all" | "replace_exact")} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
      ...
      <textarea value={rawData} onChange={(e) => setRawData(e.target.value)} rows={14} className="mt-3 w-full rounded-md border border-slate-300 px-3 py-2 font-mono text-xs" placeholder={format === "json" ? '[...]' : 'or {"passages": [{"id":"<ObjectId>", "title":"...", "subjectArea":"...", "source":"...", "content":"..."}], "questions":[{"...}]}'}></textarea>

      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" onClick={runPreview} disabled={loadingPreview} className="rounded-md bg-primary-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-primary-700 disabled:opacity-50">{loadingPreview ? "Previewing..." : "Preview Import"}</button>
        <button type="button" onClick={runConfirm} disabled={loadingConfirm || !preview} className="rounded-md border border-emerald-300 px-3 py-1.5 text-sm font-semibold text-emerald-700 hover:bg-emerald-50 disabled:opacity-50">{loadingConfirm ? "Confirming..." : "Confirm Import"}</button>
        <button type="button" onClick={runUndo} disabled={!lastBatchId} className="rounded-md border border-primary-300 px-3 py-1.5 text-sm font-semibold text-primary-700 hover:bg-primary-50 disabled:opacity-50">Undo Last Batch</button>
      </div>
    </section>
  </div>
)
...
<section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
  ...
  <p className="mt-1 text-sm text-slate-600">Filter the selected set, review dataset size, then export JSON or CSV.</p>
  <div className="mt-4 grid gap-3 md:grid-cols-3">
    <label className="text-sm text-slate-700">
      Search text
      <input type="text" value={exportFilters.search} onChange={(e) => setExportFilters((prev) => ({...prev, search: e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" placeholder="Question text contains..." />
    </label>
    <label className="text-sm text-slate-700">
      Subtest (Subject)
      <select value={exportFilters.subjectArea} onChange={(e) => setExportFilters((prev) => ({...prev, subjectArea: e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
    ...
    <label className="text-sm text-slate-700">
      Subtopic
      <input type="text" value={exportFilters.subtopic} onChange={(e) => setExportFilters((prev) => ({...prev, subtopic: e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" placeholder="Exact subtopic" />
    </label>
    <label className="text-sm text-slate-700">
      Topic tags (comma-separated)
      <input type="text" value={exportFilters.topic} onChange={(e) => setExportFilters((prev) => ({...prev, topic: e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" placeholder="grammar, vocabulary" />
    </label>
    <label className="text-sm text-slate-700">
      Difficulty
      <select value={exportFilters.difficulty} onChange={(e) => setExportFilters((prev) => ({...prev, difficulty: e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
        <option value="">All</option>
        {DIFFICULTIES.map((d) => (
    ...
    <label className="text-sm text-slate-700">
      Type
      <select value={exportFilters.type} onChange={(e) => setExportFilters((prev) => ({...prev, type: e.target.value}))}>
class Name="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm"
>
    <option value="">All</option>
...
{loadingDatasetInfo?.(
    ...
    <>
        ...
        <p className="mt-1">{datasetInfo?.total ?? 0}</p>
...
<div className="mt-3 grid gap-3 md:grid-cols-2">
    <div>
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Top subjects</p>
            ...
            {topSubjects.length === 0 ? (
                ...
                topSubjects.map(([subject, count]) => (
                    <li key={subject} className="flex items-center justify-between gap-2">
                        ...
                        <span className="rounded bg-slate-200 px-1.5 py-0.5 font-medium text-slate-700">{count}</span>
                    </li>
                ))
            )}
        </ul>
    ...
    <div>
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Top difficulties</p>
        <ul className="mt-1 space-y-1 text-xs">
            {topDifficulties.length === 0 ? (
                <li className="text-slate-500">No matching records</li>
            ) : (
                topDifficulties.map(([difficulty, count]) => (
                    <li key={difficulty} className="flex items-center justify-between gap-2">
                        <span className="truncate text-slate-700">{DIFFICULTY_LABELS[difficulty as keyof typeof DIFFICULTY_LABELS] ?? difficulty}</span>
                        <span className="rounded bg-slate-200 px-1.5 py-0.5 font-medium text-slate-700">{count}</span>
                    </li>
                ))
            )}
...
<div className="mt-4 flex gap-2">
    ...
    >
        {loadingExport === "json" ? "Exporting JSON..." : "Export JSON"}
    </button>
    <button
        type="button"
        onClick={downloadCsv}
        ...
        className="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50"
    >
        {loadingExport === "csv" ? "Exporting CSV..." : "Export CSV"}
    </button>
</div>
</section>
);