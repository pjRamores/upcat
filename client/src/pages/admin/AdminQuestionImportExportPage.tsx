...
import { DIFFICULTIES, DIFFICULTY_LABELS, SUBJECT.Areas } from "@upcat/shared";
...
export default function AdminQuestionImportExportPage() {
  ...
  const topSubjects = useMemo(() => Object.entries(datasetInfo?.bySubject ?? {}).sort((a, b) => b[1] - a[1]).slice(0, 5), [datasetInfo?.bySubject]);
  const topDifficulties = useMemo(() => Object.entries(datasetInfo?.byDifficulty ?? {}).sort((a, b) => b[1] - a[1]).slice(0, 5), [datasetInfo?.byDifficulty]);
...
useEffect(() => {
    ...
    const loadDatasetInfo = async () => {
        ...
        try {
            ...
            setDatasetInfo({
                total: result.total,
                bySubject: (result as unknown as Record<string, unknown>).filterCounts?.bySubject ?? {},
                byDifficulty: (result as unknown as Record<string, unknown>).filterCounts?.byDifficulty ?? {},
            });
        } catch {
...
<select
value={mode}
onChange={(e) => setMode(e.target.value as "skip_duplicates" | "insert_all" | "replace_exact")}
className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm"
>
...
placeholder={format === "json" ? `{...}` : `{"passages":[{"_id":"<ObjectId>","title":"...","subjectArea":"...","source":"...","content":"..."},{"...}]}`}
/>
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-3">
      ...
      <label className="text-sm text-slate-700">
        ...
        >
          <option value="">All</option>
          {TYPES.map((t) => (
            <option key={t} value={t}>{TYPES_LABELS[t]}</option>
          ))}
        </select>
      </label>
    </div>
