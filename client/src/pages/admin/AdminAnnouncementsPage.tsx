    ...
    <div className="space-y-4">
        ...
        <div className="flex justify-end">
            <button type="button" onClick={() => setEditing({form: EMPTY})}>
                <span className="rounded-md bg-primary-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-primary-700">New Announcement</span>
            </button>
        </div>
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-3">
                            <div className="min-w-0">
                                ...
                                <div className="mt-2 flex flex-wrap gap-2 text-xs">
                                    <Badge variant={a.type === "maintenance" ? "danger" : a.type === "warning" ? "warning" : "info"}>{a.type}</Badge>
                                    <Badge variant={a.isActive ? "success" : "neutral"}>{a.isActive ? "Active" : "Hidden"}</Badge>
                                    {a.startsAt && <span className="text-slate-500">From {new Date(a.startsAt).toLocaleString()}</span>}
                                    {a.expiresAt && <span className="text-slate-500">Until {new Date(a.expiresAt).toLocaleString()}</span>}
                                </div>
                            </div>
                            <div className="flex gap-2">
                                <button type="button" onClick={() => setEditing({
                                    ...
                                    <span className="rounded-md border border-slate-200 px-2 py-1 text-xs hover:bg-slate-50">Edit</span>
                                </button>
                                <button type="button" onClick={() => setConfirmDel(a._id)}>
                                    <span className="rounded-md border border-primary-200 px-2 py-1 text-xs text-primary-600 hover:bg-primary-50">Delete</span>
                                </button>
                            </div>
                        </div>
                    </li>
        ...
        <Modal isOpen={editing !== null} onClose={() => setEditing(null)} title={editing?.id ? "Edit announcement" : "New announcement"} size="lg">
            <form onSubmit={handleSubmit}>
                <div className="space-y-4">
                    <div className="flex justify-between gap-2">
                        <input type="text" placeholder="Title" value={editing?.title || ""} onChange={(e) => setEditing({ ...editing, title: e.target.value })} />
                        <input type="text" placeholder="Message" value={editing?.message || ""} onChange={(e) => setEditing({ ...editing, message: e.target.value })} />
                    </div>
                    <div className="flex justify-between gap-2">
                        <input type="date" placeholder="Starts At" value={editing?.startsAt ? new Date(editing.startsAt).toISOString().split('T')[0] : ""} onChange={(e) => setEditing({ ...editing, startsAt: e.target.value })} />
                        <input type="date" placeholder="Expires At" value={editing?.expiresAt ? new Date(editing.expiresAt).toISOString().split('T')[0] : ""} onChange={(e) => setEditing({ ...editing, expiresAt: e.target.value })} />
                    </div>
                </div>
            </form>
        </Modal>
...
<div className="space-y-3 text-sm">
    ...
    <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Expires at (optional)">
            <input type="datetime-local" value={editing.form.expiresAt} onChange={(e) => setEditing({
                ...editing,
