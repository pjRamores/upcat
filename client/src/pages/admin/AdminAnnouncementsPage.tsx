...
export default function AdminAnnouncementsPage() {
    ...
    const refresh = async () => {
        ...
        try {
            ...
            const msg = (e as { response?: { data?: string } }).response?.data?.error;
...
return (
    <div className="space-y-4">
                    ...
                    <li key={a._id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <div className="flex gap-2">
                                ...
                                className="rounded-md border border-slate-200 px-2 py-1 text-xs hover:bg-slate-50">Edit
                                </button>
        ...
        <Modal isOpen={editing !== null} onClose={() => setEditing(null)} title={editing?.id ? "Edit announcement" : "New announcement"} size="lg" footer={
            <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setEditing(null)} className="rounded-md border border-slate-200 px-3 py-1.5 text-sm">Cancel</button>
                <button type="button" onClick={save} className="rounded-md bg-primary-600 px-4 py-4 text-sm font-semibold text-white hover:bg-primary-700">Save</button>
            </div>
        }>
        </Modal>
    </div>
