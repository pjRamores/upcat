      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Panel title="Oldest open ticket">
          {data.oldestOpenTicket ? (
            <Link
              to={`/admin/support/tickets/${data.oldestOpenTicket.ticketNumber}`}
              className="block rounded-md bg-amber-50 p-3 text-sm hover:bg-amber-100"
            >
              <p className="font-mono text-xs text-amber-700">
                {data.oldestOpenTicket.ticketNumber}
              </p>
              <p className="font-semibold text-amber-900">
                {data.oldestOpenTicket.subject}
              </p>
              <p className="mt-1 text-xs text-amber-700">
                Opened {new Date(data.oldestOpenTicket.createdAt).toLocaleDateString()}
              </p>
            </Link>
          ) : (
          ...
          <ul className="space-y-1 text-sm">
            {Object.entries(data.byType).map(([t, n]) => (
              <li key={t} className="flex justify-between">
                <span
                  className={`icon ${SUPPORT_TICKET_TYPE_META[t as keyof typeof SUPPORT_TICKET_TYPE_META]?.icon}`}
                >
                  {SUPPORT_TICKET_TYPE_META[t as keyof typeof SUPPORT_TICKET_TYPE_META]?.label ?? t}
                </span>
        ...
        <Panel title="By priority">
          <ul className="space-y-1 text-sm">
            {Object.entries(data.byPriority).map(([p, n]) => {
              const meta =
                SUPPORT_TICKET_PRIORITY_META[p as keyof typeof SUPPORT_TICKET_PRIORITY_META];
              return (
                <li key={p} className="flex justify-between">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-semibold bg-${meta?.color}-50 text-${meta?.color}-700`}
                  >
                    {meta?.label ?? p}
</span>
            <span className="font-mono text-gray-700">{n}</span>
...
<span className="font-mono text-xs text-gray-500">{t.subject}</span>
</div>
<p className="mt-0.5 text-xs text-gray-500">{SUPPORT_TICKET_TYPE_META[t.type]?.label} · {new Date(t.createdAt).toLocaleString()}</p>
<span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-semibold bg-${status.color}-50 text-${status.color}-700`}>{status.label}</span>
</Link>
</li>
)}
</ul>
