 ...
 *
 * Lands here after the IdP redirects the browser back. Reads 'code' and 'state' from the URL, posts to the backend callback, then either:
 * - login: stores the returned JWT and redirects to /dashboard (or to the redirectPath captured at start);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const msg = (err as { response?: { error?: string } }).response?.data?.error || "Sign-in failed.";
        setError(msg);
      }
    })();
