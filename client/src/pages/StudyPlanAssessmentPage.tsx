  ...
  if (result) {
    ...
    return (
      <div className="mx-auto max-w-4xl space-y-5 px-4 py-8">
        ...
        <section
          className={`rounded-2xl border p-6 ${passed ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}
        >
          <h1 className="text-2xl font-bold">{passed ? "You Passed!" : "Not quite there yet"}</h1>
          <p className="mt-2 text-sm">
            Score: {(result as any).score?.percentage}%<br/>{((result as any).score?.correct) / ((result as any).score?.total)}
          </p>
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4 px-4 py-8">
      ...
      <header className="rounded-xl border border-slate-200 bg-white p-5">
        <h1 className="text-2xl font-bold text-slate-900">Test Your Knowledge</h1>
