    ...
    if (result) {
        ...
        return (
            <div className="mx-auto max-w-4xl space-y-5 px-4 py-8">
                ...
                <section className={`rounded-2xl border-p-6 ${passed ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}>
                    <h1 className="text-xl font-bold">{passed ? "You Passed!" : "Not quite there yet"}</h1>
                    <p className="mt-2 text-sm">
                        Score: {(result as any).score?.percentage}%
                    </p>
                    <p className="mt-2 text-sm">{(result as any).score?.correct}/{(result as any).score?.total}</p>
                    <p className="mt-2 text-sm">{(result as any).feedback?.recommendation}</p>
                    <div className="mt-4 flex gap-2">
                        <button type="button" onClick={() => void studyPlanApi.getAssessmentReview(assessmentSessionId).then((review) => console.log(review))} className="rounded-lg border border-slate-300 px-3 py-2 text-sm">
                            Review Answers
                        </button>
                        <button type="button" onClick={() => navigate("/study-plan")} className="rounded-lg bg-sky-600 px-3 py-2 text-sm font-semibold text-white">
                            Continue
                        </button>
                    </div>
                </section>
    ...
    return (
        <div className="mx-auto max-w-4xl space-y-4 px-4 py-8">
            ...
            <header className="rounded-xl border border-slate-200 bg-white p-5">
                <h1 className="text-xl font-bold text-slate-900">Test Your Knowledge</h1>
                ...
                <article key={q._id} className="rounded-xl border border-slate-200 bg-white p-4">
                    ...
                    <div className="mt-3 space-y-2">
                        {(q.choices ?? []).map((choice: any) => (
                            <label key={choice.label} className="flex cursor-pointer items-center gap-2 rounded border border-slate-200 p-2 hover:bg-slate-50">
                                <input type="radio" name={q._id} value={choice.label} />
...
/>
<span className="text-sm">{choice.label}. {choice.text}</span>
</label>
</div>
</article>
))}
