...
function deriveSpentByBatchFromElapsedSeconds(
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    const used = Math.min(allotted, Math.max(0, elapsedSeconds));
    spent[i] = used;
    elapsedSeconds -= used;
  }
  return spent;
...
useEffect(() => {
  ...
  // with an elapsed-time derivation. The stored value is the frozen source of
  // truth for paused sessions and prevents a React same-commit race where this
    ...
    return (
        <div className="mx-auto max-w-xl px-4 py-16 text-center">
            ...
            <button type="button" onClick={() => navigate("/dashboard")}>
                <span className="mt-6 rounded-md bg-primary-600 px-5 py-2 text-sm font-semibold text-white hover:bg-primary-700">
                    Back to Dashboard
                </span>
            </button>
        </div>
...
const timeoutSubjectLabel = timeoutSubject ? (SUBJECT_META[timeoutSubject.subject]?.label ?? timeoutSubject.subject) : "this subject";
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
                    ...
                    <p className="mt-2 text-sm text-slate-600">
                        {timeoutTargetBatch !== null ? `The allotted time for ${timeoutSubjectLabel} has ended.` : "The allotted time for this subject has ended."}
                    </p>
                    <button
                        type="button"
                        onClick={() => {
...
<div className="grid-gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
  <section>
    ...
    <div className={`relative rounded-2xl border border-slate-200 bg-white p-6 shadow-sm`}>
      {isPaused && (
        <div className="absolute inset-0 z-10 flex items-center justify-center rounded-2xl backdrop-blur-sm bg-white/60">
...
<div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-slate-500">
  <span className="flex items-center gap-1">
    ...
    <span className="inline-block h-3 w-3 rounded-sm bg-emerald-100 ring-1 ring-emerald-300">Answered</span>
    <span className="inline-block h-3 w-3 rounded-sm bg-sky-100 ring-1 ring-sky-300">Visited</span>
    <span className="inline-block h-3 w-3 rounded-sm bg-slate-100 ring-1 ring-slate-300">Not visited</span>
...
<div className="mt-4 space-y-2">
  ...
  <button
    ...
    disabled={pauseInFlight}
    className="w-full rounded-md bg-slate-700 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 disabled:cursor-not-allowed"
    >Exit</button
  >
  {!isLastBatch && (
    <button
      ...
      >Proceed to {SUBJECT_META[nextBatch?.subject ?? currentBatch.subject]?.label}</button
    >
  )}
  <button
    type="button"
    ...
    >{submitting || submitBlocking ? "Submitting..." : "Submit Now"}</button
  >
</div>
        ...
        <div>
            <div className="mb-2 flex items-start justify-between">
                ...
                <button
                    ...
                    title={state.flagged ? "Mark for review" : "Mark for review"}
                    className={`
                        ml-2 shrink-0 rounded p-1 transition disabled:cursor-not-allowed disabled:opacity-50 ${
                ...
                >
                    {state.flagged ? (
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5">
                            <path d="M17.593 3.322c1.128 1.907 1.077 1.907 2.185v21L12 17.25 4.5 21V5.507c0-1.108-0.806-2.057-1.907-2.185a48.507 48.507 0 0 1 0.1 11.186 02"/>
                        </svg>
                    ) : (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-5 w-5">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M17.593 3.322c1.128 1.907 1.077 1.907 2.185v21L12 17.25 4.5 21V5.507c0-1.108-0.806-2.057-1.907-2.185a48.507 48.507 0 0 1 0.1 11.186 02"/>
                        </svg>
                    )}
                </button>
            </div>
                    ...
                    return (
                        <button
                            ...
                            className={`block w-full rounded-md border px-4 py-2 text-left text-sm transition ${
                                selected ? "border-primary-500 bg-primary-50 ring-2 ring-primary-200" : "border-slate-200 bg-white hover:border-slate-300"
                            }`}
                        >
