...
export default function StudyPlanSetupPage() {
  ...
  const [selfAssessment, setSelfAssessment] = useState<Record<SubjectArea, "beginner" | "intermediate" | "advanced">>(
    Object.fromEntries(SUBJECT_AREAS.map((s) => [s, "beginner"]))
  );
  ...
  const toggleStudyDay = (day: number) => {
    setParameters(prev => ({
      ...prev,
      studyDaysPerWeek: prev.studyDaysPerWeek.includes(day)
        ? prev.studyDaysPerWeek.filter(d => d !== day).concat([...prev.studyDaysPerWeek, day])
        : [...prev.studyDaysPerWeek.sort((a, b) => a - b)]
    }));
  };
    ...
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-8">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-3">
          {["none", "historical", "self_assessment"].map((opt) => (
            <button
              key={opt.value}
              type="button"
...
Target exam date
<input type="date" className="mt-1 w-full rounded border border-slate-300 px-2 py-1" onChange={(e) => {
...
<input type="range" min={0.5} max={6} step={0.5} value={parameters.availableHoursPerDay} onChange={(e) => setParameters((prev) => ({...prev, availableHoursPerDay: Number(e.target.value)}))} className="mt-2 w-full"/>
</label>
</div>
<div className="mt-4">
<p className="text-sm font-medium">Study days</p>
<div className="mt-2 flex flex-wrap gap-2">{[1, 2, 3, 4, 5, 6, 7].map((day) => (
<button key={day} type="button" onClick={() => toggleStudyDay(day)} className={`rounded-lg border px-3 py-1 text-sm ${parameters.studyDaysPerWeek.includes(day) ? "border-emerald-500 bg-emerald-50" : "border-slate-300"}`}>{day === 1 ? "Mon" : day === 2 ? "Tue" : day === 3 ? "Wed" : day === 4 ? "Thu" : day === 5 ? "Fri" : day === 6 ? "Sat" : "Sun"}</button>
))}
</div>
</div>
</section>
...
<select className="rounded border border-slate-300 px-2 py-2 text-sm" value={parameters.learningStyle} onChange={(e) => setParameters((prev) => ({...prev, learningStyle: e.target.value as any}))}>
<option value="mixed">Mixed</option>
<option value="visual">Visual</option>
<option value="reading">Reading</option>
<option value="practice">Practice-heavy</option>
</select>
<select className="rounded border border-slate-300 px-2 py-2 text-sm" value={parameters.difficultyPreference} onChange={(e) => setParameters((prev) => ({...prev, difficultyPreference: e.target.value as any}))}>
<option value="gradual">Gradual</option>
<option value="balanced">Balanced</option>
<option value="aggressive">Aggressive</option>
</select>
<select className="rounded border border-slate-300 px-2 py-2 text-sm" value={templateId ?? ""} onChange={(e) => setTemplateId(e.target.value)}>
{templates.map((t) => (
