    ...
    if (!plan) {
        return (
            <div className="mx-auto max-w-5xl px-4 py-10">
                ...
                <section className="rounded-3xl border border-sky-100 bg-[radial-gradient(circle_at_top_left,_#f0f9ff,_#e0f2fe_35%,_#f8fafc_80%)] p-8" data-help="sp_diagnostic">
                    ...
                    <div className="mt-7">
                        <Link to="/study-plan/setup" className="inline-flex items-center rounded-xl bg-sky-600 px-5 py-3 font-semibold text-white hover:bg-sky-700">
                            Get Started
                        </Link>
                    </div>
                </section>
        ...
        <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" data-help="sp_on_track">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    {todaySession && (
                        <Link to={`/study-plan/session/${todaySession.session.id}?plan=${plan.id}`} className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700">
            ...
            <section className="grid gap-4 md:grid-cols-3">
                <Card title="Progress" value={`${plan.progress.overallProgress}%`}>
                    ...
                    <p className="text-sm text-slate-600">You have completed {plan.progress.completedSessions} out of {plan.progress.totalSessions} sessions.</p>
                </Card>
                <Card title="Modules" value={`${plan.progress.modulesCompleted}%`}>
                    <p className="text-sm font-semibold text-slate-900">Modules Completed</p>
                    <p className="text-sm text-slate-600">You have completed {plan.progress.completedModules} out of {plan.progress.totalModules} modules.</p>
                </Card>
                <Card title="Phases" value={`${plan.progress.phasesCompleted}%`}>
                    <p className="text-sm font-semibold text-slate-900">Phases Completed</p>
                    <p className="text-sm text-slate-600">You have completed {plan.progress.completedPhases} out of {plan.progress.totalPhases} phases.</p>
                </Card>
            </section>
        </div>
    );
...
function Step({title, body}: {title: string; body: string}) {
    return (
        <div className="rounded-xl.border.border-sky-200.bg-white.p-4">
            ...
            <p className="mt-2.text-sm.font-bold.text-slate-900">{body}</p>
        </div>
    );
}
