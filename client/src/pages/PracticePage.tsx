...
export default function PracticePage() {
  ...
  const [subjectArea, setSubjectArea] = useState<SubjectArea>(SUBJECT_AREAS);
  ...
  const canStart = useMemo(() => {
    ...
    if (mode === "random") return totalDeck > 0;
    if (mode === "mixed" || mode === "weak_areas" || mode === "subject_focus") {
      return totalDeck > 0 || newCardsLimit > 0;
    }
    return totalDeck > 0;
  }, [mode, dueToday, totalDeck, newCardsLimit]);
  ...
  async function startPracticeSession() {
    ...
    try {
      ...
      try {
        sessionStorage.setItem(`upcat.practice.${res.sessionId}`, JSON.stringify(res));
      } catch {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-primary-700">
                ...
                switch to <strong>Review mode</strong> to reinforce what you&apos;ve learned. This
                approach helps you learn systematically before attempting a full exam.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Tile
          label="Mastered"
          value={stats?.totals.mastered ?? 0}
          accent="bg-emerald-50 text-emerald-700"
        />
        <Tile
          label="Retention"
          value={`${stats?.retentionPct.toFixed(1) ?? "0.0"}%`}
      ...
      <section
        ...
        data-tour="practice-config"
        data-help="pt_subject_select"
      >
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          <div
            className={`rounded-xl border p-4 transition ${
              subjectEnabled
                ? "border-maroon-200 bg-gradient-to-br from-white to-maroon-50/70 shadow-sm"
                : "border-slate-200 bg-slate-50/80"
            }`}
            data-help="pt_subtopic_select"
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-sm font-semibold text-slate-900">Subject</div>
                <p className="mt-1 text-xs leading-5 text-slate-600">
                  Narrow the session to one subject when you want a deliberate drill instead of a
                  mixed deck.
                </p>
              </div>
              <span
            ...
            <select
              ...
              className="mt-4 w-full rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm shadow-sm focus:border-maroon-500 focus:ring-maroon-500 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400"
            >
            ...
            <p className="mt-3 text-xs leading-5 text-slate-500">
              {subjectEnabled
                ? "The selected subject limits both due cards and new introductions for this session."
                : "Switch to Subject Focus mode to choose a single subject for the session."}
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
            ...
            <div className="mt-4 space-y-2 text-xs text-slate-600">
              <div>Max questions is the total cap for the whole session.</div>
        ...
        <div className="mt-4 rounded-2xl border border-sky-200 bg-sky-50/80 p-4 text-sm text-sky-950">
          ...
          <div className="mt-2 grid gap-3 sm:grid-cols-2">
            <p className="leading-6 text-sky-900/90">
              <strong>Max questions</strong> is the hard ceiling for the session. If you set 20,
              the session will never exceed 20 cards.
            </p>
            <p className="leading-6 text-sky-900/90">
              <strong>New cards limit</strong> is not extra on top. It only controls how many unused
              cards may be introduced inside that same total cap.
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div className="flex-1">
              ...
              <p className="mt-1 text-xs leading-5 text-emerald-800/90">
                Add fresh cards to your deck anytime. This is always available so you can grow your
                deck without leaving this page.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={(e) =>
                  setRandomCardsCount(Math.max(1, Math.min(50, Number(e.target.value) || 1)))
                }
        ...
        <div className="mt-6 flex items-center justify-between" data-help="pt_presets">
          <p className="text-xs text-slate-500">
            {mode === "review"
              ...
              : mode === "random"
                ? `Sampling from ${totalDeck} existing card${totalDeck === 1 ? "" : "s"} in your deck.`
                : `Session will use up to ${maxQuestions} cards with up to ${newCardsLimit} new card${newCardsLimit === 1 ? "" : "s"}.`}
          </p>
...
function Tile({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div
        className={`mt-1 inline-flex items-baseline rounded-md px-2 py-0.5 text-2xl font-bold ${accent}`}
      >
        {value}
      </div>
    </div>
