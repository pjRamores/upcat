...
return (
    <div className="mx-auto max-w-5xl px-4 py-8">
        ...
        {isNewUser && (
            <div
                className="mb-6 rounded-lg border border-primary-200 bg-gradient-to-r from-primary-50 to-primary-100 p-4 text-sm"
            >
                <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="mt-1 text-primary-700">
                            ...
                            switch to <strong>Review mode</strong> to reinforce what you've learned. This approach
                            helps you learn systematically before attempting a full exam.
                        </p>
                    </div>
                </div>
            </div>
...
<div>
  ...
  {/* Mode picker */}
  <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm" data-tour="practice-config">
      ...
      {PRACTICE_MODES.map((m) => (
        <button key={m} type="button" onClick={() => setMode(m)} className={`
          rounded-lg border p-4 text-left transition ${
            mode === m ? "border-maroon-500 bg-maroon-50 ring-2 ring-maroon-200" : "border-slate-200 bg-white hover:border-slate-300"
          }
        `}>
    ...
    <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-[1.2fr_0.8fr]">
      <div className={`
        rounded-2xl border p-4 transition ${
          subjectEnabled ? "border-maroon-200 bg-gradient-to-br from-white to-maroon-50/70 shadow-sm" : "border-slate-200 bg-slate-50/80"
        }
      `} data-help="pt_subtopic_select">
        ...
        <span className={`
          rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide ${
            subjectEnabled ? "bg-maroon-100 text-maroon-700" : "bg-slate-200 text-slate-500"
          }
        `}>
      ...
      <select value={subjectArea} disabled={!subjectEnabled} onChange={(e) => setSubjectArea(e.target.value as SubjectArea)} className="mt-4 w-full rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm shadow-sm focus:border-maroon-500 focus:ring-maroon-500" disabled="cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400">
        {SUBJECT_AREAS.map((sa) => (
          <option key={sa} value={sa}>{SUBJECT_META[sa]?.label ?? sa}</option>
        ))}
      </select>
  ...
  <div className="flex.flex-col.gap-3.sm:flex-row.sm:items-end.sm:justify-between">
    ...
    <div className="flex.items-center.gap-2">
      <input
        ...
        className="w-20.rounded-md.border.border-emerald-300.bg-white.px-2.py-2.text-center.text-sm.text-slate-900.focus:border-emerald-500.focus:outline-none"
      />
      <button
        type="button"
...
<div className="mt-6.flex.items-center.justify-between" data-help="pt_presets">
  <p className="text-xs.text-slate-500">
    {mode === "review" ? `${dueToday} cards${dueToday === 1 ? "" : "s"} due right now.` : "Pulling from ${totalDeck} card${totalDeck === 1 ? "" : "s"} in your deck."}
  </p>
  <button
    type="button"
    onClick={handleStart}
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            className="rounded-md border border-slate-500 bg-slate-50 text-slate-900 px-3 py-2.5 font-semibold transition hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-500"
            onClick={decrement}
          >
            -
          </button>
          <div className="flex items-center gap-1">
            <input
              className="h-8 w-8 rounded-md border border-slate-500 bg-slate-50 text-slate-900 text-center text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-slate-500"
              type="number"
              value={value}
              onChange={(e) => {
...
<div className="mt-4 flex items-center gap-3">
    <button
        ...
        className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-slate-300 bg-white text-lg font-semibold text-slate-700 shadow-sm transition hover:border-maroon-300 hover:text-maroon-700 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-300"
    >
        -
    </button>
