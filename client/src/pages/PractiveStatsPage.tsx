...
import { type PracticeCardListEntry, type PracticeCardStatus, type PracticeStatsResponse } from "@upcat/shared";
...
<section className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
  ...
  <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Total reviews logged</div>
    ...
    <div className="mt-4 space-y-2">
      {stats.upcoming.map((u) => (
        <div key={u.date} className="flex items-center gap-3">
          ...
          <div className="w-10 text-right text-xs font-semibold text-slate-700">{u.count}</div>
        </div>
      ))}
    </div>
  </section>
...
<input type="search" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search question text..." className="flex-1 min-w-[200px] rounded-md border-slate-300 text-sm shadow-sm focus:border-primary-500 focus:ring-primary-500" />
</div>
<div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
{cardsLoading ? (
...
{cards.map((c) => (
<tr key={c.cardId}>
<td className="px-4 py-2 text-slate-700">{c.questionPreview || <span className="italic text-slate-400">(no preview)</span>}</td>
...
<td className="px-4 py-2 text-slate-600">{new Date(c.nextReviewDate).toLocaleDateString()}</td>
</tr>
))}
</tbody>
</table>
...
<button type="button" disabled={page === 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="rounded border border-slate-300 px-3 py-1 disabled:opacity-50">← Prev</button>
function Tile({
    label,
    value,
    accent,
