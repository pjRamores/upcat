 ...
 *
 * The session payload is loaded once at mount; per-card grading + rating
 * happen via /practice/:sessionId/answer and /rate.
 */
...
import { save3Snapshot } from "@lib/sessionRecovery";
import { clampPracticeIndex, clearPersistedPracticeRuntime, createPracticeSnapshot, loadPersistedPracticeRuntime, persistPracticeRuntime, } from "@/pages/practiceSessionRecovery";
...
export default function PracticeSessionPage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setReveal({
            isCorrect: false,
            correctAnswer: null,
            rationale: "Failed to load session.",
          });
          setCompleting(true);
          setReward(null);
          setSummary({
    ...
    "This.practice.session.has.expired.Return.to.the.Practice.page.to.start.a.new.one.",
    );
    setLoading(false);
  }
} catch {
...
if (summary) {
    return (
        <div className="mx-auto max-w-xl px-4 py-12">
            ...
            {showOverlay && reward && (
                <p>Award overlay reward={reward} onClose={() => setShowOverlay(false)}/>
            )}
                ...
                <div className="flex items-center justify-between text-xs text-slate-500">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            ...
                            className="rounded px-2 py-0.5 text-xs font-medium text-slate-500 hover:bg-slate-100 hover:text-slate-700"
                        >
                            Exit
                        </button>
                    </div>
            ...
            <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="mb-3 flex items-center justify-between text-xs">
                    <span className="rounded bg-slate-100 px-2 py-0.5 font-medium text-slate-700">
                        {currentCard.question.subjectArea}
                    </span>
                    <span className="text-slate-400 capitalize">
                        {currentCard.isNewIntroduction ? "· new" : ""}
                    </span>
                </div>
                {currentCard.question.passage && (
                    <details className="mb-4 rounded-md border border-slate-200 bg-slate-50 p-3 text-sm">
                        <summary className="cursor-pointer font-medium text-slate-700">
                            Passage: {currentCard.question.passage.title}
                        </summary>
                        <p className="mt-2 whitespace-pre-wrap text-slate-700">
                            {currentCard.question.passage.content}
                        </p>
                    </details>
                )}
                ...
                <div className="mt-4 space-y-2">
                    {["A", "B", "C", "D"] as const}.map((label) => {
                        const text = choiceMap.get(label) ?? "";
                        const isPicked = userAnswer === label;
                        const isCorrect = reveal && reveal.correctAnswer === label;
                        const wasWrong = reveal && isPicked && !isCorrect;
                        const base = "block w-full rounded-md border px-4 py-2 text-left text-sm transition";
                        let cls = `${base} border-slate-200 bg-white hover:border-slate-300`;
                        if (reveal) {
                            if (isCorrect) cls = `${base} border-emerald-500 bg-emerald-50 text-emerald-900`;
                            else if (wasWrong) cls = `${base} border-rose-400 bg-rose-50 text-rose-900`;
                            else cls = `${base} border-slate-200 bg-slate-50 text-slate-600`;
                        } else if (isPicked) {
                            cls = `${base} border-indigo-500 bg-indigo-50 ring-2 ring-indigo-200`;
                        }
                        return (
                            <div key={label} className={cls}>
                                <div className="flex items-center gap-2">
                                    <button
                                        type="button"
                                        onClick={() => handleAnswerClick(label)}
                                        className="rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"
                                    >
                                        {text}
                                    </button>
...
{!reveal ? (
    <div className="mt-6 flex items-center justify-between">
        ...
        <button
            ...
            className="rounded-md bg-indigo-600 px-6 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-slate-300"
        >
    ...
    <div className="mt-6 space-y-4">
        ...
        <div>
            ...
            <p className="mt-2 text-xs text-slate-500">
                {rating ? completing ? "Wrapping up your session..." : "Loading next card..." : PRACTICE_RATING_HINTS[rating ?? "good"]}
            </p>
        </div>
    </div>
</article>
