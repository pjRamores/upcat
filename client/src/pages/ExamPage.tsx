...
import { SUBJECT_META } from "@/pcat/shared";
...
export default function ExamPage() {
  ...
  const shouldAutoResume = useMemo(() => {
    return new URLSearchParams(location.search).get("resume") === "1";
  }, [location.search]);
        ...
        <header className="sticky top-0 z-30 border-b border-gray-200 bg-white shadow-sm">
            ...
            <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    {curQuestion && (
                        <span
                            className={`rounded-lg px-4 py-1.5 font-mono text-base font-bold tabular-nums ${criticalTime
    ...
    <div className="flex-1">
        {curQuestion ? (
            <article
                key={curQuestion.id}
                className={`
                    rounded-xl border border-gray-200 bg-white p-6 shadow-sm animate-fade-in transition ${
...
function PassagePanel({
    ...
    open,
    onToggle,
}: { title: string; content: string; onToggle: () => void }) {
    return (
        <section className="mb-5 rounded-lg border border-amber-200 bg-amber-50/40">
            <button
                type="button"
                onClick={onToggle}
                className="flex w-full items-center justify-between px-4 py-2 text-sm font-semibold text-amber-900"
                aria-expanded={open}
            >
                <span>{title}</span>
                ...
                {states.map((s, i) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex h-8 w-8 items-center justify-center rounded-md text-xs font-semibold transition ${cls} ${isCurrent ? "ring-2 ring-primary-600 ring-offset-1" : "hover:opacity-80"}`}
                        >
                            {i + 1}
                        </button>
                    );
...
return (
    <div className="hidden flex-wrap gap-x-3 gap-y-1 text-gray-500 lg:flex">
        {items.map((it) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true">
            <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
                ...
                <p className="mt-2 text-sm text-gray-600">
                    ...
                    {unanswered > 0 && (
                        <span className="text-orange-600">{unanswered} unanswered</span> will be marked incorrect.
                    )}
                </p>
