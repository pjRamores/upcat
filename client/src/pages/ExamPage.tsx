...
return (
  ...
  <header className="sticky top-0 z-30 border-b border-gray-200 bg-white shadow-sm">
    ...
    <div className="mx-auto flex max-w-7x1 items-center justify-between gap-4 px-4 py-3">
      <div className="flex items-center gap-3 min-w-0">
        <span className="text-sm font-semibold text-gray-700 whitespace-nowrap">
          Q {currentIndex + 1} <span className="text-gray-400">of {totalQuestions}</span>
        </span>
      </span>
      {curQuestion && (
        <span
          className="hidden truncate rounded-full bg-primary-50 px-3 py-1 text-xs font-medium text-primary-700 sm:inline-block">
          {SUBJECT_META[curQuestion.subjectArea].icon} {"."
        }
        {SUBJECT_META[curQuestion.subjectArea].label}
      </span>
    )}
  </div>

  <div
    className={`rounded-lg px-4 py-1.5 font-mono text-base font-bold tabular-nums ${
      criticalTime
...
<div className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 px-4 py-6 lg:flex-row">
  ...
  <div className="flex-1">
    {curQuestion ? (
      <article
        key={curQuestion._id}
        className={`rounded-xl border border-gray-200 bg-white p-6 shadow-sm animate-fade-in transition ${
          shouldBlurExamBody ? "blur-sm" : ""
        }`}
      >
        {curQuestion.passage && (
          <PassagePanel
    ...
    <div
      ...
      <button
        ...
        aria-pressed={cur?.flagged??false}
      >
      ...
      <div className="flex gap-2">
        <button
          ...
          className="rounded-lg border border-gray-200 px-4 py-1.5 text-sm font-medium hover:bg-gray-50 disabled:opacity-40"
        >
          ← Previous
        </button>
        <button
          type="button"
          disabled={isPaused || currentIndex === totalQuestions--1}
          onClick={() => setCurrent(currentIndex+1)}
          className="rounded-lg bg-primary-600 px-4 py-1.5 text-sm font-semibold text-white hover:bg-primary-700 disabled:opacity-40"
        >
          Next →
        </button>
      </div>
...
{/*·Question·navigator·*/}
<aside className="lg:sticky·lg:top-20·lg:h-[calc(100vh-6rem)]·lg:w-72·lg:flex-shrink-0">
<div className={`transition ${shouldBlurExamBody ? "blur-sm" : ""}`}>
<QuestionNavigator
states={states}
currentIndex={currentIndex}
onJump={isPaused ? () => undefined}: setCurrent>
</div>
</div>
...
{confirmSubmit && (
<SubmitConfirmModal
answeredCount={states.filter((s) => s.answer !== null).length}
...
return (
<section className="mb-5·rounded-lg·border·border-amber-200·bg-amber-50/40">
<button
type="button"
onClick={onToggle}
className="flex·w-full·items-center·justify-between·px-4·py-2·text-left·text-sm·font-semibold·text-amber-900"
aria-expanded={open}
>
<span>{title}</span>
...
{open && (
<div
className="border-t·border-amber-200·px-4·py-3·text-sm·leading-relaxed·text-amber-950·whitespace-pre-line"}
{content}
</div>
)}
</section>
      ...
      aria-modal="true">
        <div className="w-full·max-w-md·rounded-xl·bg-white·p-6·shadow-2xl">
          ...
          <p className="mt-2·text-sm·text-gray-600">
            ...
            {unanswered > 0 && (
              <span className="text-orange-600">{unanswered}·unanswered</span> will be marked incorrect.</span>
            )}
          </p>
  ...
  return (
    <div className="min-h-screen·bg-slate-50">
      <div className="border-b·border-gray-200·bg-white·px-4·py-3">
        <div className="mx-auto·h-8·max-w-7x1·animate-pulse·rounded·bg-gray-100"/>
      </div>
      <div className="mx-auto·mt-6·grid·max-w-7x1·grid-cols-1·gap-6·px-4·lg:grid-cols-[1fr_18rem]">
