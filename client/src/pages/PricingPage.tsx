...
export default function PricingPage() {
    ...
    <div className="mx-auto·max-w-6x1·px-4·py-10">
      ...
      <header>
        className="mb-8·rounded-2x1·bg-gradient-to-r·from-rose-900·via-rose-800·to-orange-700·p-8·text-white">
          <h1 className="text-3x1·font-bold">Upgrade to Premium</h1>
          <p className="mt-2·max-w-2x1·text-sm·text-rose-100">
            Unlock full exam access, advanced analytics, ad-free study sessions, and priority support.
          </p>
        </header>

        {isFreeMode ? (
          <section className="rounded-x1·border·border-emerald-200·bg-emerald-50·p-6">
          ...
          <section className="grid·gap-4·md:grid-cols-2·lg:grid-cols-4">
            {plans.map((plan) => (
              <article>
                key={plan.id}
                className={`relative rounded-x1 border bg-white p-5 shadow-sm ${
              ...
              >
                ...
                <div className="mt-4">
                  <div className="text-2x1·font-bold·text-slate-900">¥{plan.price}</div>
                  {plan.originalPrice && (
  ...
  <section className="mt-8·rounded-x1·border·border-slate-200·bg-white·p-5">
    <details>
...
Already paid manually? <Link to="/settings">className="font-sembold text-primary-700 underline">Track
your submissions in Settings</Link>
</div>
</div>
);