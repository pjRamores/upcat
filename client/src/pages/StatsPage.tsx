  ...
  useEffect(() => {
    ...
    return (
      <div className="mx-auto max-w-7xl px-4 py-10">
        <Seo title="My Statistics" description="Track your UPCAT preparation progress and identify weak areas."
        noindex/>
        ...
        <section className="mt-6 grid-grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6"
          ...
          {overview ? (
            <>
              ...
              <OverviewCard icon="🔍" label="Streak" value={`${overview.currentStreak}d`}
              sub={`Best ${overview.longestStreak}d`} accent="orange"/>
              <OverviewCard icon="🔍" label="Accuracy" value={`${overview.overallAccuracy}%`}
...
{studyPlan && (
<section
className="mt-6·rounded-2x1·border·border-emerald-200·bg-gradient-to-r·from-emerald-50·to-teal-50·p-5">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie
              ...
              paddingAngle={3}
              animationDuration={800}
              label={(e:{difficulty:Difficulty;totalQuestions:number}) =>
              ...
              {difficulty.map((d) => (
                <div key={d.difficulty}>
                  <div className="flex·items-center·justify-between·text-sm">
                    <span
                      className="font-medium·text-gray-800">{DIFFICULTY_LABELS[d.difficulty]}</span>
      ...
      <Panel title="Weak·Areas">
        {weak === null ? (
          ...
          <ul className="space-y-3·data-help="st_weak_areas">
            {weak.map((w) => (
              <WeakAreaItem key={`${w.subtopic}`} weak={w}/>
            ))}
          </ul>
        )}
      </Panel>
