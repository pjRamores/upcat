...
export default function StatsPage() {
  ...
  useEffect(() => {
    apiClient
      .get(API_ROUTES.STATS.OVERVIEW)
      .then((r) => setOverview(r.data.data))
      .catch(() => setOverview(emptyOverview()));

    apiClient
      .get(API_ROUTES.STATS.SUBJECT_BREAKDOWN)
      .then((r) => setSubjects(r.data.data))
      .catch(() => setSubjects([]));

    apiClient
      .get(API_ROUTES.STATS.DIFFICULTY_BREAKDOWN)
      .then((r) => setDifficulty(r.data.data))
      .catch(() => setDifficulty([]));

    apiClient
      .get(API_ROUTES.STATS.WEAK_AREAS)
  ...
  useEffect(() => {
    setProgress(null);
    apiClient
      .get(`${API_ROUTES.STATS.PROGRESS}?period=${progressPeriod}`)
  ...
  useEffect(() => {
    setLeaderboard(null);
    apiClient
      .get(`${API_ROUTES.STATS.LEADERBOARD}?period=${leaderboardPeriod}`)
      .then((r) => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      <Seo
        title="My Statistics"
        description="Track your UPCAT preparation progress and identify weak areas."
        noindex
      />
      ...
      >
        {overview ? (
          <>
            <OverviewCard label="Exams Taken" value={overview.totalExamsTaken} icon="📘" />
            ...
            <OverviewCard
              label="Study Time"
              value={overview.totalTimeSpent.formatted}
              icon="⏱"
            />
            <OverviewCard
              label="Streak"
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <h2 className="text-xl font-bold text-slate-900">
                {studyPlan.progressPercent}% complete
              </h2>
              <p className="mt-1 text-sm text-slate-600">
                {studyPlan.completedDays} of {studyPlan.totalDays} study days finished. Avg
                assessment: {studyPlan.averageAssessmentScore}%
              </p>
            </div>
            <div className="text-right">
              <div
                className={`text-sm font-semibold ${
                  studyPlan.daysAhead >= 0 ? "text-emerald-700" : "text-amber-700"
                }`}
              >
                {studyPlan.daysAhead >= 0
                  ? `${studyPlan.daysAhead} day(s) ahead`
                  : `${Math.abs(studyPlan.daysAhead)} day(s) behind`}
              </div>
              <Link
                to="/study-plan/analytics"
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                ...
                <div className="h-64 min-h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart
                      data={subjects.map((s) => ({
                        subject: SUBJECT_META[s.subjectArea].label.split(" "),
                        accuracy: s.accuracy,
                      }))}
                    >
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        ...
                        paddingAngle={3}
                        animationDuration={800}
                        label={(e: DifficultyStat) =>
                          `${DIFFICULTY_LABELS[e.difficulty]}: ${e.totalQuestions}`
                        }
                      >
                        {difficulty.map((d) => (
                          <Cell
                            key={d.difficulty}
                            fill={PALETTE.difficulty[d.difficulty]}
                          />
                        ))}
                      </Pie>
                    ...
                    <div key={d.difficulty}>
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-gray-600">
                          {d.correct}/{d.totalQuestions} · <strong>{d.accuracy}%</strong>
                        </span>
                      </div>
                      <div className="mt-1 h-2 w-full overflow-hidden rounded-full bg-gray-100">
                        <div
                          ...
                          style={{
                            width: `${d.accuracy}%`,
                            backgroundColor: PALETTE.difficulty[d.difficulty],
                          }}
                        />
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <Panel title="Leaderboard">
            ...
            {leaderboard === null ? (
              ...
              <EmptyState compact message="No leaderboard data yet." />
            ) : (
              <LeaderboardTable
                rows={leaderboard}
                me={myRank}
...
function RecentActivity() {
  ...
  useEffect(() => {
    apiClient
      .get(`${API_ROUTES.EXAM_SESSIONS}?limit=10`)
...
function SkeletonCard() {
  return <div className="h-20 animate-pulse rounded-xl border border-gray-200 bg-gray-50" />;
}
...
function EmptyState({
  ...
  return (
    <div
      className={`rounded-lg border border-dashed border-gray-200 bg-gray-50 text-center text-gray-500 ${
