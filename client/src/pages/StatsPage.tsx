...
export default function StatsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      ...
      {/* Overview cards */}
      <section className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6" data-help="st_practice_vs_mock">
        {overview ? (
          <>
            <OverviewCard icon="&#128070;" label="Exams Taken" value={overview.totalExamsTaken} />
            <OverviewCardCircle label="Avg. Score" percentage={overview.averageScore} />
            <OverviewCard icon="&#128072;" label="Best Score" value={`${overview.highestScore}`} accent="amber" />
            <OverviewCard icon="&#128073;" label="Study Time" value={overview.totalTimeSpent.formatted} />
            <OverviewCard icon="&#128074;" label="Streak" value={`${overview.currentStreak}`} sub={`${bestStreakId}`} accent="orange" />
            <OverviewCard icon="&#128075;" label="Accuracy" value={`${overview.overallAccuracy}%`} sub={`${totalQuestionsAnswered} answered`} />
          </>
        ) : (
...
{studyPlan && (
    <section className="mt-6 rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50 p-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-emerald-700">Study Plan</p>
          <h2 className="text-xl font-bold text-slate-900">{studyPlan.progressPercent}% complete</h2>
          <p className="mt-1 text-sm text-slate-600">{studyPlan.completedDays} of {studyPlan.totalDays} study days finished • Avg assessment {studyPlan.averageAssessmentScore}%</p>
        </div>
        <div className="text-right">
          <div className={text-sm font-semibold uppercase tracking-wide text-emerald-700}>{studyPlan.daysAhead >= 0 ? "text-emerald-700" : "text-amber-700"}">{studyPlan.daysAhead >= 0 ? `${studyPlan.daysAhead} day(s) ahead` : `${Math.abs(studyPlan.daysAhead)} day(s) behind`}</div>
          <Link to="/study-plan/analytics" className="mt-1 inline-block text-sm font-medium text-emerald-700 hover:underline">View study plan analytics →</Link>
        </div>
      </div>
    </section>
  )}
  ...
  <div className="space-y-6 lg:col-span-2">
        ...
        {progress === null ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={progress} margin={{top: 10, right: 16, left: -8, bottom: 0}}>
              ...
              <Tooltip contentStyle={{borderRadius: 8, border: "1px solid #e5e7eb", fontSize: 12}} formatter={(v: number, name: string) => [`${v}%`, name]}/>
              <Legend wrapperStyle={{fontSize: 12}}/>
              <Line type="monotone" dataKey="averageScore" name="Avg. Score" stroke={PALETTE.primary} strokeWidth={2.5} dot={{r: 4}} activeDot={{r: 6}} animationDuration={800}/>
              <Line type="monotone" dataKey="accuracy" name="Accuracy" stroke={PALETTE.good} strokeWidth={2} dot={{r: 3}} animationDuration={800}/>
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
    ...
    <Panel title="Subject Breakdown">
      {subjects === null ? (
        ...
        <div className="grid grid-gap-6 lg:grid-cols-2">
          {/* Bars */}
                ...
                <div className="relative" style={{ width: size, height: size }}>
                    <svg width={size} height={size} className="rotate-90">
                        ...
                        <circle
                            ...
                            cy={size / 2}
                            r={r}
                            stroke={colorForAccuracy(percentage)}
                            strokeWidth={stroke}
                            fill="none"
                            strokeDasharray={c}
  ...
  return (
    <div>
      ...
      <div className="h-2 w-full overflow-hidden rounded-full bg-gray-100">
        <div className="h-full rounded-full transition-all" style={{ width: `${stat.accuracy}%`, backgroundColor: color }} />
      </div>
      {stat.trend.length > 0 && (
        <p className="mt-1 text-[11px] text-gray-400">Last {stat.trend.length}: {stat.trend.map((t) => `${t.accuracy}%`).join("→")}</p>
      )}
    </div>
  );
}
  ...
  return (
    <li className="rounded-lg border border-gray-100 bg-gray-50/50 p-3">
      ...
      <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-gray-200">
        <div className="h-full rounded-full bg-amber-500" style={{ width: `${weak.accuracy}%` }} />
      </div>
      <button type="button" onClick={() => navigate("/dashboard")} className="mt-2 text-xs font-semibold text-primary-600 hover:underline">Practice This Topic →</button>
    </li>
  );
}

function LeaderboardTable({ rows, me, myEmail: _myEmail, }: { rows: LeaderboardRow[]; me: LeaderboardRow | null; myEmail?: string }) {
  return (
    <div className="overflow-hidden rounded-lg border border-gray-100">
      <table className="w-full text-sm">
        <thead className="bg-gray-50 text-xs uppercase text-gray-500">
          <tr>
            <th className="w-10 px-3 py-2 text-left">Rank</th>
        ...
        <tbody className="divide-y divide-gray-100">
          ...
          {me && (
            <tr>
              <td colSpan={4} className="border-t border-dashed border-gray-200 py-1 text-center text-[11px] text-gray-400">...</td>
            </tr>
    ...
    return (
        <Panel title="Recent Activity">
            {items === null ? (
                ...
                <ol className="relative ml-3 space-y-4 border-1-2 border-gray-100 pl-4">
                    {items.map((it) => {
        ...
        <div className="flex flex-col items-center justify-center space-y-4 py-8">
            <div className="text-center">
                ...
                {compact ? (
                    ...
                    <p className="text-sm text-gray-500">No exams found. Check back later.</p>
                )}
            </div>
        </div>
    );
