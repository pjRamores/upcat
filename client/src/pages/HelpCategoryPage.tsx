                    ...
                    {items.map((item) => (
                        <Link key={item.slug} to={`/help/article/${item.slug}`}>
                            <div className="block rounded-xl border border-slate-200 bg-white p-4 shadow-sm hover:bg-slate-50">
                                ...
                                <p className="mt-2 text-xs text-slate-500">Estimated read: {item.estimatedReadingMinutes} min</p>
                            </div>
                        </Link>
                    ))}
                    {items.length === 0 && <p className="text-sm text-slate-600">No published articles in this category yet.</p>}
                </div>
