 ...
 *
 * Consumes a `recoveryToken` (15-min, scope "recovery") obtained via
 * the recovery-code or security-questions flow, lets the user pick a
 * new password, and on success redirects to /login.
 */
    ...
    return (
      <div className="mx-auto max-w-md px-4 py-12 text-center">
        ...
        <p className="mt-2 text-sm text-gray-600">
          Please restart the recovery flow.
        </p>
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg =
        ...
        "Could not reset your password. The recovery link may have expired.";
      addToast("error", msg);
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-12">
      ...
      <p className="mt-1 text-sm text-gray-500">
        Choose a strong, unique password. All existing sessions will be signed out.
      </p>

      <form
        onSubmit={submit}
        className="mt-6 space-y-3 rounded-xl border border-gray-200 bg-white p-6 shadow-sm"
      >
