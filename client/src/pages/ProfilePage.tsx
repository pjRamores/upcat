...
export default function ProfilePage() {
  ...
  if (!profile) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center">
        ...
        <p className="text-sm text-slate-600">Failed to load your profile.</p>
        <button
          type="button"
          onClick={() => void load()}
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <section
        className="rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-600 p-6 text-white shadow-lg"
        data-help="gm_xp_bar"
        data-tour="xp-earned"
      >
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <StatPill label="Multiplier" value={`×${profile.streak.multiplier.toFixed(2)}`} />
            <StatPill label="Achievements" value={`${unlockedCount}/${achievements.length}`} />
          </div>
        </div>
          ...
          {weekly ? (
            <>
              ...
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100">
                <div
                  className={weekly.completed ? "h-full bg-emerald-500" : "h-full bg-indigo-500"}
        ...
        {profile.recentXp.length === 0 ? (
          ...
          <ul className="mt-3 divide-y divide-slate-100">
            {profile.recentXp.map((tx) => (
              <li key={tx._id} className="flex justify-between gap-4 py-2 text-sm">
                <span className="text-slate-700">{tx.description}</span>
                <span className="shrink-0 font-mono font-semibold text-emerald-600">
                  +{tx.amount}
                </span>
              </li>
            ))}
      ...
      <section className="mt-6" data-help="gm_hidden_badge">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="text-sm text-slate-500">
            {profile.achievementsSummary.points.toLocaleString()} points · {unlockedCount} of{" "}
