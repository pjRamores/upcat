      ...
      <section className="rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-600 p-6 text-white shadow-lg" data-help="gm_xp_bar" data-tour="xp-earned">
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex items-center gap-4">
            <StatPill label="Streak" value={`${profile.streak.current}`} />
            <StatPill
              label="Multiplier"
...
<div className="mt-6">
    <div className="mb-1 flex justify-between text-xs opacity-90">
        ...
        <span>{(profile.level.xpForNext - profile.level.xpForCurrent).toLocaleString()} XP</span>
        <span>{profile.level.xpToNextLevel.toLocaleString()} XP to next level</span>
    </div>
...
<section className="mt-6 grid gap-4 md:grid-cols-2" data-help="gm_streak_mult">
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-1 text-sm text-slate-600">
            Longest: {profile.streak.longest} days · Multiplier x {profile.streak.multiplier.toFixed(2)}
        </div>
        {profile.streak.hoursUntilExpiry !== null && (
            <div className="mt-3 text-xs text-slate-500">
                Keep it up — {profile.streak.hoursUntilExpiry} hours until midnight UTC.
            </div>
        )}
    </div>
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {weekly ? (
            ...
            <div className="mt-2 text-sm text-slate-500">
                No active challenge — check back Monday for a fresh one!
            </div>
        )}
    </div>
</section>
...
<section className="mt-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex items-center justify-between">
        ...
        <Link to="/leaderboard" className="text-sm font-medium text-indigo-600 hover:underline">
            View leaderboard →
        </Link>
    </div>
        ...
        <button
            ...
            className={`
                rounded-full border px-3 py-1 text-sm transition ${
                    active ? "border-indigo-600 bg-indigo-600 text-white" : "border-slate-300 bg-white text-slate-700 hover:bg-slate-50"
                }
            `}
            >
            {label}
    ...
    return (
        <div className={`
            relative rounded-xl border p-4 transition ${
                achievement.unlocked ? `${styles.border} ${styles.bg}` : "border-slate-200 bg-slate-50 opacity-75"
            }
        `}>
            <div className="flex items-start gap-3">
                <div className={`
                    flex h-12 w-12 shrink-0 items-center justify-center rounded-lg text-xl ${
                        achievement.unlocked ? `${styles.iconBg} ${styles.iconText}` : "bg-slate-200 text-slate-400"
                    }
                `}>
