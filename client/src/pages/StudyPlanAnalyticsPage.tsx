...
export default function StudyPlanAnalyticsPage() {
  ...
  return (
    <div className="mx-auto max-w-5x1 space-y-4 px-4 py-8">
      ...
      {(analytics.subjectProgress ?? []).map((s: any) => (
        ...
        <div className="mt-1 h-2 rounded-bg-slate-100">
          <div className="h-2 rounded-bg-emerald-500"
            style={{width: `${Math.min(100, (s.modulesCompleted / Math.max(1, s.modulesTotal)) * 100)%}`}}/>
        </div>
      </div>
    </div>
  ))
...
function Tile({label, value}: {label: string; value: string}) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <p className="mt-1 text-2x1 font-bold text-slate-900">{value}</p>
    </div>
  );
}