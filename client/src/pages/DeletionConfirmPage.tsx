 ...
 * On success, sets status to "processing" and emails the user.
 */
...
export default function DeletionConfirmPage() {
  ...
  useEffect(() => {
    ...
    deletionApi
      ...
      .then(() => setState("ok"))
      .catch((err) => {
        const msg = (err as { response?: { data?: { error?: string } }; }).response?.data?.error || "This link is invalid or has expired.";
        setErrorMsg(msg);
        setState("error");
      });
  }, [id, token]);
