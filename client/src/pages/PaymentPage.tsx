...
export default function PaymentPage() {
  ...
  async function startPangMeryenda() {
    ...
    try {
      ...
      const message =
        (err as { response?: { data?: { error?: string } } }).response?.data?.error ||
        "Could not initiate PangMeryenda payment.";
      setStatusMessage(message);
      setSubmitting(false);
    }
  }
...
<p className="mt-1 text-sm text-slate-600">Plan: {selectedPlan.name} • ₱{selectedPlan.price}</p>
...
<div className="mt-6 space-y-5">
    ...
    {(channel && (
        <section className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-1 text-sm text-slate-700">Send exactly ₱{selectedPlan.price.toFixed(2)} to:</p>
            <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm">
                <strong>Account Name: </strong>{channel.accountName}</p>
