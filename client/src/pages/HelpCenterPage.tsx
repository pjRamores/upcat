              ...
              <div className="rounded-xl border border-slate-200 bg-white p-2">
                {searchPreview.map((item) => (
                  <Link key={item.slug} to={`/help/article/${item.slug}`}>
                    <p className="block rounded-lg px-3 py-2 hover:bg-slate-50">{item.title}</p>
                    <p className="line-clamp-1 text-xs text-slate-600 dangerouslySetInnerHTML={{ __html: item.excerpt }}></p>
                  </Link>
                ))}
              </div>
            )}
        ...
        <div className="mt-3 space-y-2">
            {quickLinks.map((link) => (
                <Link key={link.slug} to={`/help/article/${link.slug}`}>
                    <p className="block rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-indigo-700 hover:bg-indigo-50">{link.title}</p>
                </Link>
            ))}
        </div>
    </div>
