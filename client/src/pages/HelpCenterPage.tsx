...
export default function HelpCenterPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <Seo
        title="Help Center"
        description="Beginner-friendly guides for every UPCAT Simulator feature."
      />

      <header>
        <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-sky-50 via-white to-indigo-50 p-6">
          <h1 className="text-3xl font-bold text-slate-900">Help Center</h1>
          <p className="mt-2 max-w-2xl text-sm text-slate-700">
            Learn every feature without leaving context. Search guides, open category docs, and
            ...
            {searchPreview.length > 0 && (
              <div className="rounded-xl border border-slate-200 bg-white p-2">
                {searchPreview.map((item) => (
                  <Link key={item.slug} to={`/help/article/${item.slug}`} className="block">
                    <p className="rounded-lg px-3 py-2 hover:bg-slate-50">{item.title}</p>
                    <p
                      className="line-clamp-1 px-3 pb-2 text-xs text-slate-600"
      ...
      <section className="mt-8">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map((category) => (
            <Link key={category.category} to={`/help/category/${category.category}`}>
              <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm hover:bg-slate-50">
      ...
      <section className="mt-8 grid gap-6 md:grid-cols-2">
        <div>
          ...
          <div className="mt-3 space-y-2">
            {popular.map((article) => (
              <Link key={article.slug} to={`/help/article/${article.slug}`} className="block">
          ...
          <div className="mt-3 space-y-2">
            {quickLinks.map((link) => (
              <Link key={link.slug} to={`/help/article/${link.slug}`}>
                <p className="block rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-indigo-700 hover:bg-indigo-50">
                  {link.title}
                </p>
              </Link>
            ))}
        ...
        <div className="md:col-span-2">
          <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
            ...
            <p className="mt-1 text-xs text-slate-600">
              Replay onboarding tours from Settings → Help & Guidance.
            </p>
          </div>
        </div>
      </section>
      ...
      <footer className="mt-10 text-sm text-slate-600">
        ...
        <Link to="/contact" className="font-medium text-indigo-700 hover:underline">
          Contact Support
        </Link>
      </footer>
    </div>
