...
export default function HelpCenterPage() {
    ...
    return (
        <div className="mx-auto max-w-6xl px-4 py-10">
            ...
            <header className="rounded-2xl border border-slate-200 bg-gradient-to-br from-sky-50 via-white to-indigo-50 p-6">
                        ...
                        <div className="rounded-xl border border-slate-200 bg-white p-2">
                            {searchPreview.map((item) => (
                                <Link key={item.slug} to={`/help/article/${item.slug}`}>
                                    <div className="block rounded-lg px-3 py-2 hover:bg-slate-50">
                                        <p className="text-sm font-medium text-slate-900">{item.title}</p>
                                        <p className="line-clamp-1 text-xs text-slate-600" dangerouslySetInnerHTML={{ __html: item.excerpt }} />
                                    </div>
                                </Link>
                            ))}
                        </div>
        ...
        <div className="mt-3 space-y-2">
            {popular.map((article) => (
                <Link key={article.slug} to={`/help/article/${article.slug}`}>
                    <div className="block rounded-lg border border-slate-200 bg-white px-4 py-3 hover:bg-slate-50">
        ...
        <div className="mt-3 space-y-2">
            {quickLinks.map((link) => (
                <Link key={link.slug} to={`/help/article/${link.slug}`}>
                    <div className="block rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-indigo-700 hover:bg-indigo-50">
                        {link.title}
                    </div>
                </Link>
            ))}
