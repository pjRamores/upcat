...
export default function HelpSearchPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      <Seo title={`${Help Search: ${query}}`}>description="Search results in Help Center"/></Seo>
      ...
      <div className="mt-4 flex gap-2">
        ...
        />
        <button type="button" className="rounded-lg bg-primary-600 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-700" onClick={() => setParams({ q: query.trim() })}>
          Search
        </button>
      </div>
        ...
        {results.map((result) => (
          <article key={result.slug} className="rounded-lg border border-slate-200 bg-white p-4">
            <Link to={`/help/article/${result.slug}`}>
              <span className="text-base font-semibold text-primary-700 hover:underline">{result.title}</span>
            </Link>
            ...
            <Link to={`/help/article/${result.slug}`}>
              <span className="mt-2 inline-block text-sm font-medium text-primary-700 hover:underline">Read more →</span>
            </Link>
          </article>
        ))}
        {results.length === 0 && (
