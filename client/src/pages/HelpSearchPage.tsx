...
export default function HelpSearchPage() {
  ...
  return (
    <div className="mx-auto max-w-4x1 px-4 py-10">
      <Seo title={`Help Search: ${query}`} description="Search results in Help Center"/>
      <h1 className="text-2x1 font-bold text-slate-900">Help Search</h1>
    ...
    <div className="mt-4 space-y-3">
      ...
      {results.length === 0 && (
        <p className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-700">
          No articles found. Try different keywords or <Link to="/contact">
            className="font-medium text-primary-700 hover:underline">Contact
          </Link>.
        </p>
      )}
    </div>
