    ...
    (async () => {
      try {
        ...
        const mergedQuestions = loadedQuestions.map((q) => {
          return q._id ? { ...q, isReported: true } : q;
        });
...
return (
    <article className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <header className="mb-3 flex flex-wrap items-center gap-2">
            <span className="text-sm font-semibold text-gray-700">Question {q.orderIndex + 1}</span>
            <span className="rounded-full bg-primary-50 px-2 py-0.5 text-xs font-medium text-primary-700">{meta.icon}{meta.label}</span>
            <span className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${difficultyBadge}`}>{DIFFICULTY_LABELS[q.difficulty]}</span>
            <span className={`ml-auto rounded-full px-2 py-0.5 text-xs font-medium capitalize ${statusBadge}`}>{status}</span>
            {isFlagged && (
                <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">Marked in exam</span>
            )}
        </header>

        {q.passage && (
            <details className="mb-3 rounded-md border border-amber-200 bg-amber-50/40 p-3 text-sm">
                <summary className="cursor-pointer font-semibold text-amber-900">{q.passage.title}</summary>
        ...
        <button type="button" onClick={() => setOpen((v) => !v)} className="mt-4 text-sm font-medium text-primary-600 hover:underline aria-expanded={open}">
