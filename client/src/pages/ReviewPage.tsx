import {useEffect, useMemo, useState} from "react";
import {Link, useParams} from "react-router-dom";
import apiClient from "@/lib/api";
import AdSlot from "@/components/AdSlot";
import Spinner from "@/components/Spinner";
import MathText from "@/components/MathText";
import Seo from "@/components/Seo";
import FlagQuestionModal from "@/components/FlagQuestionModal";
import {
  API_ROUTES,
  type:Difficulty,
  DIFFICULTY_LABELS,
  SUBJECT_AREAS,
  SUBJECT_META,
  type:SubjectArea,
} from "@upcat/shared";

interface ReviewQ {
  _id: string;
  subjectArea: SubjectArea;
  difficulty: Difficulty;
  type: string;
  questionText: string;
  choices: { label: "A" | "B" | "C" | "D"; text: string }[];
  correctAnswer: "A" | "B" | "C" | "D";
  rationale: string;
  userAnswer: "A" | "B" | "C" | "D"; null;
  isCorrect: boolean;
  isFlagged?: boolean;
  isReported?: boolean;
  orderIndex: number;
  passage?: { title: string; content: string } | null;
}

type StatusFilter = "all" | "correct" | "incorrect" | "unanswered";

export default function ReviewPage() {
  const {sessionId} = useParams<{sessionId: string}}();
  const [questions, setQuestions] = useState<ReviewQ[]>(([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string>|null>(null);

  const [statusFilter, setStatusFilter] = useState<StatusFilter>("all");
  const [activeIndex, setActiveIndex] = useState(0);
  const [jumpSubject, setJumpSubject] = useState<SubjectArea>|null>(null);
  const [flaggedQuestionIds, setFlaggedQuestionIds] = useState<Set<string>>(new.Set());
  const [reportedQuestionIds, setReportedQuestionIds] = useState<Set<string>>(() => {
    if (!sessionId) return new.Set();
    try {
      const stored = localStorage.getItem(`upcat.reported_questions.{sessionId}`);
      return stored ? new.Set(JSON.parse(stored)) : new.Set();
    } catch {
      return new.Set();
    }
  });

  const [reportingId, setReportingId] = useState<string>|null>(null);
  // Persist reported question IDs to localStorage
  useEffect(() => {
    if (!sessionId || reportedQuestionIds.size === 0) return;
    try {
      localStorage.setItem(
        `upcat.reported_questions.{sessionId}`,
        JSON.stringify(Array.from(reportedQuestionIds)),
      );
    } catch {
      // Silently fail if storage is unavailable
    }
  }, [reportedQuestionIds, sessionId]);

  useEffect(() => {
    if (!sessionId) return;
    let cancelled = false;
    (async () => {
      try {
        const {data} = await apiClient.get(API_ROUTES.EXAM.REVIEW(sessionId));
        if (cancelled) return;
        const loadedQuestions = (data.data.questions ?? []) as ReviewQ[];

        // Merge locally reported questions with API data
        const mergedQuestions = loadedQuestions.map((q) =>
          reportedQuestionIds.has(q._id) ? {...q, isReported: true} : q
        );

        setQuestions(mergedQuestions);
        setFlaggedQuestionIds(
          new.Set(mergedQuestions.filter((q) => q.isFlagged).map((q) => q._id)),
        );
        // Keep reportedQuestionIds as is (already set from localStorage)
        } catch (e) {
          const msg =
            (e as { response?: { data?: { error?: string } } })?.response?.data?.error ||
            "Could not load review";
          setError(msg);
        } finally {
          if (!cancelled) setLoading(false);
        }
      })();
      return () => {
        cancelled = true;
      };
    }, [sessionId, reportedQuestionIds]);

    // Subjects available given the current status filter (for the subject selector).
const availableSubjects = useMemo(() => {
  const subjectSet = new Set(
    questions
    .filter((q) => {
      if (statusFilter === "correct" && !q.isCorrect) return false;
      if (statusFilter === "incorrect" && (q.isCorrect || q.userAnswer === null)) return false;
      if (statusFilter === "unanswered" && q.userAnswer !== null) return false;
      return true;
    })
    .map((q) => q.subjectArea),
    );
  return SUBJECT_AREAS.filter((s) => subjectSet.has(s));
}, [questions, statusFilter]);

const filtered = useMemo(() => {
  return questions.filter((q) => {
    if (jumpSubject && q.subjectArea !== jumpSubject) return false;
    if (statusFilter === "correct" && !q.isCorrect) return false;
    if (statusFilter === "incorrect" && (q.isCorrect || q.userAnswer === null)) return false;
    if (statusFilter === "unanswered" && q.userAnswer !== null) return false;
    return true;
  });
}, [questions, jumpSubject, statusFilter]);

const activeQuestion = filtered[activeIndex] ?? null;

if (loading) return <div className="py-20 flex justify-center"><Spinner /></div>;
if (error) {
  return (
    <div className="mx-auto max-w-md px-4 py-20 text-center">
      <p className="text-amber-600">{error}</p>
      <Link to="/dashboard" className="btn-secondary mt-4 inline-block">
        Back to Dashboard
      </Link>
    </div>
  );
}

const summary = {
  correct: questions.filter((q) => q.isCorrect).length,
  incorrect: questions.filter((q) => !q.isCorrect && q.userAnswer !== null).length,
  unanswered: questions.filter((q) => q.userAnswer === null).length,
};

const statusMeta = (q: ReviewQ) => {
  if (q.userAnswer === null) {
    return {label: "unanswered", chip: "bg-gray-100 text-gray-600", dot: "bg-gray-400"};
  }
  if (q.isCorrect) {
    return {label: "correct", chip: "bg-green-100 text-green-700", dot: "bg-green-500"};
  }
  return {label: "incorrect", chip: "bg-amber-100 text-amber-700", dot: "bg-amber-500"};
};

return (
  <div className="mx-auto max-w-7x1 px-4 py-10">
    <Seo title="Review Answers" description="Review your practice exam answers and rationales." noindex/>
    <div className="flex flex-wrap items-center justify-between gap-4">
      <div>
        <h1 className="text-2x1 font-bold text-gray-900">Review Answers</h1>
        <p className="mt-1 text-sm text-gray-500">
          {summary.correct} correct • {summary.incorrect} incorrect • {summary.unanswered} unanswered
        </p>
        <p className="mt-1 text-xs text-gray-500">
          Marked in exam = your personal follow-up marker • ▲ Report a content issue = sends feedback
        </p>
        </div>
        <Link to={`/results/${sessionId}`} className="text-sm font-medium text-primary-600 hover:underline">
          Back to Results
        </Link>
      </div>
    </div>

    {/* --- Two-column layout: question card + right nav panel --- */}
    <div className="mt-6 flex flex-col gap-6 lg:flex-row lg:items-start">

      {/* --- Focused question card --- */}
      <div className="min-w-0 flex-1">
        {!activeQuestion && (
          <p className="rounded-lg border border-gray-200 bg-white p-8 text-center text-sm text-gray-400">
            No questions match the current filters.
          </p>
        )}
        {activeQuestion && (
          <QuestionCard
            key={activeQuestion._id}
            q={activeQuestion}
            isFlagged={flaggedQuestionIds.has(activeQuestion._id)}
            isReported={reportedQuestionIds.has(activeQuestion._id)}
            onReport={() => setReportingId(activeQuestion._id)}
          />
        )}
      </div>
    </div>
  </div>
{/* --- Right: Navigation panel ------------------- */}
<aside className="w-full shrink-0 lg:w-64">
  <div className="sticky top-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm space-y-3">

    {/* Subject selector */}
    <div>
      <label className="mb-1 block text-xs font-medium text-gray-500">Subject</label>
      <select
        className="w-full rounded-md border border-gray-300 bg-white px-2 py-1.5 text-sm"
        value={jumpSubject ?? ""}
        onChange={(e) => setJumpSubject(e.target.value as SubjectArea)}
        disabled={availableSubjects.length === 0}
      >
        {availableSubjects.map((s) => (
          <option key={s} value={s}>
            {SUBJECT_META[s].icon} {SUBJECT_META[s].label}
          </option>
        ))}
      </select>
    </div>

    {/* Status filter */}
    <div>
      <p className="mb-1 text-xs font-medium text-gray-500">Status</p>
      <div className="flex flex-wrap gap-1">
        {[["all", "correct", "incorrect", "unanswered"] as StatusFilter[]).map((s) => {
          const active = statusFilter === s;
          const label = s === "all" ? "All" : s.charAt(0).toUpperCase() + s.slice(1);
          const activeColor =
            s === "correct" ? "bg-green-600 text-white border-green-600" :
            s === "incorrect" ? "bg-amber-500 text-white border-amber-500" :
            s === "unanswered" ? "bg-gray-500 text-white border-gray-500" :
            "bg-primary-600 text-white border-primary-600";
          return (
            <button
              key={s}
              type="button"
              onClick={() => setStatusFilter(s)}
              className={`rounded-full border px-2.5 py-0.5 text-xs font-medium transition ${
                active ? activeColor : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"
              }`}
            >
              {label}
            </button>
          );
        })}
      </div>
    </div>

    {/* Prev / Next + counter */}
    <div className="flex items-center gap-2 border-t border-gray-100 pt-3">
      <button
        type="button"
        disabled={filtered.length === 0 || activeIndex === 0}
        onClick={() => setActiveIndex((idx) => Math.max(0, idx - 1))}
        className="rounded-lg border border-gray-200 px-3 py-1.5 text-sm disabled:opacity-40"
      >
        <button
          type="button"
          disabled={filtered.length === 0 || activeIndex >= filtered.length - 1}
          onClick={() => setActiveIndex((idx) => Math.min(filtered.length - 1, idx + 1))}
          className="rounded-lg border border-gray-200 px-3 py-1.5 text-sm disabled:opacity-40"
        >
          <button>
            <span className="text-xs text-gray-500">
              {filtered.length === 0 ? "No items" : `${activeIndex + 1} / ${filtered.length}`}
            </span>
          </button>
        </button>
        <span className="text-xs text-gray-500">
          {filtered.length === 0 ? "No questions match." : ""}
        </span>
      </div>

      {/* Summary pills */}
      <div className="border-t border-gray-100 pt-3">
        <p className="mb-2 text-xs font-medium text-gray-500">
          Questions{filtered.length > 0 ? `${filtered.length}` : ""}
        </p>
        {filtered.length === 0 ? (
          <p className="text-xs text-gray-400">No questions match.</p>
        ) : (
          <div className="flex flex-wrap gap-1.5">
            {filtered.map((q, idx) => {
              const meta = statusMeta(q);
              const isActive = idx === activeIndex;
              const flagged = flaggedQuestionIds.has(q._id);
              const reported = reportedQuestionIds.has(q._id);
              return (
                <button
                  key={q._id}
                  type="button"
                  onClick={() => setActiveIndex(idx)}
                  title={`Item ${idx + 1} • ${meta.label}${flagged ? " • marked" : ""}${reported ? " • reported" : ""}`}
                  className={`relative inline-flex h-7 min-w-[2rem] items-center justify-center rounded px-1 text-xs font-sembold transition ${
                    isActive
                    ? "ring-2 ring-primary-500 ring-offset-1 " + meta.chip
                    : meta.chip + " opacity-80 hover:opacity-100"
                  }`}
                >
                  {idx + 1}
                  {flagged && (
                    <span>
                      <span className="text-xs text-gray-500">
                        {filtered.length === 0 ? "No questions match." : ""}
                        {flagged && (
                          <span className="text-xs text-gray-500">
                            {filtered.length === 0 ? "No questions match." : ""}
                            {flagged && (
                              <span className="text-xs text-gray-500">
                                {filtered.length === 0 ? "No questions match." : ""}
                                {flagged && (
                                  <span className="text-xs text-gray-500">
                                    {filtered.length === 0 ? "No questions match." : ""}
                                    {flagged && (
                                      <span className="text-xs text-gray-500">
                                        {filtered.length === 0 ? "No questions match." : ""}
                                        {flagged && (
                                          <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
                                              <span className="text-xs text-gray-500">
                                            {filtered.length === 0 ? "No questions match." : ""}
                                            {flagged && (
```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` `

```{+` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` `{` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {`{` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` { {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {`{` {`{`{` {` {` {` {` {` {` {` {` {` {` {`{` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {`{`{> `{` {`{`{` {`{` {` {`{` {` {` {` {` {` {` {` {` {` {` {` {` {` {`{`{` {`{` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {`{` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {` {${` {` {`{` {{}-{`{` {` {` {` {` {` {` {` {`{`{` {`{`{`{>`{> {`{>`{` {`{`{`{` {`{`{`{`{`{`{`{`{` {`{` {` {`{`{` {`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{>$${`{`{>$${`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{>`{`{`{`{`{`{`{`{`{`{`{`{>`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{>${`{`{>`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{>{`{`{`{`{`{`{`{`{>`{`{`{>{`{`{>{`{`{`{`{`{`{`{`{`{`{`{>{>{`{>`{>{`{>{`{`{>{`{`{>{>{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{>{`{>{>{`{>{>{`{>{>{>{`{>{>{`{>{`{`{>{>{>{>{>{`{>{>{>{>{>{`{`{`{`{`{>{`{`{>{>{`{>{`{`{>{>{`{>{>{`{>{>{>{>{>{`{>{>{>{`{>{>{>{>{>{>{>{>{`{>{>{`{>{>{`{>{>{>{>{>{>{`{>{`{`{>{`{`{>${`{>{>{>{`{>{>{`{>{>{>{>{>{>{<>{`{>{>{>`{`{`{>{`{>{>{`{>{>{`{>{>{`{<td>{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{>{>{`{`{>{`{`{`{`{`{>`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{>{`{>{`{`{`{>{`{>{`{`{>{>{`{`{>{`{`{>{>{`{`{`{`{>{> {`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{<`{`{`{`{`{`{`{>{>{`{<`{<`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{>{`{`{<>{`{>{>{>{>{>$${<>{>{<>{>{>{>{<>{<>{>{>{`{>{>{`{>{>{<>{<>{<>{<>{<>{`{<`{<>{`{`{`{>{`{>{>{`{`{>`{`{`{>`{<>{>{>{<>{>{<>{>{>{`{<>{>{`{`{`{`{<>{`{`{`{`{`{`{`{`{`{<>{<>{>{>{>{>`{<>{>{<>{<>{<>{<>{<>`{>`{<>`{>`{<>`{<>`{>{>{>{>{> {`{<>{`{`{`{`{`{`{`{`{`{`<>{`{>{`{`{`{`{`{`{`{`{`{`{`{>`{`{`{`{`{>{>`{>{>{>{>{>{>{`<>{>{>`{<>{>{>{>{>`{>{>{`{>{`{`{`{`{`{`{`{`{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{`{`{>{>{`{>{>{`{>{>{`{`{`{`{>{>{>{>{>{>{>{>{>{>{>{>{<>{>{<><>{>{<>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{<>{>{<>{<>{<>{<>{>{>{>{>{`{>{`<> {`{> {`{`{> {`{>{`{>{>{>{`{> {`{`{>{`{`{`{`{`{>`{`{`{`{`{`{>{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{`{> {`{> {`{`{> {. `{`{> {m{`{>{>{`{>{> {`{`{`{`{`{`{`{`{>{`{>{>{`{>{>{> {>{>{>{>{>{`{>{>{>{`{>{`{`{`{>{`{>{>{>{>{>{`{`{>{`{`{>{`{`{>{`{>{>{`{`{`{`{`{>{`{`{>{`{`{>{`{`{>{>{`{>{><>{`{>{>{>{>{>{>{>{> {> {`{>{`{`{`{>{>{>{`<>{`{>{`{> {`{`{`{> {`{> {`{> {>{> {`{>{>{> {>{>{>{>{>{>{>{>{>{>{>s</td>s{> {s">{>{>{>{>{>{> {`{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{s>{>{>{s{>{>{>{>{>{>{>{>{>{>{> {> {<>{>{>{>{>{>{> <td>{>{> {> {>{>{>{> {<>{> {`<>{>{> {`{>{>{>{>{>{> <> {<>{>{> {`{> {<>{>{> {<>{<>{<>{>{> <td>{> {<td>{> {<td>{<td>{`{> {<>{> {<td>{>{>{> {<td>{>{> {<td>{`{> {`{`{`{> {`{> {`{`{`{>{> {`{> {`{<td>{<td>{>{>{<td>{>{<>{<>{<td>{<>{<td>{<>{<td>{<>{<td>{<td>{<td>{s</td>s</td> <td>* `{<td>{<td>{<td>s{<td>s</td> <td>{<>{<>{>{> {`<td>{>{> {`{>{`{> {`{>{> {<>{>{>{>{> {<>{>{>{>{>{>{>{>{<>{>{>{>{>{>{>{>{>{>{<>{>{<>{>{<>{>{>{>{>{<>{<td>{s
    <td>{<td>s{<td>s{<td>s{>{>{<td>{<td>{>{<td>* <td>{> {`{`{`{<td>{> {`{> {<td>{<td>s{<td>s{>s{<>{<>{>{>{> {`{>{> {-{>{>{>{>{>{>{>{>{>{>{>{>{>s{>s> {<td>s> {<td>* <td>* {->
    <td>{> {<td>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{> {>s{> {>{>{>{>{>{>{> { {>{>{>{>{>{>{>{>{>{>{>{>{>{>{>{<>{>{>{>{>{>{>{>{>s{> {s>s{s{> {> {> {> {> {>{> {s{> {s
```{>{> {>{>{> {>{>{>{>{>{s{>{>{> {> {>*{>{> {> {<td>* {<td>s
```{>{- {> {<td>* <td>{> {`{> {->
    <td>* {*</td>* {s
```{> {> <td>* {- "</td{> <td>*{> <td>s{> <td>{> {<td>{> {s> {->
    <td>* {<>* <td>* {<> {> {`{> {> {`{> <td>{> {name</td> <td>s{<>* {* {* {* {* {> {> {m{>{> {<td>s{> `{> {> {> {`{`{> {`{> {`{> {> {> {>*</td> {> {<td>{> {s>
    <td>{> {<td>{`{> {s`{> {> {->
    <td>*</td> {s</td>*{> {<>{> {->
    <td>{> {<td>{>{>{>s>s <td>* {> {->
    <td>*</td>s <td>s>*{> {-{>{>{>s>*{>{>*{> {->
    <td>* {> {> {m{>s</td> {->
    <tr>
    <tr>
    <tr>
    <tr>
    <tr>
    <tr>
    <tr>
    <tr>
    <tr>
```{* <tr>
    <td>* {title{>*{>*{*{>*{>*{> {> {> {<td>* {<td> {> {<td>{> <td>{> <>{>{> {> {> {> {> {> {>* <td>*{>{>{> {>* {>{>{> <td>{>{> <td>{> <td>{>{>{>* <td>*{> <td>*{>{> {- {>*{> {> {> {> {> {> {> {> {> <tr>
    <tr>
    <tr>
    <tr: "m{>{-{>{> {*{>*{> {*{*{>*{>{> <tr>
```{>{*{*{*{*{> {*{*{> <td>*{>{> {-{>{>{>{>{>{> {>{>{>{> {<td>* {<td>*{> {> {m
```{> {m> <td>{> {->
```{> {> <td>* {<td>*{> {> {> {> {> {> {> {> <td>{>{>{>* {title{> {* { {>*{* {m
```{>{<td>{<>{>{>{*{*{ { {*{ {*{ { {>{ { {>{ { { {*{*{*{*{ { {>{ { {*{|td>{ {*{ { { {*{ { { {<td>*{> {* {m
```{*{*{*{*{ {*{*{*{*{> {<td>­{* *{*{*{*{*{* {* { { { { {* {*{> {*{*{*{*{> {> {>* {m
```{>* {*{*{* 
```{* {*</td>* {*{* {*{> <td>{<td>{<td>*{>{>{> {<td>* {<td>* {<td>{> {* {<td>{>{*{*{*{*{*{*{*{*{*{*{*{* {>{>{>*{* {-{>*{* {*{* {*{>{* <td>*{*{> {* {title{* {title{*{>{> `{ {* {id{> {m
```{> {<td>{<td>* {<td>{>*{*{* <tr>
    <tr>
    <tr>
    <tr>
    <tr>
    <tr>
    <tr>
    <tr>
    <tr>*{*{*{*{*{*{*{*{*{*{*{*{ {*{>*{* { { {*{ {*{  alongue <td>* {m</td> <td>* {-{* <td>* {-{>* {* {* {* {*{*{*{* <td>*{*{*{*{*{*{*{*{*{*{*{* <td>*{* {m{* <td>* {*{* {i
```{*{* {i>*{* {td>* {i>* {i
```{> <td>* {* {* <td>* {* <td>* {m
    <td>* {* <td>* {* {* {* {* <td>* {* {* <td>*</td> {* {* {* {*{*{*{*{*{*{*{*{*{*{*{*{*{* {```{* {* { * {*{*</td> {* {* {* {* {* {* {* {* {* {* {* {* {* {*{>* {* {*{*{*{*{*{*{*{* {td>{*{* {*{*{ {*{*{ { { { { {* {td>{>{*{*{>{*{td>*{ { { { { {>* {m{td>{td> <td>{td>*{td> <td>{ {>{* {title{ { {td> <td>* <td>{td> <td>* {id: <td>{* {i
```{*{>*{*{ `{ {> <td>*{ { { { {>* {```{ { { {* {*{ {>* <td>{ { { { { {* <td>* <td>* <td>*{ { { { { { { {* {i{ {>*{ { { { { {*{*</td>
    {* <td>* {* {*{* {*{* <td>*</td>
    <td>* <td>*</td>
    {*</td>
    {*</td>
    {*</td>
    <td>*</td>
    {*</td> <td>* <td>*</td>* <td>* {* <td>* <td>*</td>*{*</td> {*{* {* {* {* {* {*{>* <td>* {m
    {```{* {m <td>* <td>* <td>* {*{ { { { { { { {* <td>* {* <td>* {nowing <td>* {nowhere{* <td>* {* <td>* {*</td>
    {*{> {* {* {* {* {* {* {- {* { { {* {* *{* { {*{*{*{*{*{*{*{*{*{*{ { { { { * * {* { *{* {*{*{*{ {*{ { { {*{ { { { { {*{ * *{* {nowledge{>* <td>* { {* {*{*{* *{*{*{*{*{*{*{* { { { { {* { * * {*{* {*{*{*{* {*{*{*{* {*{ {* {*{* {* {title{* {*{* {nowhere{* {* {* {*{ { { {*{*{*{*{* {*{*{*{*{*{*{*{*{*{*{*{* {*{*{*{* {i>*{*{*{*{*{*{*{*{*{*{*{* {*{* <td>*{* {*{* {* <td>*{*{*{*{*{*{* {* {* {* {* {* {* {*{*{ { {  *{*{*{*{*{*{*{*{*{   {*{*{*{*{* {*{*{ {*{*{*{*{ {   { { {*{*{*{*{*{*{*{*{*{*  {*{ { { {  * {i  * {* {i  *{*{*{*{*{*{*{*{*{*{*{*{*{>*{*{*{> {> <td>*{*{> {> {*{> {*{*{*{* {* {> {> { {>* {>* {* { { {* {*{>*{ { { {* {*{*{*{> {*{*{*{*{*{* { { { { { { { { {*{> <td>*{> <td>*{*{> {*{*{>*{> <tr>
    <tr>
    {* {>* {* {> {*{>*{>*{>*{*{*{*{*{>*{* <td>*{*{*{*{* {*{*{*{*{*{*{*{*{*{*{*{*{ {*{>*{td> {*{ {*{*{ {*{*{ {*{*{*{*{*{*{*{*{*{*{*{> { {*{*{> <td>*{*{> <td>*{>*{> <td>*{>*{>*{*{*{ {> {*{*{> {*{> {*{*{*{*{> {*{*{*{*{*{*{*{* {*{*{*{*{*{*{*{*{*{*{* {i {> {i <td>{> {* <td>*{>*{*{>*{> <td>{*{*{> {*{*{*{*{*{*{*{ {> {*{> {> {* {*{*{*{*{* <td>{* {*{* <td>{*{*{* {= {*{* <td>{* {*{*{*{*{*{*{*{td> <td>{*{*{* {*{> {|{td> {* <td>*{* <td>* <td>*{*{*{*{*{*{*{*{* <td>*{* <td>*{*{* <td>*{* <td>*{* <td>*{*{* <td>* {- {* <td>* <td>* <td>{* <td>* <td>* <td>*{ {*{*{*{* <td>* {i { {>*{*{> { {*{* <td>* <td>* <td>{* <td>* {= `{* <td>* <td>*{* <td>* <td>*{*{* {- {* <td>
    {* { { {* {i>
    {* {i {i>
    {*{*{*{* {s{*{*{*{*{*{| `{td>
    { {> {*{td>
    {*{*{*{>*{ {b{| `{b{*{*{*{* {b{*{*{*{b{*{td> { {*{ <sup "un{*{ * {m <td>*{* {* {*{* {*{*{*{ * <td>* {* <td>*{* <td>*{*{*{*{* <td>*{*{*{*{bard{*{*{* {bard{*{* <td>*{*{*{*{*{*{*{*{*{*{*{*{*{*{*{*{* {* {*{*{* {* {*{* {* {* {* {*{*{*{*{*{>*{*{*{*{*{*{*{*{*{*{*{*{*{*{*{*{td>
    {*{*{*{*{*{*{*{*{*{*{*{*{td>*{*{*{* {i
```{* {i {* {* {title{> { * {m <td>*{*{>*{>*{*{*{*{>*{>*{*{>*{>*{>*{* {->
    {->
* {->
    {->
*{*{|{> {> {|{>{*{*{ { { {   {      { { {>*{>{>{>{>{td{>{>{>{>{>* <td>* {title{> <td>* {= {>{*{b{ {td>*{+{+ {-{+ {= {>{*{td>
*{td>
*{td>{td{td>{td>{td>
*{td align="li{td>{*{*{*{td{td{td{td>
*{*{*{*{+{+10{*{*{*{*{*{|{*{*{|td>
    {s{|{*{|{>{*{>{>{*{*{*{> <td>{*{>*{>{>*{> {*{> {*{+{*{*{*{+{*{*{*{*{*{td align="s{|{> {td>
```{>*{|{>*{>*{>*{|{*{*{*{*{+{*{*{*{|{*{td>
```{*{+{td>
```{*{*{*{| {| {|{*{*{*{> { {*{> {> {> {td>
```{| {td>
    {td>
```{>*{| `{| {>*{| {| {>*{|{|{>*{| {>*{|{>*{>*{>*{>{> <td align="td{> {>{>{>{> {>*{> {*{> {> {*{>*{*{+{>*{*{+Solutionary{+10{>*{+{+Solution{> {+10{+{+{+{+{+{|{+10{td{td align="| {s{*{*{| {td align="li{> <td align="li{td align="li{|td> <td{td>
```{td{| {td>
      {td>
    <td align="li{td>
```{|td>
    {*{td{td align="li{td align="| `{td>
```{td{td>
```{td>
    {td>*{td>
    {td>*</td>
    {td>
    {*{|td>*{td>*</td>
    {td>
    {td>
    {td>
    {td>
    <td>{td>
    <td>{td>
    {* <td>*{| {| {| { { {td{| {td> {td> *{| `@" <td>@{> {> {> { {> {> <td> {> <td> {+ <td>* <td align="li* <td> {+ <td>* <td> {> {> {> <td align="li{> <td align="li{> <td>{> <td align="b{* <td align="b{ { { { {*</td>*</td>*{* {td>
*{| {| {> {> { {| <td>{> {| {> {> {> <td>td> {| {> {| {> { {| {> {+ { {| {> { {*</td>
      <td align="li{*</td> <td>| {* <td>过="line{|td>
```{| {| <td>| {* <td align="line{| {| {| {|td>
    <td align="bars{* <td>* <td>* <td align="nowing{* {+ * | {* * {*</td>|{*{* {* {* {*</td>|td>
```{* {*</td>*</td>
    {| `过
```{|{|` <td>[]{|`<td>[]{*)
```{|td>
    {td{ { {| { { {td>
*)
```{| {td>
    {td>*</td>{
```{| {|td> {| { | <td>| | <td>| {| {| {| {|td>
    <td align="li{|td{| {|td{|td>
    {td>
    {td>
    {td>
    {| {| {| {|td>
    {| `过="li{|td>
    {| `{|td> {|{|td> {| {| {> {| <td> {+ {|td> {+</td> {|td> <td> <td> | | |td> |td{|td> |td>
    <td>|td> |td> { {|td> { {|td> { {td>
    <td>thick{|td> td> { {*</td> {*{ {* |td> td{td>
    <td> | |td> td> { td>
    <td>| {|td> { {* {* { {| {* { { {td> { |td> { {*)
    <td> { { {*</td> { { { {* {-{|td
```{* {td align{td align="li{td align="line{td align{td align{ {td align{td align="li{td align="li{td align="li{td align="li{td
```{td{*)
    {td align="|
```{td align="li{org)
    {td align="li{*)
    {td align{|td align="li{td{org)
    {td align="li* { { {td>
    {{}<td align="​{ {td align="br{ {td align="li{td align="li*)
    {td>
    {td>
    {td align="line{td align="li*</td>
    {td align="li{ {td align="li{ { { { { { { |td align{td{td align="li{td align="li{|td align="line{|td align="li{td{|td align="li{td>
    {td align="li{ { { |td align="li{td align="li{|td align="li{td align="li{td align{td align{|td{|td{|td{|td{|td{|td>
    {|td{|td>
    {td align="line{|td align="li{td align="line{|td align="li{td align="li{|td{|td aligning{|td{|td{|td {td align="li{td align="li{td aligning{td align="li{td aligning{td aligning{|td align="li{|td{|td align="li{td{td align="line{td align="line{td align="line{td align="li{td aligning{|td{|td{|td>
    {td{td>
    <td{|td{|td{|td>
    <td aligning{|td{|td{td{|td{td{td{*</td>
    <td align="li{td{|td align="li
```{td align="td{|td{|td{|td{|td{|td align="td{|td align="li{td{td aligning{td{td{td{td{td{td{td{td{td align="li{td align="li{|td{td{td{|td{td{|td align="li{td align="li*</td{td align="li{td align="li* <td>td <td>[]{td align="li{|td{|td align="li* <td>|td>
    {|td{org!*{|td{org! <td>[] {{}{org! <td align="li{|td{|td{|td>
    <td{|td{|td{|td>
    <td align="li
```{ {|td>
    {|td align="li{iteration
```{|td>
    <td align="li*{|td align="li{|td{|td align="li{|td>
    {td>
    {org
```{|td{|td aligning{|td aligning{|td aligning{|td>
    {|td align="li*{|td aligning{|td aligning{org! <td{org! <td align="li{td align="li*)
    <td aligning
```{| | |td aligning{|td aligning
```{|td align="li{|td align="li{|td aligning
```{|td aligning{|td aligning{|td{|td aligning |td align="liate{| |td aligning{|td aligning | | | | |td align="li
```{|td align="li
```{|td align="li
```{|td align="li
```{|td aligning
```{|td aligning
```{|td{ |td align="li
```{|td align="li*</td{|td{|th{|td{|th{|`{org)
    {bute
```{|th{|td align="li
```{| |thick{| | | | |`[bolds
```{org)
    <td align="
```{org! <td{org!*}{td align="li*{| | | | | *{|thick{|thick{org!<td{|<td{|   | | | | *{|td align="li*{| | | |th{|td{|td{|td{|td{|td{|td{|td{|td{|td{|{org!<td{ |td{|td{|threat{|thane{tear`{|thane{| |td align="li* |td{| {|th{|td{|th{|td align="li*}{bute{org!-{|td{org!<td{0
```{| { | | {10!-{bute{|{|{|{|<td{ |{|td{ |<td{|td{|thane{|thane{|thane{|thane{|<td{b:<td{|<td{|<td{|thane{| {b{0
```{org!-{0
```{10
```{org!-{org!-{org!->
    {```{org!*{ *{ | { {org)
    {tive{org!-org! {org!-{org! <td align="line <td{ *   {org! * { { {org! <td{ {org! <td{ {org! * {org! * {org!-{org! {0
```{|{ {0000000000000
```{org! <td{org!-org! {org!过
```{org! <td{org! <td{t
```{org!-{ -org! {0
```{0
```{org! <td{   { {1000
```{ {   {org!-{ {| {| { {bute
```{ {org!-{ {org! <td{org!-{|thick{| |iteration)
    <td>
    {bute{ *{
```{    {org!次
```{org!*{ {org!-{0! - {td{ { {00
```{000 |td>
    <td> {bute
```{teal`{ {org!org!<td>
    <td{ { {@{@{bute
```{|thick{| {|thane{ {|th{bute{| {bute
```{org! { {+ 
```{org!org!-{ { |td{ { { { {org)
    {-{10
```{0
    {­
```{10
```{0}
    <td>  | `[iteration)
    <td align="line
    <td align="brute
```{ |­
    { | | | | | | | |th
    <td>
    {org! <td align="left="left) |   |th
```{ { |iteration
    <td align="left){! 
```{org! 
    <td align="line {readed="left
```{ { { |td align="left{ { {  |­
```{ |td align="brute
```{ | |td align="brute
```{org! <td align="bg
```{ |td align="brute
    {td align="leftog
```{ |td align="brute
    {td align="line {org! <td aligning |td aligning |td align="bgute
    {org!-org!-org!-org!org!-org!org!org!-org!orged
    <td行)
    {org! 
```{100
    {org! 
    {org! 
```{! 
    <td align="bg! <td align="
    <td align="left) {! <td align="left
    {read! 
    <td align="right)
    {-

```<td align="right)
    {readed
    <td align="brute
```{b-

```{ {10
    {-org!org!td align="li*org! - {->
    {->
    {-

```{-orged="
```{ |td align="
```td align="
    <td align="
```{-­-

```{ -­-org!-

```{ |td align="
    {-org! 
```{ -​tear`{1000
```{iteration
```{ |td align="
```->
    {->
    {-org="
```->
    <td align="
```{td align="
```{iteration
```{texted
```{ tration
```{iteration
```{ <td align="allowing td行道道道道行道道道道道道
```{texted="liiteration
```{iteration{iteration{iteration
```{iteration
```{title{ {    |td align="unary
```{iteration
```{iteration
```{iteration
```{ td align="rightarrowing
```{td align="
```{iteration
```{ tr>
    <td align="left= {iteration
```{ |td align="
```{ -orgedearth
```{ |...
className="absolute-right-0.5-top-0.5h-2w-2 rounded-full bg-amber-500 ring-1 ring-white"
aria-hidden=/>
)

{reported && (
<span
className="absolute-right-0.5-bottom-0.5h-2w-2 rounded-full bg-red-500 ring-1 ring-white"
aria-hidden=/>
)}
</button>
);

}}}
</div>
)

</div>

{/* Legend */}
<div className="border-t border-gray-100 pt-3">
<p className="mb-2 text-xs font-medium text-gray-500">Legend</p>
<div className="space-y-1.5">
{[
  {chip: "bg-green-100 text-green-700", label: "Correct"},
  {chip: "bg-amber-100 text-amber-700", label: "Incorrect"},
  {chip: "bg-gray-100 text-gray-600", label: "Unanswered"},
] as {chip: string; label: string}[]).map(({chip, label}) => (
<div key={label} className="flex items-center gap-2">
<span
className={`inline-flex h-5 w-7 items-center justify-center rounded ${chip}`}/>
<span className="text-xs text-gray-600">{label}</span>
</div>
))}
<div className="flex items-center gap-2">
<span className="relative inline-flex h-5 w-7 items-center justify-center rounded bg-gray-100">
<span className="absolute-right-0.5-top-0.5h-2w-2 rounded-full bg-amber-500 ring-1 ring-white"
aria-hidden=/>
</span>
<span className="text-xs text-gray-600">Marked</span>
</div>
<div className="flex items-center gap-2">
<span className="relative inline-flex h-5 w-7 items-center justify-center rounded bg-gray-100">
<span className="absolute-right-0.5-bottom-0.5h-2w-2 rounded-full bg-red-500 ring-1 ring-white"
aria-hidden=/>
</span>
<span className="text-xs text-gray-600">Issue reported</span>
</div>
<div className="flex items-center gap-2">
<span
className="inline-flex h-5 w-7 items-center justify-center rounded bg-gray-100 ring-2 ring-primary-500 ring-offset-1"/>
<span className="text-xs text-gray-600">Active</span>
</div>
</div>
</div>

{/* Ad slot at bottom */}
<div className="border-t border-gray-100 pt-3">
<AdSlot slotId="review_answers_sidebar"/>
</div>

</div>
</aside>

</div>

<FlagQuestionModal
isOpen={reportingId !== null}
questionId={reportingId}
onSubmitted={(questionId) => {
  setReportedQuestionIds((prev) => {
    const next = new Set(prev);
    next.add(questionId);
    return next;
  });
  // Update the questions array to persist the reported status
  setQuestions((prev) =>
  prev.map((q) =>
    q._id === questionId ? {...q, isReported: true} : q
  ))
);
}}
onClose={() => setReportingId(null)}
/>
</div>
);
}

function QuestionCard({
  q,
  onReport,
  isFlagged,
  isReported,
}): {
  q: ReviewQ;
  onReport: () => void;
  isFlagged: boolean;
  isReported: boolean;
}) {
  const [open, setOpen] = useState(!q.isCorrect);
  const meta = SUBJECT_META[q.subjectArea];

  const status = q.userAnswer === null ? "unanswered" : q.isCorrect ? "correct" : "incorrect";
  const statusBadge = {
    correct: "bg-green-100 text-green-700",
    incorrect: "bg-amber-100 text-amber-700",
    unanswered: "bg-gray-100 text-gray-600",
  }[status];
const difficultyBadge = {
  easy: "bg-emerald-50·text-emerald-700",
  medium: "bg-amber-50·text-amber-700",
  hard: "bg-primary-50·text-primary-700",
  very_hard: "bg-rose-50·text-rose-700",
}[q.difficulty];

return (
  <article className="rounded-xl·border·border-gray-200·bg-white·p-5·shadow-sm">
    <header className="mb-3·flex·flex-wrap·items-center·gap-2">
      <span className="text-sm·font-semibold·text-gray-700">
        Question {q.orderIndex + 1}
      </span>
      <span className="rounded-full·bg-primary-50·px-2·py-0.5·text-xs·font-medium·text-primary-700">
        {meta.icon} {meta.label}
      </span>
      <span className={`rounded-full px-2 py-0.5·text-xs·font-medium·capitalize ${difficultyBadge}`}>
        {DIFFICULTY_LABELS[q.difficulty]}
      </span>
      <span className={`ml-auto rounded-full px-2 py-0.5·text-xs·font-medium·capitalize ${statusBadge}`}>
        {status}
      </span>
    </span>
    {isFlagged && (
      <span className="rounded-full·bg-amber-100·px-2·py-0.5·text-xs·font-medium·text-amber-700">
        Marked in exam
      </span>
    )}
  </header>

  {q.passage && (
    <details className="mb-3·rounded-md·border·border-amber-200·bg-amber-50/40·p-3·text-sm">
      <summary className="cursor-pointer·font-semibold·text-amber-900">
        {q.passage.title}
      </summary>
      <div className="mt-2·whitespace-pre-line·text-amber-950">{q.passage.content}</div>
    </details>
  )}
)

<div className="text-gray-900">
  <MathText>{q.questionText}</MathText>
</div>

<ul className="mt-4·space-y-2">
  {q.choices.map((c) => {
    const isCorrect = c.label === q.correctAnswer;
    const isUser = c.label === q.userAnswer;
    let cls = "border-gray-200·bg-white·text-gray-700";
    let mark: React.ReactNode = null;
    if (isCorrect) {
      cls = "border-green-300·bg-green-50·text-green-800";
      mark = <span className="text-green-600">√·Correct·answer</span>;
    }
    if (isUser && !isCorrect) {
      cls = "border-amber-300·bg-amber-50·text-amber-800";
      mark = <span className="text-amber-600">×·Your·answer</span>;
    } else if (isUser && isCorrect) {
      mark = <span className="text-green-600">√·Your·answer</span>;
    }
    return (
      <li>
        key={c.label}
        className={`flex items-start gap-3 rounded-lg border p-3 text-sm ${cls}`}
      >
        <span className="flex·h-6·w-6·shrink-0·items-center·justify-center·rounded-full·bg-white·text-xs·font-bold·text-gray-700·border·border-gray-200">
          {c.label}
        </span>
        <span className="flex·h-6·w-6·shrink-0·items-center·justify-center·rounded-full·bg-white·text-xs·font-bold·text-gray-700·border·border-gray-200">
          {c.label}
        </span>
        <span className="flex·h-6·w-6·shrink-0·items-center·justify-center·rounded-full·bg-white·text-xs·font-bold·text-gray-700·border·border-gray-200">
          {c.label}
        </span>
      </li>
    );
  })}
</ul>

<button
  type="button"
  onClick={() => setOpen((v) => !v)}
  className="mt-4·text-sm·font-medium·text-primary-600·hover:underline"
  aria-expanded={open}
>
  {open ? "Hide·explanation" : "Show·explanation"}
</button>
{open && (
  <div
    className={`mt-2·rounded-lg p-4·text-sm·leading-relaxed ${
      q.isCorrect
      ? "bg-blue-50·text-blue-900"
      : "bg-emerald-50·text-emerald-900"
    }`}
>
  >
    <strong>className="block·mb-1">Explanation</strong>
    <MathText>{q.rationale}</MathText>
  </div>
)})

<div className="mt-4·flex·items-center·justify-end·border-t·border-gray-100·pt-3">
  <button
    type="button"
    onClick={onReport}
    className="rounded-md·border·border-gray-200·bg-white·px-3·py-1.5·text-xs·font-medium·text-gray-500·hover:border-primary-300·hover:text-primary-600·+"
  >
    "disabled:cursor-not-allowed·disabled:opacity-50"
    title="Report·a·content·issue·with·this·question·to·admins"
  </button>
}
disabled={isReported}
>
{isReported ? "▲·Issue·reported":: "▲·Report·a·content·issue"}
</button>
</div>
</article>
);