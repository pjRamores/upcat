...
import { API_ROUTES, checkServerStatus } from "@/lib/sessionRecovery";
  ...
  async init(sessionId) {
    ...
    try {
      ...
      set({
        ...
        pausedAt: payload.session.pausedAt
          ? new Date(payload.session.pausedAt).getTime()
          : null,
  ...
  async submit() {
    ...
    try {
      const flaggedQuestionIds = [
        ...new Set(
          get()
            .states.filter((q) => q.flagged && q.questionId)
            .map((q) => q.questionId),
        ),
      ];
