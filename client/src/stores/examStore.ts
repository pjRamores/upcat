...
import {Difficulty, ExamQuestion, Passage, SubjectArea} from "@upcat/shared";
...
try {
    ...
    const restoredIndex = cached?.sessionId === sessionId
...
try {
    ...
    const extensionMs = status?.maintenanceExtension ?? status?.timerState?.adjustments ?? 0;
...
persistFromStore(get);
    ...
    const msg = (e as { response?: { data?: { error?: string } } }).response?.data?.error || "Failed to load exam";
...
async loadPage(page) {
    const { sessionId } = get();
    if (!sessionId) return;
    const { data } = await apiClient.get(`${API_ROUTES.EXAM.QUESTIONS(sessionId)}?page=${page}&limit=${PAGE_SIZE}`);
    ...
    try {
        ...
        set((s) => ({
            states: s.states.map((st) => ({
                dirty: dirtyIds.has(st.questionId) ? { ...st, dirty: false } : st,
            })),
        }));
        persistFromStore(get);
...
async submit() {
    ...
    try {
        ...
        const msg = (e as { response?: { data?: { error?: string } } }).response?.data?.error || "Failed to submit exam";
