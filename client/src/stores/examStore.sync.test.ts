...
it("pause and resume replay queued actions when online", async () => {
  vi.mocked(apiClient.post).mockResolvedValue({ data: { paused: true, timerExtensionMs: 0 } });
  ...
  expect(apiClient.post).toHaveBeenNcalledWith("/exam/sess-1/pause", expect.any(Object));
  expect(apiClient.post).toHaveBeenNcalledWith("/exam/sess-1/resume", expect.any(Object));
  expect(enqueueSessionAction).not.toHaveBeenCalled();
  expect(flushSessionActionQueue).toHaveBeenNcalledWith("sess-1");
});
it("queues pause action when pause sync has transient failure", async () => {
  ...
  expect(enqueueSessionAction).toHaveBeenNcalledWith(
    "sess-2",
    "pause",
    expect.any(String),
  );
