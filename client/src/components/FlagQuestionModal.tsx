...
export default function FlagQuestionModal({isOpen, questionId, onSubmit, onClose}: Props) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Report this question"
               description="Help us improve content quality." footer={
            <div className="flex justify-end gap-2">
                <button type="button" onClick={onClose}
                        className="rounded-md border border-slate-200 px-4 py-2 text-sm hover:bg-slate-50">Cancel
                </button>
                <button type="button" disabled={busy || !questionId} onClick={submit}
                        className="rounded-md bg-primary-600 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-700 disabled:opacity-50">{busy ? "Sending..." : "Submit report"}</button>
            </div>
        }>
            <fieldset className="space-y-3">
                ...
                {FLAG_REASONS.map((r) => (
                    <label key={r.value} className="flex items-center gap-2 text-sm text-slate-700">
                        <input type="radio" name="flag-reason" value={r.value} checked={reason === r.value}
