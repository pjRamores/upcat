 ...
 */
/**
 * Displays a stack of achievement unlock notifications with rarity-tinted glow. Pass an array of AchievementUnlockEvent objects; the queue is displayed one at a time with auto-dismissal.
 */
            ...
            >
                <div className="rounded-[14px] bg-white p-4">
                    <div className="flex items-start gap-3">
                        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-amber-200 to-amber-400 text-2xl">
                            &
                        </div>
                        ...
                        <button
                            type="button"
                            onClick={onClose}
                            className="rounded-full p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-700 aria-label='Dismiss'"
                        >
                            ×
                        </button>
                    </div>
                ...
                {events.length > 1 && (
                    <div className="mt-3 flex gap-1">
                        {events.map((_, i) => (
                            <div key={i} className={`h-1 flex-1 rounded-full ${i <= index ? "bg-indigo-500" : "bg-slate-200"}`}>
                                />
                            </div>
                        ))}
                    </div>
