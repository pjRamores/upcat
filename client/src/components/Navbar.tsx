    ...
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <nav aria-label="Primary" className="hidden items-center gap-1 md:flex">
          ...
          {isLanding && (
            <a href="#features" onClick={goToFeatures} className="rounded-md px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 hover:text-primary-600 transition-colors">
              Features
            </a>
          )}
        ...
        <div className="flex items-center gap-2">
          {isAuthenticated ? (
            ...
            <div className="hidden md:flex">
              <button onClick={handleLogout} className="text-sm font-semibold text-gray-600 hover:text-gray-800 transition-colors">
                Logout
              </button>
            </div>
          )}
        ...
        {/* Mobile hamburger */}
        <button type="button" onClick={() => setDrawerOpen(true)} aria-label="Open menu" aria-expanded={drawerOpen} aria-controls="mobile-drawer" className="inline-flex h-10 w-10 items-center justify-center rounded-lg text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500 md:hidden">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="h-6 w-6">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4.6h16M4.12h16M4.18h16"/>
            </svg>
        </button>
    </div>
    <MobileDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} isAuthenticated={isAuthenticated} isAdmin={isAdmin} userName={user?.firstName ?? null} userLast={user?.lastName ?? null} isLanding={isLanding} onFeaturesClick={goToFeatures} onLogout={handleLogout} />
</header>
);
}
...
function UserMenu({ user, isAdmin, open, setOpen, onLogout, }) {
    ...
    <div ref={wrapperRef} className="relative">
        ...
        >
            ...
            <svg
                ...
                aria-hidden
            >
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 9.6 6.6 6 6 6.6 9.6 9.6 6 12 9.6 9.6 6 15.6 12 12 15.6 9.6 12 6 15.6 9.6 12 6 9.6 9.6 6 6 6.6 6.6 9.6z"/>
            </svg>
        </button>
        {open && (
            <div
...
function MenuLink({
  ...
  return (
    <Link to={to} role="menuitem" className="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 focus:bg-gray-50 focus:outline-none">
...
function MobileDrawer({
  ...
  const initials = ((userName?.[0] ?? "") + (userLast?.[0] ?? "")).toUpperCase() || "?";
  ...
  return createPortal(
    ...
    >
      ...
      <aside className="animate-drawer-in absolute right-0 top-0 flex h-full w-80 max-w-[85vw] flex-col bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3">
          ...
          <button type="button" onClick={onClose} aria-label="Close menu" className="inline-flex h-10 w-10 items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="h-5 w-5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 12 18 24" />
            </svg>
          </button>
        </div>
        <div className="flex flex-col gap-4 py-4">
          ...
          <MenuLink to="/logout" icon="Logout" onClick={onLogout} />
        </div>
      </aside>
    </div>
  );
