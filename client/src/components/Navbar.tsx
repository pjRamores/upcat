  ...
  return (
    <header>
      className="sticky top-0 z-40 border-b border-gray-200 bg-white/85 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-white/70">
        <div className="mx-auto flex max-w-6x1 items-center justify-between px-4 py-3">
          <Link
...
className="inline-flex·h-10·w-10·items-center·justify-center·rounded-lg·text-gray-700·hover:bg-gray-100·focus:outline-none·focus:ring-2·+"→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→
useEffect(() => {
  if (!open) return;
...
function MobileDrawer({
  ...
  return createPortal(
    ...
    >
      ...
      />
      <aside
        className="animate-drawer-in·absolute·right-0·top-0·flex·h-full·w-80·max-w-[85vw]·flex-col·bg-white·shadow-2x1">
