 ...
 *
 * Animated count-up notification that surfaces after the API returns a GamificationReward
 * (typically from exam submit / practice complete / login). Stacks XP rewards, then plays
 * a level-up burst if applicable.
 */
...
export default function XpAwardOverlay({
  ...
  return (
    <>
      <div
        className="pointer-events-none fixed inset-x-0 top-6 z- flex justify-center px-4"
        role="status"
        aria-live="polite"
      >
        <div
        ...
        >
          <div className="flex items-start gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-amber-300 text-amber-900">
              <svg
                width={24}
                height={24}
                viewBox="0 0 24 24"
                ...
                aria-hidden="true"
              >
                <path d="M5 8h14v6H5V8zm7 3h-3V5H4v14h5V11zm.08-2H8L4 5h.08l4-4 4 4z" />
              </svg>
            </div>
            ...
            <button
              ...
              aria-label="Close XP award overlay"
            >
              X
            </button>
          </div>
      ...
      <style>{`
        ...
        .animate-in-down {
          animation: xp-in-down 280ms ease-out;
        }
      `}</style>
    </>
  );
    ...
    <li className="flex justify-between">
      ...
      <span className="font-mono">
        ...
        {isAchievement && reward.multiplier > 1 && (
          <span className="ml-1 text-amber-200">x{reward.multiplier.toFixed(2)}</span>
        )}
      </span>
    </li>
  );
