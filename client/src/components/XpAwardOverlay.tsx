 ...
 * GamificationReward (typically from exam submit / practice complete /
...
export default function XpAwardOverlay({
    ...
    return (
        ...
        >
            <div
                className="pointer-events-auto w-full max-w-sm overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-600 p-5 text-white shadow-2xl ring-1 ring-white/20 animate-in-down"
                role="status"
            >
                <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-amber-300 text-2xl text-amber-900">
                        <div className="text-2xl font-semibold uppercase tracking-wide opacity-80">XP earned</div>
                        <div className="text-2xl font-bold tabular-nums">{displayedXp.toLocaleString()}</div>
                    </div>
                    <button
                        type="button"
                        onClick={onClose}
                        className="rounded-full bg-white/10 px-2 py-1 text-xs hover:bg-white/20 aria-label='Dismiss'"
                    >
                        X
                    </button>
                </div>
                ...
                {levelUp && (
                    <div className="mt-4 rounded-lg bg-amber-300/95 px-4 py-3 text-center text-amber-900 shadow-inner">
                        <div className="text-xs font-semibold uppercase tracking-wide">Level up!</div>
                        <div className="text-xl font-bold">Level {levelUp.newLevel} - {levelUp.newTitle}</div>
                    </div>
                )}
            </div>
        </div>
