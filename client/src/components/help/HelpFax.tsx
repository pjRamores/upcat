...
<div className="mt-3 space-y-2">
    {results.map((result) => (
        ...
        >
            ...
            <p className="mt-0.5 line-clamp-2 text-xs text-slate-600" dangerouslySetInnerHTML={{ __html: result.excerpt }} />
        </Link>
    ))}
</div>

<div className="mt-4 flex flex-wrap gap-3 text-sm">
    <Link to="/help" onClick={() => setOpen(false)} className="font-medium text-primary-700 hover:underline">View Full Help Center</Link>
    <Link to="/contact" onClick={() => setOpen(false)} className="text-slate-700 hover:underline">Contact Support</Link>
    {isAuthenticated && (
        <button type="button" onClick={() => void replayTour()} className="text-slate-700 hover:underline">Replay Tour</button>
    )}
</div>
</section>
</div>
