  ...
  return (
    <div role="dialog" aria-live="polite" aria-label="Cookie and ads consent" data-testid="consent-banner" className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white shadow-2xl">
      <div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-slate-700">We use cookies to serve relevant ads and improve the UPCAT Simulator experience. See our{" "}
          <Link to="/privacy" className="underline hover:text-primary-700">privacy policy</Link> for details.
        </p>
        <div className="flex flex-shrink-0 gap-2">
          <button type="button" data-testid="consent-decline" onClick={() => setConsent("denied")} className="rounded border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">Decline</button>
          <button type="button" data-testid="consent-accept" onClick={() => setConsent("granted")} className="rounded bg-primary-600 px-4 py-2 text-sm font-medium text-white hover:bg-primary-700">Accept</button>
        </div>
      </div>
    </div>
  );
