...
import {
  ...
  resolveVideoTriggerSettings,
} from "@/hooks/useVideoInterstitial";
...
export default function VideoAdModal({
  ...
  return createPortal(
    <div
      ...
      className="fixed inset-0 z- flex items-center justify-center bg-black/90 p-4"
    >
      <div className="relative flex w-full max-w-3xl flex-col gap-3">
        {canSkip ? (
          <button
            ...
            className="self-end rounded bg-white/90 px-3 py-1.5 text-sm font-medium text-slate-900 hover:bg-white"
          >
            Skip ad
          </button>
        ) : (
          <span
            ...
            className="self-end rounded bg-black/60 px-3 py-1.5 text-xs text-white"
          >
            Skip in {remaining}s
          </span>
        )}
        ...
        {testMode && <TestModeTicker onTick={() => setWatched((w) => w + 1)} />}
