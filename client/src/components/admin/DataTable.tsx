            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-200">
                        {rows.map((row) => (
                            <tr key={getRowId(row)} className={`bg-white hover:bg-gray-50 ${isLoading ? "cursor-wait" : ""}`}>
                                {selectable && (
                                    <td className="w-10 px-3 py-3 text-center">
                                        <input
                                            ...
                                            ref={(el) => {
                                                if (el) el.indeterminate = !!selectedIds?.has(getRowId(row));
                                            }}
                                ...
                                {columns.map((col) => (
                                    <td key={`${getRowId(row)}-${col.key}`} className={`px-4 py-3 ${col.className ?? ""}`}>
                                        {row[col.key]}
                                    </td>
                                ))}
                            </tr>
...
{isLoading ? (
    ...
    rows.map((row, i) => {
        ...
        return (
            <tr key={id} className={`${onRowClick ? "cursor-pointer hover:bg-slate-50" : ""} ${selectedIds?.has(id) ? "bg-primary-50/50" : ""}`} onClick={(e) => {
                ...
                {columns.map((col) => {
                    return col.render ? col.render(row, i) : (row as Record<string, unknown>)[col.key] as ReactNode ?? "-";
                })}
            </tr>
        );
    })
...
export function Pagination({
    ...
    return (
        <div className="flex items-center justify-between gap-3 border-t border-slate-200 px-4 py-3 text-xs">
            <span className="text-slate-500">Page {page} of {Math.max(1, totalPages)} • {total} record{total === 1 ? "" : "s"}</span>
            <div className="flex items-center gap-1">
                <button type="button" disabled={page <= 1} onClick={() => onPageChange(page - 1)} className="rounded-md border border-slate-200 px-2 py-1 disabled:opacity-40">← Prev</button>
                <button type="button" disabled={page >= totalPages} onClick={() => onPageChange(page + 1)} className="rounded-md border border-slate-200 px-2 py-1 disabled:opacity-40">Next →</button>
            </div>
        </div>
    );
}