...
function renderMath(tex: string, displayMode: boolean, key: string): ReactNode {
  ...
  return (
    <span key={key} style={displayMode ? { display: "block", margin: "0.5em 0" } : undefined} dangerouslySetInnerHTML={{ __html: html }} />
  );
}
...
function parse(text: string): ReactNode[] {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[1] !== undefined) {
      parts.push(renderMath(match[1], true, `b-${key++}`));
    } else if (match[2] !== undefined) {
      parts.push(renderMath(match[2], false, `i-${key++}`));
    }
