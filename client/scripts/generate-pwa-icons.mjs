 ...
 */
import { mkdirsSync, writeFileSync } from "node:fs";
...
mkdirsSync(OUT_DIR, { recursive: true });
...
script
const cornerR = maskable ? 0 : Math.round(size * 0.15);
...
function generate(size, name, { maskable = false } = {}) {
    ...
    console.log(`\x1b[32m${path}\x1b[0m (${png.length.toLocaleString()} bytes)`);
}
